...
SH_DECL_MANUALHOOK2_void(ResolveFlyCollisionCustom, 0, 0, 0, trace_t &, Vector &);
SH_DECL_MANUALHOOK1(ShouldTransmit, 0, 0, 0, int, const CCheckTransmitInfo *);
...
SH_DECL_MANUALHOOK2_void(ComputeWorldSpaceSurroundingBox, 0, 0, 0, Vector *, Vector *);
...
SH_DECL_MANUALHOOK0(GetDamage, 0, 0, 0, float); 
SH_DECL_MANUALHOOK1_void(SetDamage, 0, 0, 0, float); 
SH_DECL_MANUALHOOK0(VPhysicsIsFlesh, 0, 0, 0, bool); 
SH_DECL_MANUALHOOK1_void(UpdatePhysicsShadowToCurrentPosition, 0, 0, 0, float); 
SH_DECL_MANUALHOOK2_void(Event_KilledOther, 0, 0, 0, CBaseEntity *, const CTakeDamageInfo &); 
...
DECLARE_HOOK(ResolveFlyCollisionCustom, CEntity);
DECLARE_HOOK(ShouldTransmit, CEntity);
...
DECLARE_HOOK(ComputeWorldSpaceSurroundingBox, CEntity);
...
DECLARE_HOOK(GetDamage, CEntity);
DECLARE_HOOK(SetDamage, CEntity);
DECLARE_HOOK(VPhysicsIsFlesh, CEntity);
DECLARE_HOOK(UpdatePhysicsShadowToCurrentPosition, CEntity);
DECLARE_HOOK(Event_KilledOther, CEntity);
...
DECLARE_DEFAULTHANDLER_void(CEntity,TraceAttack, (const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump), (info, vecDir, ptr, dump));
...
DECLARE_DEFAULTHANDLER_SPECIAL(CEntity, EarPosition, Vector, () const, (), vec3_origin);
DECLARE_DEFAULTHANDLER_SPECIAL(CEntity, GetAutoAimCenter, Vector, (), (), vec3_origin);
DECLARE_DEFAULTHANDLER_SPECIAL(CEntity, EyePosition, Vector, (), (), vec3_origin);
...
DECLARE_DEFAULTHANDLER_void(CEntity, ResolveFlyCollisionCustom, (trace_t &tr, Vector &vecVelocity ), (tr, vecVelocity));
DECLARE_DEFAULTHANDLER(CEntity,ShouldTransmit, int, (const CCheckTransmitInfo *pInfo), (pInfo));
...
DECLARE_DEFAULTHANDLER_void(CEntity, ComputeWorldSpaceSurroundingBox, (Vector *pWorldMins, Vector *pWorldMaxs), (pWorldMins, pWorldMaxs));
DECLARE_DEFAULTHANDLER(CEntity, ShouldAttractAutoAim, bool, (CBaseEntity *pAimingEnt ),(pAimingEnt));
DECLARE_DEFAULTHANDLER_REFERENCE(CEntity, GetViewOffset, const Vector &, (), ());
...
DECLARE_DEFAULTHANDLER(CEntity, GetDamage, float, (), ());
DECLARE_DEFAULTHANDLER_void(CEntity, SetDamage, (float flDamage), (flDamage));
...
DECLARE_DEFAULTHANDLER_void(CEntity, Event_KilledOther, (CBaseEntity *pVictim, const CTakeDamageInfo &info), (pVictim, info));
...
DEFINE_PROP(m_flElasticity, CEntity);
...
DEFINE_PROP(m_iParent, CEntity);
DEFINE_PROP(m_flSimulationTime, CEntity);
DEFINE_PROP(m_ubInterpolationFrame, CEntity);
...
bool CEntity::InternalDispatchKeyValue(const char *szKeyName, const char *szValue)
{
	...
	//if(pEnt->IsCustomEntity())
	{
		...
		{
			if(ParseKeyvalue(pEnt, dmap->dataDesc, dmap->dataNumFields, szKeyName, szValue))
			{
...
Vector CEntity::InternalGetSoundEmissionOrigin()
{
	...
	if (!pEnt)
	{
		RETURN_META_VALUE(MRES_IGNORED, vec3_origin);
	}
...
CEntity *CEntity::FirstMoveChild(void)
{
	return Instance(m_hMoveChild); 
}

CEntity *CEntity::NextMovePeer(void)
{
	return Instance(m_hMovePeer); 
}
...
{
	int nResult = m_pEdict ? m_pEdict->m_EdictIndex : 0;
	Assert( nResult == engine->IndexOfEdict(m_pEdict) );
	return nResult;
}
...
{
	SetOwnerEntity((pOwnerEntity)? pOwnerEntity->BaseEntity() : NULL);

	//(*m_hOwnerEntity.ptr).Set((pOwnerEntity)? pOwnerEntity->BaseEntity() : NULL);
}
...
{
	servertools->SetMoveType(BaseEntity(), val, moveCollide);
}
...
void CEntity::AddEffects(int nEffects)
{
	...
	if(m_pEdict)
		m_pEdict->StateChanged(m_fEffects.offset);
}
...
void CEntity::ApplyAbsVelocityImpulse( const Vector &inVecImpulse )
{
...
void CEntity::SUB_Remove_CBE()
{
	CEntity *cent = (CEntity *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->SUB_Remove();
	}
...
int	CEntity::PrecacheModel( const char *name, bool bPreload )
{
	return g_helpfunc.PrecacheModel(name, bPreload);
}
...
void CEntity::DispatchTraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
	...
	TraceAttack( info, vecDir, ptr, dump );
}
...
trace_t *CEntity::GetTouchTrace( void )
{
	return g_TouchTrace;
}
...
void CEntity::ResolveFlyCollisionBounce( trace_t &trace, Vector &vecVelocity, float flMinTotalElasticity )
{
	...
	if ( trace.plane.normal.z > 0.7f )			// Floor
	{
		...
		CEntity *pEntity = CEntity::Instance(trace.m_pEnt);
		Assert( pEntity );
		...
		if ( flSpeedSqr < ( 30 * 30 ) )
		{
			...
			{
				SetGroundEntity( pEntity->BaseEntity() );
			}
...
void CEntity::PhysicsPushEntity( const Vector& push, trace_t *pTrace )
{
	g_helpfunc.PhysicsPushEntity(BaseEntity(), push, pTrace);
}

#define	STOP_EPSILON	0.1
int CEntity::PhysicsClipVelocity( const Vector& in, const Vector& normal, Vector& out, float overbounce )
{
	float	backoff;
	float	change;
	float angle;
...
bool CEntity::IsStandable() const
{
...
bool CEntity::DispatchKeyValue( const char *szKeyName, int iValue )
{
	char string[256];
	Q_snprintf(string,sizeof(string), "%i", iValue );
	return CustomDispatchKeyValue( szKeyName, string );
}

bool CEntity::DispatchKeyValue( const char *szKeyName, float fValue )
{
	char string[256];
	Q_snprintf(string,sizeof(string), "%f", fValue );
	return CustomDispatchKeyValue( szKeyName, string );
}

bool CEntity::DispatchKeyValue( const char *szKeyName, const Vector &vecValue )
{
	char string[256];
...
bool CEntity::IsInWorld( void ) const
{  
	if ( !edict() )
		return true;

	Vector vec = GetAbsOrigin();
	// position 
	if (vec.x >= MAX_COORD_INTEGER) return false;
	if (vec.y >= MAX_COORD_INTEGER) return false;
	if (vec.z >= MAX_COORD_INTEGER) return false;
	if (vec.x <= MIN_COORD_INTEGER) return false;
	if (vec.y <= MIN_COORD_INTEGER) return false;
	if (vec.z <= MIN_COORD_INTEGER) return false;
	// speed
	if (vec.x >= 2000) return false;
	if (vec.y >= 2000) return false;
	if (vec.z >= 2000) return false;
	if (vec.x <= -2000) return false;
	if (vec.y <= -2000) return false;
	if (vec.z <= -2000) return false;
...
IPhysicsObject *CEntity::GetPhysicsObjectNum(int num)
{
	if(!IsWorld())
	{
		if(num < 0)
		...
		IPhysicsObject *pEntitypList[VPHYSICS_MAX_OBJECT_LIST_COUNT];
		int c = VPhysicsGetObjectList( pEntitypList, ARRAYSIZE(pEntitypList) );
		if(c)
		{
			if(num >= c)
			{
...
CE_CEntityBlocker *CE_CEntityBlocker::Create( const Vector &origin, const Vector &mins, const Vector &maxs, CBaseEntity *pOwner, bool bBlockPhysics )
{
...
void CEntity::IncrementInterpolationFrame()
{
