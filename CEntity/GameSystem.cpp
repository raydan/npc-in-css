...
extern void CleanupUtlSymbolTable_Shutdown();
...
public:
	...
	void Shutdown()
	{
		CleanupUtlSymbolTable_Shutdown();
...
{
	if(s_GameSystems == NULL)
		return;
	s_GameSystems->FindAndRemove(&g_CHookedValveAutoSystem);
...
const char *CPropData::GetRandomChunkModel( const char *pszBreakableSection, int iMaxSize )
{
	...
	// Now pick a enginerandom one and return it
	int iRandom;
	if ( iMaxSize == -1 )
	{
