...
public:
	...
	IHandleEntity *GetIHandleEntity() const
	{
		return ((CBaseHandle &)(*this)).Get();
	}
	operator CEntity*()
	{
		return Get();
...
public:
	...
	{
		Assert(0);
		(*ptr).Set(input.GetIHandleEntity());
		if(pEdict)
			pEdict->StateChanged(offset);
		return *this;
	}
...
public:
	...
	Redirect& operator =(const Redirect<CFakeHandle>& input)
	{
		Assert(0);
		CFakeHandle& hh = input;
		(*ptr).Set(hh.GetIHandleEntity());
		return *this;
	}
	bool operator ==(const CEntity *lhs)
	{
