...
public:
	...
	void SetAmmo(int nAmmoIndex, int iAmmount);
	...
	CCombatWeapon		*Weapon_Create( const char *pWeaponName );
	CEntity				*FindHealthItem( const Vector &vecPosition, const Vector &range );
	CEntity				*Weapon_FindUsable( const Vector &range );
	bool				Weapon_IsOnGround( CCombatWeapon *pWeapon );
	CCombatWeapon		*Weapon_GetWpnForAmmo( int iAmmoIndex );
...
public:
	...
	DECLARE_DEFAULTHEADER(GiveAmmo, int, ( int iCount, int iAmmoIndex, bool bSuppressSound = false ));
...
public: //Sendprops
	...
	DECLARE_SENDPROP(CEFakeHandle<CCombatWeapon> , m_hActiveWeapon);
