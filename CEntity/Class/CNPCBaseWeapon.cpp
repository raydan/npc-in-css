...
CNPCBaseWeapon::CNPCBaseWeapon()
{

}

void CNPCBaseWeapon::DelayRemove_CBE()
{
	CNPCBaseWeapon *cent = (CNPCBaseWeapon *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->DelayRemove();
	}
...
void CNPCBaseWeapon::Drop( const Vector &vecVelocity )
{
	if(m_bIsNPCUsing)
	{
		SetContextThink(&CNPCBaseWeapon::DelayRemove_CBE, gpGlobals->curtime + g_MonsterConfig.m_fNPCWeaponRemoveDelay, s_pDelayRemoveThinkContext); 
	}
