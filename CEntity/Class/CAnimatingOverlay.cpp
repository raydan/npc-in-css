...
void CAnimatingOverlay::RemoveGesture( Activity activity )
{
...
bool CAnimatingOverlay::HasActiveLayer( void )
{
	for (int j = 0; j < m_AnimOverlay->Count(); j++ )
	{
		CAnimationLayer	*p_m_AnimOverlay = &m_AnimOverlay->Element(j);
		if ( p_m_AnimOverlay->IsActive() )
			return true;
	}
	return false;
