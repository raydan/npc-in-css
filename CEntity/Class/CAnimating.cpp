...
float CAnimating::GetPoseParameter( const char *szName )
{
...
int	CAnimating::GetPhysicsBone( int boneIndex )
{
	CStudioHdr *pStudioHdr = GetModelPtr();
	if ( pStudioHdr )
	{
...
{
	unsigned char value = *(m_nMuzzleFlashParity) + 1;
	m_nMuzzleFlashParity = value & ((1 << EF_MUZZLEFLASH_BITS) - 1);
}

void CAnimating::ResetSequence(int nSequence)
{
	servertools->ResetSequence(BaseEntity(), nSequence);
}

void CAnimating::ResetSequenceInfo()
{
	servertools->ResetSequenceInfo(BaseEntity());
}
...
int CAnimating::TranslatePhysBoneToBone( int physicsBone )
{
	Assert( GetModelPtr() );
	CStudioHdr *pStudioHdr = GetModelPtr( );
	for ( int i = 0; i < pStudioHdr->numbones(); i++ )
	{
		if ( pStudioHdr->pBone( i)->physicsbone == physicsBone )
		{
			return i;
		}
	}
...
bool CAnimating::GotoSequence( int iCurrentSequence, float flCurrentCycle, float flCurrentRate, int iGoalSequence, int &nNextSequence, float &flNextCycle, int &iNextDir )
{
...
void CAnimating::UseClientSideAnimation()
{
	m_bClientSideAnimation = true;
}

int CAnimating::GetEntryNode( int iSequence )
{
	CStudioHdr *pstudiohdr = GetModelPtr();
	if (! pstudiohdr)
		return 0;
