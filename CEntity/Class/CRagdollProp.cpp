...
void CE_CRagdollProp::FadeOut( float flDelay, float fadeTime)
{
	...
	m_flFadeScale = 0;
	SetContextThink( &CE_CRagdollProp::FadeOutThink_CBE, gpGlobals->curtime + flDelay + 0.01f, s_pFadeOutContext );
...
void CE_CRagdollProp::FadeOutThink_CBE()
{
	CE_CRagdollProp *cent = (CE_CRagdollProp *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->FadeOutThink();
	}
...
void CE_CRagdollProp::FadeOutThink()
{
	...
	if ( dt < 0 )
	{
		SetContextThink( &CE_CRagdollProp::FadeOutThink_CBE, gpGlobals->curtime + 0.1, s_pFadeOutContext );
	}
	else if ( dt < m_flFadeTime )
	{
		...
		NetworkStateChanged();
		SetContextThink( &CE_CRagdollProp::FadeOutThink_CBE, gpGlobals->curtime + TICK_INTERVAL, s_pFadeOutContext );
	}
	else
	{
		...
		CPlayer *pPlayer = UTIL_GetLocalPlayer();
		CTakeDamageInfo info( (pPlayer)?pPlayer->BaseEntity():NULL, (pPlayer)?pPlayer->BaseEntity():NULL, 10000.0, DMG_GENERIC );
		TakeDamage( info );
		UTIL_Remove( this );
	}
}
