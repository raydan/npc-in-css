...
//CE_LINK_ENTITY_TO_CLASS(CWeaponCSBase, CCombatWeapon);
CE_LINK_ENTITY_TO_CLASS(CBaseCombatWeapon, CCombatWeapon);


extern void PreWeaponReplace(CCombatWeapon *pWeapon);
...
DECLARE_DEFAULTHANDLER_void(Template_CCombatWeapon, Weapon_SetActivity, (Activity act, float duration), (act, duration));

SH_DECL_MANUALHOOK1(SendWeaponAnim, 0, 0, 0, bool,int);
DECLARE_HOOK(SendWeaponAnim, Template_CCombatWeapon);
DECLARE_DEFAULTHANDLER(Template_CCombatWeapon, SendWeaponAnim, bool, (int iActivity), (iActivity));
...
CCombatCharacter *CCombatWeapon::GetOwner()
{
...
{
	if(m_bRemoveOnDrop || g_MonsterConfig.m_bEnableRemoveDropWeapon)
	{
		UTIL_Remove(this);
	}
}
...
bool CCombatWeapon::UsesSecondaryAmmo( void )
{
