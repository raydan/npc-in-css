...
bool CAI_Navigator::ShouldSimplifyTo( bool passedDetour, const Vector &pos )
{
	...
	GetMoveProbe()->MoveLimit( GetNavType(), 
		...
		(cent)?cent->BaseEntity():NULL, 100, flags, &moveTrace );
...
bool CAI_Navigator::PrependLocalAvoidance( float distObstacle, const AIMoveTrace_t &directTrace )
{
	...
	if ( distObstacle < GetHullWidth() * 0.5 )
	{
		...
		if ( GetMoveProbe()->MoveLimit( GetNavType(), GetLocalOrigin(), vTestBackaway, 
													 MASK_NPCSOLID, (cent)?cent->BaseEntity():NULL, 
													 100.0, 
													 flags, &backawayTrace ) )
		{
...
bool CAI_Navigator::DoFindPathToPos(void)
{
	...
	AI_Waypoint_t *pFirstWaypoint = pPathfinder->BuildRoute( origin, actualGoalPos, (pTarget)?pTarget->BaseEntity():NULL, tolerance, GetNavType(), m_bLocalSucceedOnWithinTolerance );
...
AIMoveResult_t CAI_Navigator::MoveClimb() 
{
	...
	CEntity *cent = GetNavTargetEntity();
	if ( climbDist > 0.01 &&
		!GetMoveProbe()->MoveLimit( NAV_CLIMB, GetLocalOrigin(), GetLocalOrigin() + ( climbDir * MIN(0.1,climbDist - 0.005) ), MASK_NPCSOLID, (cent)?cent->BaseEntity():NULL, &moveTrace ) )
	{
...
AIMoveResult_t CAI_Navigator::MoveJump()
{
	...
	if ( (GetNavType() != NAV_JUMP) && (GetEntFlags() & FL_ONGROUND) )
	{
		...
		GetMoveProbe()->MoveLimit( NAV_JUMP, GetLocalOrigin(), GetPath()->CurWaypointPos(), 
			MASK_NPCSOLID, (cent)?cent->BaseEntity():NULL, &moveTrace );
...
bool CAI_Navigator::SimplifyPathBacktrack()
{
		...
		(pCurWaypoint->Flags() & bits_WP_TO_NODE) )	
	{
		...
		if (firstProj>0.5*firstDist) 
		{
			...
			CEntity *cent = GetPath()->GetTarget();
			AI_Waypoint_t *route1 = GetPathfinder()->BuildLocalRoute(GetLocalOrigin(), targetPos, (cent)?cent->BaseEntity():NULL, bits_WP_TO_DETOUR, NO_NODE, buildFlags, goalTolerance);
			if (!route1) 
				return false;

			// Make sure the new point gets me to the target location
			cent = GetPath()->GetTarget();
			AI_Waypoint_t *route2 = GetPathfinder()->BuildLocalRoute(targetPos, pNextWaypoint->GetPos(), (cent)?cent->BaseEntity():NULL, bits_WP_TO_DETOUR, NO_NODE, buildFlags, goalTolerance);
			if (!route2) 
			{
				DeleteAll(route1);
				return false;
