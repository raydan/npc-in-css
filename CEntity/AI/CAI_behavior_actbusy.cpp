...
void CE_AI_ActBusyQueueGoal::InputActivate( inputdata_t &inputdata )
{
	if ( !IsActive() )
	{
		...
		SetContextThink( &CE_AI_ActBusyQueueGoal::QueueThink_CBE, gpGlobals->curtime + 5, QUEUE_THINK_CONTEXT );
	}
...
void CE_AI_ActBusyQueueGoal::MoveQueueUp( void )
{
	...
	if ( GetNextThink( QUEUE_MOVEUP_THINK_CONTEXT ) < gpGlobals->curtime )
	{
		...
		SetContextThink( &CE_AI_ActBusyQueueGoal::MoveQueueUpThink_CBE, flTime, QUEUE_MOVEUP_THINK_CONTEXT );
	}
}

void CE_AI_ActBusyQueueGoal::MoveQueueUpThink_CBE()
{
	CE_AI_ActBusyQueueGoal *cent = (CE_AI_ActBusyQueueGoal *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->MoveQueueUpThink();
	}
}
...
void CE_AI_ActBusyQueueGoal::QueueThink_CBE()
{
	CE_AI_ActBusyQueueGoal *cent = (CE_AI_ActBusyQueueGoal *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
