...
AISquadEnemyInfo_t *CAI_Squad::FindEnemyInfo( CEntity *pEnemy )
{
	...
	CBaseEntity *cbase = (pEnemy)?pEnemy->BaseEntity():NULL;
	if ( m_pLastFoundEnemyInfo && m_pLastFoundEnemyInfo->hEnemy == cbase)
		return m_pLastFoundEnemyInfo;

	for ( i = 0; i < m_EnemyInfos.Count(); i++ )
	{
		if ( m_EnemyInfos[i].hEnemy == cbase )
		{
...
int CAI_Squad::GetSquadIndex( CAI_NPC *pAI )
{
	for ( int i = 0; i < m_SquadMembers.Count(); i++ )
	{
