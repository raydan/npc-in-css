...
{
	if ( GetOuter()->ShouldAcceptGoal( this, (pGoal)?pGoal->BaseEntity():NULL ) )
	{
...
{
	GetOuter()->OnClearGoal( this, (pGoal)?pGoal->BaseEntity():NULL );
...
void CAI_FollowBehavior::GetFollowTargetViewLoc( Vector *pResult )
{
	...
	CEntity *temp = m_hFollowTarget;
	if ( temp->MyCombatCharacterPointer())
	{
		trace_t tr;
		Vector vecStart, vecDir;

		vecStart = temp->EyePosition();

		CPlayer *pPlayer = dynamic_cast<CPlayer *>(temp);
