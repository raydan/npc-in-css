...
extern ConVar *ai_enable_fear_behavior;
extern ConVar *ai_fear_player_dist;
...
{
	m_flTimeToSafety = 0.0f;
	m_flTimePlayerLastVisible = 0.0f;
	m_flDeferUntil = 0.0f;
	ReleaseAllHints();
...
void CAI_FearBehavior::StartTask( const Task_t *pTask )
{
	...
	case TASK_FEAR_IN_SAFE_PLACE:
		...
		m_hSafePlaceHint->Lock( GetOuter()->BaseEntity() );
	...
	case TASK_FEAR_GET_PATH_TO_SAFETY_HINT:
		{
			...
			case 0:// Find the hint node
				{
					ReleaseAllHints();
					CE_AI_Hint *pHint = FindFearWithdrawalDest();
					...
					{
						m_hMovingToHint.Set( pHint->BaseEntity() );
...
bool CAI_FearBehavior::EnemyDislikesMe()
{
	...
	Disposition_t disposition = pEnemy->MyNPCPointer()->IRelationType(GetOuter()->BaseEntity());
...
bool CAI_FearBehavior::CanSelectSchedule()
{
	...
	if( GetOuter()->IRelationType(pEnemy->BaseEntity()) != D_FR )
		return false;
...
void CAI_FearBehavior::GatherConditions()
{
	...
	if( GetEnemy() )
	{
		...
		else if( flEnemyDistSqr < FEAR_ENEMY_TOLERANCE_CLOSE_DIST_SQR && GetEnemy()->CB_GetEnemy() == GetOuter()->BaseEntity() )	
		{
	...
	if( GetOuter()->FVisible_Entity((pPlayer)?pPlayer->BaseEntity():NULL) )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CE_AI_Hint *CAI_FearBehavior::FindFearWithdrawalDest()
{
	CE_AI_Hint *pHint;
	CHintCriteria hintCriteria;
	CAI_NPC *pOuter = GetOuter();
	...
	hintCriteria.AddIncludePosition( pos, ( ai_fear_player_dist->GetFloat() ) );
