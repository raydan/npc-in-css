	...
	float flDistToBlocker, const CEntity *pTargetEnt, Vector *pApex )
{
	...
	if ( navType != NAV_FLY && flTotalDist > MAX_TRIAGULATION_DIST)
	{
		...
		if ( !GetOuter()->GetMoveProbe()->MoveLimit(navType, vecEnd, vecEndIn, MASK_NPCSOLID, (pTargetEnt)?pTargetEnt->BaseEntity():NULL) )
		{
			return false;
		}
	...
	const Vector &vecApex, const Vector &vecEnd, const CEntity *pTargetEnt, AIMoveTrace_t *pStartTrace )
{
	...
	if ( pMoveProbe->MoveLimit(navType, vecStart, vecApex, MASK_NPCSOLID, (pTargetEnt)?pTargetEnt->BaseEntity():NULL, pStartTrace ) )
	{
		// Ok, we got from the start to the triangle apex, now try
		// the triangle apex to the end
		if ( pMoveProbe->MoveLimit(navType, vecApex, vecEnd, MASK_NPCSOLID, (pTargetEnt)?pTargetEnt->BaseEntity():NULL, &endTrace ) )
		{
			bPathClear = true;
		}
	}
...
//-----------------------------------------------------------------------------
// Purpose: Find a short enginerandom path of at least pathLength distance.  If
//			vDirection is given enginerandom path will expand in the given direction,
...
{
	Vector vApex;
	if (!Triangulate(navType, vStart, vEnd, flDistToBlocker, CEntity::Instance(pTarget), &vApex ))
		return NULL;
