...
CE_LINK_ENTITY_TO_CLASS(assault_rallypoint, CE_CRallyPoint);
...
CE_LINK_ENTITY_TO_CLASS(assault_assaultpoint, CE_CAssaultPoint);
...
CE_CAssaultPoint *CAI_AssaultBehavior::FindAssaultPoint( string_t iszAssaultPointName )
{
	...
	// NONE left. Just return a enginerandom assault point, knowing that it's blocked. This is the old behavior, anyway.
	if( pClearAssaultPoints.Count() < 1 )
...
CE_CRallyPoint *CAI_AssaultBehavior::FindBestRallyPointInRadius( const Vector &vecCenter, float flRadius )
{
	...
	{
		CEntity *cent = CEntity::Instance(pEntities[i]);
		CE_CRallyPoint *pRallyEnt = dynamic_cast<CE_CRallyPoint *>(cent);
...
void CAI_AssaultBehavior::SetParameters( string_t rallypointname, AssaultCue_t assaultcue, int rallySelectMethod )
{
	...
	case RALLY_POINT_SELECT_RANDOM:
		{
			// Gather all available points into a utilvector, then pick one at enginerandom.
