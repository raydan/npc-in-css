...
public:
	...
	virtual bool DispatchKeyValue( const char *szKeyName, int iValue );
	virtual bool DispatchKeyValue( const char *szKeyName, float fValue );
	virtual bool DispatchKeyValue( const char *szKeyName, const Vector &vecValue );
...
public: // CBaseEntity non virtual helpers
	...
	CEntity *FirstMoveChild();
	CEntity *NextMovePeer();
	...
	const Vector&			WorldAlignCenter( ) const;
...
public: // custom
	...
	void		DispatchTraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump );
	...
	IPhysicsObject *GetPhysicsObjectNum(int num);
...
public: // All the internal hook implementations for the above virtuals
	...
	DECLARE_DEFAULTHEADER(TraceAttack, void, (const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump));
	...
	DECLARE_DEFAULTHEADER(ResolveFlyCollisionCustom, void, ( trace_t &trace, Vector &vecVelocity ));
	DECLARE_DEFAULTHEADER(ShouldTransmit, int,( const CCheckTransmitInfo *pInfo ));
	...
	DECLARE_DEFAULTHEADER(ComputeWorldSpaceSurroundingBox, void, (Vector *pWorldMins, Vector *pWorldMaxs));
	...
	DECLARE_DEFAULTHEADER(GetDamage, float, ());
	DECLARE_DEFAULTHEADER(SetDamage, void, (float flDamage));
	...
	DECLARE_DEFAULTHEADER(Event_KilledOther, void, ( CBaseEntity *pVictim, const CTakeDamageInfo &info ));
...
protected: //Sendprops
	...
	DECLARE_SENDPROP(bool, m_bSimulatedEveryTick);
...
public:
	...
	DECLARE_SENDPROP(unsigned char, m_triggerBloat);
	DECLARE_SENDPROP(float, m_flSimulationTime);
	DECLARE_DATAMAP(int, m_iMaxHealth);
	DECLARE_SENDPROP(int, m_ubInterpolationFrame);
...
public:
	...
	DECLARE_DATAMAP(string_t, m_iParent);
	...
	string_t szCashedParentName;
	Vector m_vecOriginalPos;
	QAngle m_vecOriginalAngle;
...
inline const Vector& CEntity::WorldAlignCenter( ) const
{
	return CollisionProp_Actual()->OBBCenter();
}
...
inline bool CEntity::IsSimulatedEveryTick() const
{
...
inline void CEntity::SetSimulatedEveryTick( bool sim )
{
