	...
	case TASK_SCANNER_SET_FLY_DIVE:
		{
			...
			{
				// Pick a enginerandom forward and down direction.
				Vector forward;
...
int CNPC_BaseScanner::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	int value = ( *(m_iMaxHealth) - ( m_iMaxHealth / 4 ) );
	if ( m_iHealth < value )
	{
		StartSmokeTrail();
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_BaseScanner::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
...
void CNPC_BaseScanner::TakeDamageFromPhysicsImpact( int index, gamevcollisionevent_t *pEvent )
{
	...
	int damageType = 0;
	float damage = g_helpfunc.CalculateDefaultPhysicsDamage( index, pEvent, flDamageScale, true, damageType );
	if ( damage == 0 )
		return;
...
void CNPC_BaseScanner::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	CPlayer *pPlayer = (CPlayer *)CEntity::Instance(HasPhysicsAttacker( SCANNER_SMASH_TIME ));
	if( pPlayer )
	{
...
bool CNPC_BaseScanner::OverrideMove( float flInterval )
{
	...
	else
	{
		...
		{
			if ( OverridePathMove( (pMoveTarget)?pMoveTarget->BaseEntity():NULL, flInterval ) )
			{
				BlendPhyscannonLaunchSpeed();
				return true;
			}
...
void CNPC_BaseScanner::DiveBombSoundThink_CBE()
{
	CNPC_BaseScanner *cent = (CNPC_BaseScanner *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->DiveBombSoundThink();
	}
}
	...
	if ( pPlayer )
	{
		...
		if ( DotProduct( vecDelta, vecVelocity ) > 0.5f )
		{
			...
			if ( flDist < 200.0f )
			{
				...
				SetContextThink( &CNPC_BaseScanner::DiveBombSoundThink_CBE, gpGlobals->curtime + 0.5f, s_pDiveBombSoundThinkContext );
				return;
			}
		}
	}

	SetContextThink( &CNPC_BaseScanner::DiveBombSoundThink_CBE, gpGlobals->curtime + 2.0f * TICK_INTERVAL, s_pDiveBombSoundThinkContext );
}
...
void CNPC_BaseScanner::IdleSound(void)
{
	...
	{
		// dvs: the combat sounds should be related to what is happening, rather than enginerandom
