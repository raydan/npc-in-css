...
void CNPC_AntlionGuard::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ANTLIONGUARD_VOICE_BARK )
	{
		...
		if(m_pGrowlHighSound)
		{
			duration = g_SoundController->SoundPlayEnvelope( m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardBark1, ARRAYSIZE(envAntlionGuardBark1) );
		}
	...
	if ( pEvent->event == AE_ANTLIONGUARD_VOICE_PAIN )
	{
		...
		float duration = 0.0f;
		if(m_pConfusedSound) {
			duration = g_SoundController->SoundPlayEnvelope( m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardPain1, ARRAYSIZE(envAntlionGuardPain1) );
		}
	...
	{	
		...
		float duration = 0.0f;
		if(m_pGrowlHighSound) {
			duration = g_SoundController->SoundPlayEnvelope( m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardSqueeze, ARRAYSIZE(envAntlionGuardSqueeze) );
		}
	...
	{	
		...
		float duration = 0.0f;
		if(m_pConfusedSound) {
			duration = g_SoundController->SoundPlayEnvelope( m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionGuardGrunt, ARRAYSIZE(envAntlionGuardGrunt) );
		}
...
int CNPC_AntlionGuard::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( IsHeavyDamage( dInfo ) )
	{
		...
		if ( pInflictor&& UTIL_IsCombineBall( pInflictor ) )
		{
			dInfo.SetDamage( 50 );
		}
...
void CNPC_AntlionGuard::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
...
bool CNPC_AntlionGuard::IsHeavyDamage( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() & DMG_CRUSH )
	{
		...
		if ( ( pPhysObject != NULL ) && ( pPhysObject->GetGameFlags() & FVPHYSICS_WAS_THROWN ) )
		{
			...
			if ( UTIL_IsAR2CombineBall( cent_Inflictor) )
				return true;
