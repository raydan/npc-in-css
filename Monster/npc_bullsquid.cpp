...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_bullsquid, cycler, CNPC_Bullsquid );
...
void CNPC_Bullsquid::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
		case BSQUID_AE_SPIT:
		{
			...
			vecSpitDir = vTarget - vSpitPos;
			vecSpitDir.NormalizeInPlace();
		...
		{
		// SOUND HERE!
			CEntity *pHurt = CEntity::Instance(CheckTraceHullAttack_Float( 70, Vector(-16,-16,-16), Vector(16,16,16), sk_bullsquid_dmg_bite.GetFloat(), DMG_SLASH ));
			if ( pHurt )
			{
				Vector forward, up;
...
*/
		...
		case BSQUID_AE_ROAR:
		{
			CEntity *pHurt = CEntity::Instance(CheckTraceHullAttack_Float( 70, Vector(-16,-16,-16), Vector(16,16,16), sk_bullsquid_dmg_whip.GetFloat(), DMG_SLASH ));
			if(pHurt)
			{
				pHurt->ViewPunch( QAngle(20,0,-20) );
				...
				Vector right, up;
				AngleVectors( GetAbsAngles(), NULL, &right, &up );
				pHurt->ApplyAbsVelocityImpulse( right*200 + up *100 );
			}
			break;
		}
		...
		case BSQUID_AE_THROW:
			{
				...
				CEntity *pHurt = CEntity::Instance(CheckTraceHullAttack_Float( 70, Vector(-16,-16,-16), Vector(16,16,16), 0, 0 ));
...
bool CNPC_Bullsquid::FValidateHintType( CBaseEntity *pHint )
{
	CE_AI_Hint *hint = (CE_AI_Hint *)CEntity::Instance(pHint);
	if ( hint->HintType() == HINT_HL1_WORLD_HUMAN_BLOOD )
		 return true;
...
{
	if ( gpGlobals->curtime - m_flLastHurtTime < 5 && FClassnameIs( CEntity::Instance(pTarget), "npc_headcrab" ) )
	{
...
int CNPC_Bullsquid::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if ( !FClassnameIs( CEntity::Instance(inputInfo.GetAttacker()), "npc_headcrab" ) )
	{
	...
	case NPC_STATE_ALERT:
		{
			...
			if ( HasCondition( COND_SQUID_SMELL_FOOD ) )
			{
				...
				if ( pSound && (!FInViewCone_Vector( pSound->GetSoundOrigin() ) || !FVisible_Vector( pSound->GetSoundOrigin() )) )
				{
...
// dead enemy
			...
			{
				if ( m_fCanThreatDisplay && IRelationType( GetEnemy_CBase() ) == D_HT && FClassnameIs( GetEnemy(), "npc_headcrab" ) )
				{
...
//=========================================================
bool CNPC_Bullsquid::FInViewCone_Vector( Vector pOrigin )
{
