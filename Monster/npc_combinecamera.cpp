...
// Purpose: 
//-----------------------------------------------------------------------------
class CNPC_CombineCamera : public CE_Cycler_Fix
{
public:
	CE_DECLARE_CLASS(CNPC_CombineCamera, CE_Cycler_Fix);
...
protected:
	...
	CEntity *GetTarget();
	bool UpdateFacing();
	void TrackTarget(CEntity *pTarget);
...
protected:
	...
	CFakeHandle	m_hEnemyTarget;			// Entity we acquired as an enemy.	
...
LINK_ENTITY_TO_CUSTOM_CLASS(npc_combine_camera, cycler, CNPC_CombineCamera);
...
void CNPC_CombineCamera::Spawn()
{
	...
	m_pEyeFlash = CE_CSprite::SpriteCreate(COMBINE_CAMERA_FLASH_SPRITE, GetLocalOrigin(), FALSE);
	m_pEyeFlash->SetTransparency(kRenderGlow, 255, 255, 255, 0, kRenderFxNoDissipation);
	m_pEyeFlash->SetAttachment(BaseEntity(), 2);
	...
	m_pEyeGlow = CE_CSprite::SpriteCreate(COMBINE_CAMERA_GLOW_SPRITE, GetLocalOrigin(), false);
...
// Purpose: 
//-----------------------------------------------------------------------------
CEntity *CNPC_CombineCamera::GetTarget()
{
...
//-----------------------------------------------------------------------------
bool CNPC_CombineCamera::FVisible_Entity(CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker)
{
	CBaseEntity	*pHitEntity = NULL;
	if ( BaseClass::FVisible_Entity( pEntity, traceMask, &pHitEntity ) )
		return true;

	CEntity *cent = CEntity::Instance(pHitEntity);
	// If we hit something that's okay to hit anyway, still fire
	if ( cent && cent->MyCombatCharacterPointer() )
	{
...
bool CNPC_CombineCamera::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	Vector vecDelta = cent->GetAbsOrigin() - GetAbsOrigin();
	float flDist = vecDelta.Length();
	if ( (flDist > m_nOuterRadius) || !FInViewCone_Entity(pEnemy) )
		return false;
...
//-----------------------------------------------------------------------------
CEntity *CNPC_CombineCamera::MaintainEnemy()
{
	...
	if (pEnemy)
	{
		...
		{
			if (FInViewCone_Entity(pEnemy->BaseEntity()))
			{
				...
				bool bVisible = FVisible_Entity(pEnemy->BaseEntity());
...
void CNPC_CombineCamera::ActiveThink()
{
	...
	CEntity *pTarget = MaintainEnemy();
	if ( !pTarget )
	{
		// Nobody suspicious. Go back to being idle.
		m_hEnemyTarget.Set(NULL);
	...
	if ( pTarget != m_hEnemyTarget )
	{
		...
		float flDist = vecDelta.Length();
		CBaseEntity *cbase_pTarget = pTarget->BaseEntity();
		if ( (flDist < m_nInnerRadius) && FInViewCone_Entity(cbase_pTarget) )
		{
			m_OnFoundEnemy->Set(cbase_pTarget, cbase_pTarget, BaseEntity());
			...
			if ( pTarget->IsPlayer() )
			{
				...
				m_OnFoundPlayer->Set(cbase_pTarget, cbase_pTarget, BaseEntity());
				m_hEnemyTarget.Set(cbase_pTarget);
			}
			else
			{
				...
				AddEntityRelationship( cbase_pTarget, D_NU, 99 );
			}
		}
		else
		{
...
// Input  : pTarget - 
//-----------------------------------------------------------------------------
void CNPC_CombineCamera::TrackTarget( CEntity *pTarget )
{
	if (!pTarget)
		return;
...
void CNPC_CombineCamera::SetHeight(float height)
{
	...
	UTIL_SetSize(BaseEntity(), mins, maxs);
}
