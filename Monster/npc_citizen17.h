...
{
public:
	CE_DECLARE_CLASS( CNPC_Citizen, CNPC_PlayerCompanion );
	...
	bool			ShouldAcceptGoal( CAI_BehaviorBase *pBehavior, CBaseEntity *pGoal );
	void			OnClearGoal( CAI_BehaviorBase *pBehavior, CBaseEntity *pGoal );
	...
	//---------------------------------
	bool 			IsCommandable();
	bool			IsPlayerAlly( CBaseEntity *pPlayer = NULL );
	...
	bool 			TargetOrder( CBaseEntity *pTarget, CBaseEntity **Allies, int numAllies );
	void 			MoveOrder( const Vector &vecDest, CBaseEntity **Allies, int numAllies );
	void			OnMoveOrder();
	void 			CommanderUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value );
	bool			ShouldSpeakRadio( CEntity *pListener );
	...
	CBaseEntity *	GetSquadCommandRepresentative();
	...
	bool 			CanHeal();
	bool 			ShouldHealTarget( CEntity *pTarget, bool bActiveUse = false );
#if HL2_EPISODIC
...
private:
	...
	CEFakeHandle<CE_AI_FollowGoal>	m_hSavedFollowGoalEnt;
...
class CCitizenResponseSystem : public CE_InfoTarget_Fix
{
public:
	CE_DECLARE_CLASS( CCitizenResponseSystem, CE_InfoTarget_Fix );
...
class CSquadInsignia : public CSoda_Fix
{
public:
	CE_DECLARE_CLASS( CSquadInsignia, CSoda_Fix );
	void Spawn();
};
