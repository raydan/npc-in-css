...
CNPC_Barnacle::CNPC_Barnacle(void)
{
	...
	m_flVictimHeight = 0.0f;
	m_iGrabbedBoneIndex = 0;
	m_bPlayedPullSound = false;

	m_vecTipDrawOffset.Init();
	m_nSpitAttachment = 0;
	m_vecBloodPos.Init();
	m_flLocalTimer = 0.0f;
	m_vLastEnemyPos.Init();
	m_flLastPull = 0.0f;
	
	memset(&m_pRagdollBones, 0, sizeof(m_pRagdollBones));
}
...
void CNPC_Barnacle::HandleAnimEvent( animevent_t *pEvent )
{
	...
	{
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 50, 1, "models/gibs/hgibs_rib.mdl");
		return;
	}
...
void CNPC_Barnacle::DropTongue( void )
{
	...
	CEntity *cent = CBarnacleTongueTip::CreateTongueRoot( m_vecRoot, QAngle(90,0,0) );
	m_hTongueRoot.Set(cent->BaseEntity());

	cent = CBarnacleTongueTip::CreateTongueTip( this, m_hTongueRoot, m_vecTip, QAngle(0,0,0) );
	m_hTongueTip.Set(cent->BaseEntity());
...
//-----------------------------------------------------------------------------
void CNPC_Barnacle::PlayerHasIlluminatedNPC( CBaseEntity *pPlayer, float flDot )
{
	// Create a sound to scare friendly allies away from the base on the barnacle
	if( IsAlive() )
	{
 		g_helpfunc.CSoundEnt_InsertSound( SOUND_MOVE_AWAY | SOUND_CONTEXT_ALLIES_ONLY, m_vecTip, 60, FLASHLIGHT_NPC_CHECK_INTERVAL );
	}
}
...
void CNPC_Barnacle::BarnacleThink ( void )
{
	...
	else
	{
		...
		CollisionProp_Actual()->WorldSpaceSurroundingBounds( &vecSurroundMins, &vecSurroundMaxs );
		...
		if ( m_flAltitude >= flLength )
		{
			...
			if ( ( pTouchEnt != NULL ) && ( pTouchEnt != m_hLastSpitEnemy ) && m_hTongueTip != NULL )
			{
...
void CNPC_Barnacle::LiftNPC( float flBiteZOffset )
{
	...
	if ( GetAbsOrigin().z - m_vecTip.z < flBiteZOffset )
	{
		...
		const Vector &vecSize = GetEnemy()->CollisionProp_Actual()->OBBSize();
		if ( vecSize.z < 40 )
		{
			// Start the bite animation. The anim event in it will finish the job.
...
void CNPC_Barnacle::LiftPrey( void )
{
	...
	if ( ( bEnemyIsNPC && !pVictim->IsAlive() ) || (tr.fraction < 1.0 && cent != pVictim && cent != m_hRagdoll) )
	{
		if ( !pVictim->IsPlayer() )
		{
			// ignore the object so we don't get into a loop of trying to pick it up.
			m_hLastSpitEnemy.Set(pVictim->BaseEntity());
		}
		LostPrey( false );
		return;
	}
...
CE_CRagdollProp *CNPC_Barnacle::AttachRagdollToTongue( CAnimating *pAnimating )
{
	...
	CE_CRagdollProp *pRagdoll= (CE_CRagdollProp *)g_helpfunc.CreateServerRagdollAttached( pAnimating->BaseEntity(), vec3_origin, -1, COLLISION_GROUP_NONE, pTonguePhysObject, m_hTongueTip->BaseEntity(), 0, vecBonePos, m_iGrabbedBoneIndex, vec3_origin );
	if ( pRagdoll )
	{
#if HL2_EPISODIC
...
You can use this stanza to try to counterplace the constraint on the player's head so he gets hauled sideways to the right place on the barnacle, but it is better to just move the tongue before attachment.
	...
	CE_CRagdollProp *cent = AttachRagdollToTongue( pAnimating );
	m_hRagdoll.Set(cent->BaseEntity());
	cent->SetDamageEntity( pAnimating );
...
void CNPC_Barnacle::BitePrey( void )
{
	...
	{
		if ( !pVictim->IsPlayer() || pVictim->GetHealth() <= 0 || !pVictim->IsAlive())
		{
			LostPrey( false );
		}
		return;
...
void CNPC_Barnacle::SpawnDeathGibs( void )
{
	...
	// Drop a enginerandom number of gibs
	for ( size_t i=0; i < ARRAYSIZE(m_szGibNames); i++ )
	{
		if ( enginerandom->RandomInt( 0, 1 ) )
		{
			CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 32, 1, m_szGibNames[i] );
			bDroppedAny = true;
		}
	}
	...
	{
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 32, 1, m_szGibNames[0] );
	}
}
...
CEntity *CNPC_Barnacle::TongueTouchEnt ( float *pflLength )
{
	...
	CEntity *cent = m_hLastSpitEnemy;
	// trace once to hit architecture and see if the tongue needs to change position.
	CBarnacleTongueFilter tongueFilter( (cent)?cent->BaseEntity():NULL, BaseEntity(), COLLISION_GROUP_NONE );
...
void CBarnacleTongueTip::UpdateOnRemove( )
{
	...
	if ( m_hMoveRope )
	{
		UTIL_Remove(m_hMoveRope);
		m_hMoveRope.Set(NULL);
	}
	if ( m_hKeyFrameRope )
	{
		UTIL_Remove(m_hKeyFrameRope);
		m_hKeyFrameRope.Set(NULL);
	}
...
CBarnacleTongueTip *CBarnacleTongueTip::CreateTongueTip( CNPC_Barnacle *pBarnacle, CAnimating *pTongueRoot, const Vector &vecOrigin, const QAngle &vecAngles )
{
	...
	pTip->m_hBarnacle.Set(pBarnacle->BaseEntity());
	...
	int index = pTip->entindex();
	char rope_name[56];
	Q_snprintf( rope_name, sizeof( rope_name ), "the_rope_%d",index);

	CEntity *keyframe_rope = CreateEntityByName("keyframe_rope");
	if(keyframe_rope)
	{
		keyframe_rope->SetName(rope_name);
		keyframe_rope->DispatchKeyValue("RopeMaterial","shadertest/translucentspecularsphericalenvmapwithmaskedbase");
		keyframe_rope->SetAbsOrigin(pTip->GetAbsOrigin());
		keyframe_rope->SetParent(pTip->BaseEntity());
		keyframe_rope->Spawn();
		keyframe_rope->Activate();
		pTip->m_hKeyFrameRope.Set(keyframe_rope->BaseEntity());
	}

	CEntity *move_rope = CreateEntityByName("move_rope");	
	if(move_rope)
	{
		move_rope->DispatchKeyValue("RopeMaterial","shadertest/translucentspecularsphericalenvmapwithmaskedbase");
		move_rope->DispatchKeyValue("NextKey",rope_name);
		move_rope->SetAbsOrigin(pBarnacle->GetAbsOrigin());
		move_rope->SetParent(pBarnacle->BaseEntity());
		move_rope->Spawn();
		move_rope->Activate();
		pTip->m_hMoveRope.Set(move_rope->BaseEntity());
	}
