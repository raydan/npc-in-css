...
void CNPC_PlayerCompanion::OnNewLookTarget()
{
	...
	if( ai_new_aiming.GetBool() )
	{
		if( GetLooktarget() )
		{
			...
			CE_AI_Hint *pHint = dynamic_cast<CE_AI_Hint*>( GetLooktarget() );
...
{
	return BaseClass::FindCoverPos_Entity( pEntity, pResult );
...
{
	return BaseClass::FindCoverPosInRadius( pEntity, goalPos, coverRadius, pResult );
...
{
	return BaseClass::FindCoverPos_Sound( pSound, pResult );
