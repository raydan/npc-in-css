...
void CBaseHelicopter::RotorWashThink_CBE( void )
{
	CBaseHelicopter *cent = (CBaseHelicopter *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->RotorWashThink();
	}
...
void CBaseHelicopter::RotorWashThink( void )
{
	if ( m_lifeState == LIFE_ALIVE || m_lifeState == LIFE_DYING )
	{
		...
		SetContextThink( &CBaseHelicopter::RotorWashThink_CBE, gpGlobals->curtime + HELICOPTER_ROTORWASH_THINK_INTERVAL, s_pRotorWashThinkContext );
	}
	else
	{
...
void CBaseHelicopter::UpdatePlayerDopplerShift( )
{
	...
	{
		CEntity *pPlayer = NULL;

		// UNDONE: this needs to send different sounds to every player for multiplayer.	
		// FIXME: this isn't the correct way to find a player!!!
		pPlayer = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, "!player" );
		if (pPlayer)
		{
			Vector dir;
...
void CBaseHelicopter::InputKill( inputdata_t &inputdata )
{
	...
	m_bSuppressSound = true;
	SetContextThink( &CBaseHelicopter::DelayedKillThink_CBE, gpGlobals->curtime + 3.0f, s_pDelayedKillThinkContext );
}
...
void CBaseHelicopter::DelayedKillThink_CBE( )
{
	CBaseHelicopter *cent = (CBaseHelicopter *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->DelayedKillThink();
	}
}
...
void CBaseHelicopter::Startup( void )
{
	...
	SetContextThink( &CBaseHelicopter::RotorWashThink_CBE, gpGlobals->curtime, s_pRotorWashThinkContext );
}
