...
LINK_ENTITY_TO_CUSTOM_CLASS( monster_alien_grunt, cycler, CNPC_Stalker );
...
void CNPC_Stalker::DrawAttackBeam(void)
{
	...
	if (pBCC && pBCC->Classify() != CLASS_STALKER)
	{
		if (gpGlobals->curtime > m_fNextDamageTime)
		{
			...
			pBCC->DispatchTraceAttack( info, m_vLaserDir, &tr, NULL );
			ApplyMultiDamage();
...
void CNPC_Stalker::AddZigZagToPath(void) 
{
	...
	// then add a enginerandom zig/zag to the path
	if (waypointDir.LengthSqr() > ZIG_ZAG_SIZE)
	{
		// Pick a enginerandom distance for the zigzag (less that sqrt(ZIG_ZAG_SIZE)
		...
		// Pick a enginerandom direction (left/right) for the zigzag
