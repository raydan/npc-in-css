	...
	else
	{
		...
		if ( pPlayer && mat_dxlevel->GetInt() > 0 )
		{
			...
			if ( maxRagdolls > 0 )
			{
				...
				while ( ( pRagdoll = g_helpfunc.NextEntByClass( pRagdoll ) ) != NULL )
				{
...
void CNPC_Strider::FireCannon() 
{
	...
	SetContextThink( &CNPC_Strider::CannonHitThink_CBE, gpGlobals->curtime + 0.2f, "CANNON_HIT" );
}

void CNPC_Strider::CannonHitThink_CBE()
{
	CNPC_Strider *cent = (CNPC_Strider *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->CannonHitThink();
	}
...
void CNPC_Strider::StompHit( int followerBoneIndex )
{
	...
	CEntity *pRagdoll = g_helpfunc.CreateServerRagdollAttached( pNPC->BaseEntity(), vec3_origin, -1, COLLISION_GROUP_DEBRIS, pLegPhys, BaseEntity(), bone->boneIndex, vecStabPos, -1, localHit );
	if ( pRagdoll )
	{
