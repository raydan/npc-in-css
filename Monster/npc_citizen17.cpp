...
class CCommandPoint : public CE_InfoTarget_Fix
{
public:
	CE_DECLARE_CLASS( CCommandPoint, CE_InfoTarget_Fix );
	...
	void InputOutsideTransition( inputdata_t &inputdata )
	{
		...
		{
			AISquadIter_t iter;
			for ( CAI_NPC *pAllyNpc = (CAI_NPC *)CEntity::Instance(pPlayerAISquad->GetFirstMember(&iter)); pAllyNpc; pAllyNpc = (CAI_NPC *)CEntity::Instance(pPlayerAISquad->GetNextMember(&iter)) )
			{
				if ( pAllyNpc->GetCommandGoal() != vec3_invalid )
				{
					...
					CEntity *pPlayer = UTIL_GetNearestPlayer(pAllyNpc->GetAbsOrigin());
					CBaseEntity *cbase = pAllyNpc->BaseEntity();
					pAllyNpc->TargetOrder((pPlayer)?pPlayer->BaseEntity():NULL, &cbase, 1 );
					pAllyNpc = (CAI_NPC *)CEntity::Instance(cbase);
					if ( !bHadGag )
...
LINK_ENTITY_TO_CUSTOM_CLASS( info_target_command_point, info_target, CCommandPoint );
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_citizen, cycler, CNPC_Citizen );
...
void CNPC_Citizen::Precache()
{
	...
	g_helpfunc.PrecacheInstancedScene( "scenes/Expressions/CitizenIdle.vcd" );
	g_helpfunc.PrecacheInstancedScene( "scenes/Expressions/CitizenAlert_loop.vcd" );
	g_helpfunc.PrecacheInstancedScene( "scenes/Expressions/CitizenCombat_loop.vcd" );

	for ( int i = 0; i < STATES_WITH_EXPRESSIONS; i++ )
	{
		for ( size_t j = 0; j < ARRAYSIZE(ScaredExpressions[i].szExpressions); j++ )
		{
			g_helpfunc.PrecacheInstancedScene( ScaredExpressions[i].szExpressions[j] );
		}
		for ( size_t j = 0; j < ARRAYSIZE(NormalExpressions[i].szExpressions); j++ )
		{
			g_helpfunc.PrecacheInstancedScene( NormalExpressions[i].szExpressions[j] );
		}
		for ( size_t j = 0; j < ARRAYSIZE(AngryExpressions[i].szExpressions); j++ )
		{
			g_helpfunc.PrecacheInstancedScene( AngryExpressions[i].szExpressions[j] );
		}
	}
...
{
	if ( !g_helpfunc.FindEntityByClassname( (CBaseEntity *)NULL, COMMAND_POINT_CLASSNAME ) )
	{
	...
	if ( IsInPlayerSquad() )
	{
		if ( GetSquad()->NumMembers() > MAX_PLAYER_SQUAD )
...
void CNPC_Citizen::SelectModel()
{
	...
	if( HasSpawnFlags( SF_CITIZEN_RANDOM_HEAD | SF_CITIZEN_RANDOM_HEAD_MALE | SF_CITIZEN_RANDOM_HEAD_FEMALE ) || GetModelName() == NULL_STRING )
	{
		...
		else
		{
			...
			{
				if ( /*!gender || g_ppszRandomHeads[i][0] == gender */ true)
				{
			...
			m_iHead = candidates[enginerandom->RandomInt( 0, iLimit - 1 )].iHead;
...
void CNPC_Citizen::FixupMattWeapon()
{
	...
	if ( pWeapon && pWeapon->ClassMatches( "weapon_crowbar" ) && NameMatches( "matt" ) )
	{
		...
		UTIL_Remove( pWeapon );
		//CE_TODO
		//pWeapon = (CCombatWeapon *)CREATE_UNSAVED_ENTITY( CMattsPipe, "weapon_crowbar" );
		//pWeapon->SetName( "matt_weapon" );
		//DispatchSpawn( pWeapon->BaseEntity() );
...
#endif
		pWeapon->Activate();
		Weapon_Equip( pWeapon->BaseEntity() );
	}
}
...
void CNPC_Citizen::GatherConditions()
{
	...
	if( IsInPlayerSquad() && hl2_episodic->GetBool() )
	{
		...
		if( IRelationType((pPlayer)?pPlayer->BaseEntity():NULL) == D_NU )
		{
	...
	if ( !SpokeConcept( TLK_JOINPLAYER ) && IsRunningScriptedSceneWithSpeech( this, true ) )
	{
		...
		for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			...
			if ( pNpc != this && pNpc->GetClassname() == GetClassname() && pNpc->GetAbsOrigin().DistToSqr( GetAbsOrigin() ) < Square( 15*12 ) && FVisible_Entity( pNpc->BaseEntity() ) )
			{
	...
	if( CanHeal() )
	{
		...
		if( pPlayer->IsAlive() && flPlayerDamage > 0 && (flDistSqr <= flStareDist * flStareDist) && pPlayer->FInViewCone_Entity( BaseEntity() ) && pPlayer->FVisible_Entity( BaseEntity() ) )
		{
...
void CNPC_Citizen::PredictPlayerPush()
{
	...
	if ( !bHadPlayerPush && HasCondition( COND_PLAYER_PUSHING ) && 
		 pPlayer->FInViewCone_Entity( BaseEntity() ) && CanHeal() )
	{
		if ( ShouldHealTarget( pPlayer, true ) )
		{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Citizen::FInViewCone_Entity( CBaseEntity *pEntity )
{
#if 0
	if ( IsMortar( pEntity ) )
	{
...
int CNPC_Citizen::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	...
	case SCHED_MOVE_TO_WEAPON_RANGE:
		...
		{
			if ( GetActiveWeapon() && ( GetActiveWeapon()->Weapon_CapabilitiesGet() & bits_CAP_WEAPON_RANGE_ATTACK1 ) && enginerandom->RandomInt( 0, 1 ) && HasCondition(COND_SEE_ENEMY) && !HasCondition ( COND_NO_PRIMARY_AMMO ) )
...
int CNPC_Citizen::SelectScheduleHeal()
{
	...
	if ( CanHeal() )
	{
		CEntity *pEntity = PlayerInRange( GetLocalOrigin(), HEAL_MOVE_RANGE );
...
int CNPC_Citizen::SelectScheduleRetrieveItem()
{
	...
	{
		CNPCBaseWeapon *pWeapon = dynamic_cast<CNPCBaseWeapon *>(Weapon_FindUsable( WEAPON_SEARCH_DELTA ));
		if ( pWeapon )
		{
	...
	if( HasCondition(COND_HEALTH_ITEM_AVAILABLE) )
	{
		...
		{
			CEntity *cent = m_FollowBehavior.GetFollowTarget();
			if(cent == NULL)
				return SCHED_NONE;

			CEntity *pBase = FindHealthItem(cent->GetAbsOrigin(), Vector( 120, 120, 120 ) );
			if( pBase )
			{
				SetTarget( pBase->BaseEntity() );
...
int CNPC_Citizen::SelectScheduleManhackCombat()
{
	if ( m_NPCState == NPC_STATE_COMBAT && IsManhackMeleeCombatant() )
	{
		if ( !HasCondition( COND_CAN_MELEE_ATTACK1 ) )
		{
			...
			int iRoll = enginerandom->RandomInt( 1, 4 );
			if ( iRoll == 1 )
...
{
	CPlayer *pLocalPlayer = UTIL_GetNearestVisiblePlayer(this);
	...
	case SCHED_RANGE_ATTACK1:
		...
		if ( !IsMortar( GetEnemy() ) && GetActiveWeapon() && FClassnameIs( GetActiveWeapon(), "weapon_rpg" ) )
		{
			...
			{
				CPlayer *pPlayer = UTIL_GetNearestPlayer(GetEnemy()->GetAbsOrigin());
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Citizen::ShouldAcceptGoal( CAI_BehaviorBase *pBehavior, CBaseEntity *pGoal )
{
	if ( BaseClass::ShouldAcceptGoal( pBehavior, pGoal ) )
	{
		...
		if ( pFollowBehavior )
		{
			if ( IsInPlayerSquad() )
			{
				m_hSavedFollowGoalEnt.Set(pGoal);
				return false;
			}
		}
		return true;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Citizen::OnClearGoal( CAI_BehaviorBase *pBehavior, CBaseEntity *pGoal )
{
	if ( m_hSavedFollowGoalEnt == CEntity::Instance(pGoal) )
		m_hSavedFollowGoalEnt.Set(NULL);
}
...
void CNPC_Citizen::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
		case TASK_WAIT_FOR_MOVEMENT:
		{
			...
			{
				AddFacingTarget_E_F_F_F( GetEnemy_CBase(), 1.0, 0.5 );
			}
...
void CNPC_Citizen::HandleAnimEvent( animevent_t *pEvent )
{
	if ( pEvent->event == AE_CITIZEN_GET_PACKAGE )
	{
		...
		CCombatWeapon *pWeapon = Weapon_Create( "weapon_citizenpackage" );
		if ( pWeapon )
		{
			// If I have a name, make my weapon match it with "_weapon" appended
			if ( GetEntityName_String() != NULL_STRING )
			{
				pWeapon->SetName( UTIL_VarArgs("%s_weapon", GetEntityName()) );
			}
			Weapon_Equip( pWeapon->BaseEntity() );
		}
		return;
...
{
	CEntity *cent = CEntity::Instance(pItem);
	Assert( cent != NULL );
	if( FClassnameIs( cent, "item_healthkit" ) )
	{
	...
	else if( FClassnameIs( cent, "item_healthvial" ) )
	{
...
void CNPC_Citizen::SimpleUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( !SelectPlayerUseSpeech() )
	{
		if ( HasSpawnFlags(SF_CITIZEN_NOT_COMMANDABLE) || IRelationType( pActivator ) == D_NU )
		{
			...
			m_OnDenyCommanderUse->FireOutput( BaseEntity(), BaseEntity() );
		}
	}
	...
	if ( pRPG && GetEnemy() )
	{
		if ( FClassnameIs( GetEnemy(), "npc_combinegunship" ) )
		{
			...
			for ( size_t i = 0; i < ARRAYSIZE(flShotOffsets); i++ )
			{
				...
				// Add some enginerandom height to it
				vecTest.z += RandomFloat( -512, 512 );
				trace_t tr;
				UTIL_TraceLine( shootOrigin, vecTest, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr);

				// If we can see the point, it's a clear shot
				if ( tr.fraction == 1.0 && tr.m_pEnt != GetEnemy_CBase() )
				{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Citizen::OnChangeActiveWeapon( CBaseEntity *pOldWeapon, CBaseEntity *pNewWeapon )
{
	CCombatWeapon *cent_pNewWeapon = (CCombatWeapon *)CEntity::Instance(pNewWeapon);
	if ( cent_pNewWeapon )
	{
		GetShotRegulator()->SetParameters( cent_pNewWeapon->GetMinBurst(), cent_pNewWeapon->GetMaxBurst(), cent_pNewWeapon->GetMinRestTime(), cent_pNewWeapon->GetMaxRestTime() );
	}
...
bool CNPC_Citizen::ShouldLookForBetterWeapon()
{
	if ( BaseClass::ShouldLookForBetterWeapon() )
	{
		...
		if( pWeapon )
		{
			...
			else if( FClassnameIs( pWeapon, WEAPON_RPG_REPLACE_NAME ) )
			{
				// Content to keep this weapon forever
				m_flNextWeaponSearchTime = OTHER_DEFER_SEARCH_TIME;
				bDefer = true;
			}
			else if( FClassnameIs( pWeapon, WEAPON_SHOTGUN_REPLACE_NAME ) )
			{
				...
				if( NumWeaponsInSquad(WEAPON_SHOTGUN_REPLACE_NAME) > 1 )
				{
...
#define CITIZEN_SCORCH_FLOOR	75
	...
	if( IsInSquad() && (info.GetDamageType() & DMG_BLAST) && info.GetInflictor() )
	{
		if( npc_citizen_explosive_resist.GetBool() )
		{
			...
			// single explosion.
			if( GetSquad()->IsSquadInflictor( info.GetInflictor() ) )
			{
			...
			else
			{
				...
				{
					GetSquad()->SetSquadInflictor( info.GetInflictor() );
				}
			}
		}
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Citizen::IsPlayerAlly( CBaseEntity *pPlayer )											
{ 
...
bool CNPC_Citizen::CanJoinPlayerSquad()
{
	...
	CPlayer *pPlayer = UTIL_GetNearestPlayer(GetAbsOrigin());
	if ( IRelationType( (pPlayer)?pPlayer->BaseEntity():NULL ) != D_LI )
		return false;
...
bool CNPC_Citizen::ShouldAutoSummon()
{
	...
	else if ( m_AutoSummonTimer.Expired() )
	{
		...
		{
			bool bCommandPointIsVisible = pPlayer->FVisible_Vector( vCommandGoal + pPlayer->GetViewOffset() );
		...
		if ( bSetFollow && bTestEnemies )
		{
			for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
			{
				...
				else if ( pNpc->GetEnemy() )
				{
					...
					{
						if ( pNpc->CanBeAnEnemyOf( BaseEntity() ) )
						{
							bSetFollow = false;
							break;
						}
...
//-----------------------------------------------------------------------------
bool CNPC_Citizen::TargetOrder( CBaseEntity *pTarget, CBaseEntity **Allies, int numAllies )
{
	CEntity *cent = CEntity::Instance(pTarget);
	if ( cent->IsPlayer() )
	{
		// I'm the target! Toggle follow!
		if( m_FollowBehavior.GetFollowTarget() != cent )
		{
			ClearFollowTarget();
			...
			m_FollowBehavior.SetFollowTarget( cent );
			m_FollowBehavior.SetParameters( AIF_SIMPLE );			
			SpeakCommandResponse( TLK_STARTFOLLOW );

			m_OnFollowOrder.FireOutput( this, this );
		}
		else if ( m_FollowBehavior.GetFollowTarget() == cent )
		{
			// Stop following.
...
//-----------------------------------------------------------------------------
void CNPC_Citizen::MoveOrder( const Vector &vecDest, CBaseEntity **Allies, int numAllies )
{
	...
	CPlayer *pPlayer = (CPlayer *)UTIL_GetNearestPlayer(GetAbsOrigin());
	Assert(pPlayer);
	...
	CAI_NPC *pClosest = NULL;
	float closestDistSq = FLT_MAX;

	for( int i = 0 ; i < numAllies ; i++ )
	{
		CAI_NPC *npc = (CAI_NPC *)CEntity::Instance(Allies[i]);
		if( npc->IsInPlayerSquad() )
		{
			Assert( npc->IsCommandable() );
			float distSq = ( pPlayer->GetAbsOrigin() - npc->GetAbsOrigin() ).LengthSqr();
...
void CNPC_Citizen::CommanderUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( cent_pActivator == pPlayer )
	{
		...
		else if ( GetCurSchedule() && ConditionInterruptsCurSchedule( COND_IDLE_INTERRUPT ) )
		{
			...
			{
				if ( enginerandom->RandomInt( 1, 4 ) < 4 )
				{
					CEntity *pRespondant = FindSpeechTarget( AIST_NPCS );
					if ( pRespondant )
					{
						g_CEventQueue->AddEvent( pRespondant->BaseEntity(), "SpeakIdleResponse", ( GetTimeSpeechComplete() - gpGlobals->curtime ) + .2, BaseEntity(), BaseEntity() );
					}
				}
			}
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Citizen::ShouldSpeakRadio( CEntity *pListener )
{
	if ( !pListener )
		return false;
...
void CNPC_Citizen::AddToPlayerSquad()
{
	...
	CEntity *cent =  m_FollowBehavior.GetFollowGoal();;
	m_hSavedFollowGoalEnt.Set((cent)?cent->BaseEntity():NULL);
...
void CNPC_Citizen::UpdatePlayerSquad()
{
	...
	if ( pPlayerSquad )
	{
		...
		for ( CAI_NPC *pPlayerSquadMember = (CAI_NPC *)CEntity::Instance(pPlayerSquad->GetFirstMember(&iter)); pPlayerSquadMember; pPlayerSquadMember = (CAI_NPC *)CEntity::Instance(pPlayerSquad->GetNextMember(&iter)) )
		{
	...
	if ( pPlayer && ShouldAutosquad() && !(pPlayer->GetFlags() & FL_NOTARGET ) && pPlayer->IsAlive() )
	{
		...
		for ( i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			...
			else
			{
				...
				{
					bool bPlayerVisible = pCitizen->FVisible_Entity( pPlayer->BaseEntity() );
					if ( bPlayerVisible )
					{
				...
				if ( bShouldAdd )
				{
					...
					AI_Waypoint_t *pPathToPlayer = pCitizen->GetPathfinder()->BuildRoute( pCitizen->GetAbsOrigin(), vPlayerPos, pPlayer->BaseEntity(), 5*12, NAV_NONE, true );
			...
			for ( i = 0; i < initialCount; i++ )
			{
				if ( candidates[i].iSquadIndex == -1 )
				{
					for ( int j = 0; j < g_AI_Manager.NumAIs(); j++ )
					{
						...
						if ( !pCitizen->FVisible_Entity( pPlayer->BaseEntity() ) )
							continue;
...
int CNPC_Citizen::PlayerSquadCandidateSortFunc( const SquadCandidate_t *pLeft, const SquadCandidate_t *pRight )
{
	...
	CCombatWeapon *pLeftWeapon = pLeftCitizen->GetActiveWeapon();
	CCombatWeapon *pRightWeapon = pRightCitizen->GetActiveWeapon();
...
void CNPC_Citizen::FixupPlayerSquad()
{
	...
	if ( GetSquad()->NumMembers() > MAX_PLAYER_SQUAD )
	{
		CAI_NPC *pFirstMember = (CAI_NPC *)CEntity::Instance(GetSquad()->GetFirstMember(NULL));
		GetSquad()->RemoveFromSquad( pFirstMember );
	...
	AISquadIter_t iter;
	for ( CAI_NPC *pAllyNpc = (CAI_NPC *)CEntity::Instance(GetSquad()->GetFirstMember(&iter)); pAllyNpc; pAllyNpc = (CAI_NPC *)CEntity::Instance(GetSquad()->GetNextMember(&iter)) )
	{
...
void CNPC_Citizen::UpdateFollowCommandPoint()
{
	if ( IsInPlayerSquad() )
	{
		if ( HaveCommandGoal() )
		{
			CEntity *pFollowTarget = m_FollowBehavior.GetFollowTarget();
			CEntity *pCommandPoint = g_helpfunc.FindEntityByClassname( (CBaseEntity *)NULL, COMMAND_POINT_CLASSNAME );
...
CBaseEntity *CNPC_Citizen::GetSquadCommandRepresentative()
{
	if ( IsInPlayerSquad() )
	{
		static float lastTime;
		static CEFakeHandle<CAI_NPC> hCurrent;
		...
		if ( gpGlobals->curtime - lastTime > 2.0 || !hCurrent || !hCurrent->IsInPlayerSquad() ) // hCurrent will be NULL after level change
		{
			...
			if ( pPlayer )
			{
				...
				if ( candidates.Count() > 0 )
				{
					...
					hCurrent.Set(candidates[0].pMember->BaseEntity());
				}
			}
		}

		CAI_NPC *temp = hCurrent;
		if ( temp != NULL )
		{
			Assert( dynamic_cast<CNPC_Citizen *>(temp) && temp->IsInPlayerSquad() );
			return temp->BaseEntity();
		}
	}
	return NULL;
}
...
//-----------------------------------------------------------------------------
bool CNPC_Citizen::HandleInteraction(int interactionType, void *data, CBaseEntity* sourceEnt)
{
	...
	else if (interactionType ==	g_interactionScannerInspectBegin)
	{
		...
		Vector	targetDir = ( cent->WorldSpaceCenter() - WorldSpaceCenter() );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Citizen::FValidateHintType( CBaseEntity *pHint )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Citizen::ShouldHealTarget( CEntity *pTarget, bool bActiveUse )
{
	Disposition_t disposition;
	
	if ( !pTarget && ( ( disposition = IRelationType( pTarget->BaseEntity() ) ) != D_LI && disposition != D_NU ) )
		return false;
			...
			)
	 	{
			if ( pTarget->m_iHealth > 0 )
			{
				...
				if ( bReadyToHeal )
				{
					...
					if ( ( pTarget->m_iHealth <= requiredHealth ) && IRelationType( pTarget->BaseEntity() ) == D_LI )
						return true;
				}
			}
		}
	...
	if ( IsAmmoResupplier() && bTargetIsPlayer )
	{
		if ( m_flPlayerGiveAmmoTime <= gpGlobals->curtime )
		{
			...
			else
			{
				...
				int iCount = ((CPlayer*)pTarget)->GetAmmoCount(iAmmoType);
				if ( !iCount || ((iMax - iCount) >= m_iAmmoAmount) )
				{
					// Only give the player ammo if he has a weapon that uses it
					if ( ((CPlayer*)pTarget)->Weapon_GetWpnForAmmo( iAmmoType ) )
						return true;
				}
			}
		}
...
void CNPC_Citizen::Heal()
{
	...
	if ( IsAmmoResupplier() )
	{
		...
		if ( pTarget->IsPlayer() )
		{
			...
			{
				((CPlayer*)pTarget)->GiveAmmo( m_iAmmoAmount, iAmmoType, false );
			}
...
void	CNPC_Citizen::TossHealthKit(CBaseCombatCharacter *pThrowAt, const Vector &offset)
{
	...
	if (pHealthKit)
	{
...

		{
			...
			if ( pPhysicsObject )
			{
				unsigned int cointoss = enginerandom->RandomInt(0,0xFF); // int bits used for bools

				// some enginerandom precession
				Vector angDummy(enginerandom->RandomFloat(-200,200), enginerandom->RandomFloat(-200,200), 
					cointoss & 0x01 ? enginerandom->RandomFloat(200,600) : -1.0f * enginerandom->RandomFloat(200,600));
...
bool CNPC_Citizen::ShouldLookForHealthItem()
{
	...
	CPlayer *pNearest = UTIL_GetNearestVisiblePlayer(this);
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Citizen::OnGivenWeapon( CBaseEntity *pNewWeapon )
{
	FixupMattWeapon();
}
...
LINK_ENTITY_TO_CUSTOM_CLASS( ai_citizen_response_system, info_target, CCitizenResponseSystem );
		...
		for ( int i = 0; i < MAX_CITIZEN_RESPONSES; i++ )
		{
			if ( m_flResponseAddedTime[i] )
			{
				...
				else if ( m_flNextResponseTime < gpGlobals->curtime )
				{
					...
					CEntity *pNearestCitizen = NULL;
					CEntity *pCitizen = NULL;
					while ( (pCitizen = g_helpfunc.FindEntityByClassname( pCitizen, "npc_citizen" ) ) != NULL)
					{
						CPlayer *pPlayer = UTIL_GetNearestPlayer(pCitizen->GetAbsOrigin());
						Assert(pPlayer);
...
{
	CEntity *pMark = CreateEntityByName( "squadinsignia" );
	pMark->SetOwnerEntity( BaseEntity() );
	pMark->Spawn();
}
...
void CNPC_Citizen::RemoveInsignia()
{
	...
	CEntity *pEntity = CEntity::Instance(g_helpfunc.FirstEnt());
	...
	while( pEntity )
	{
		...
		pEntity = CEntity::Instance(g_helpfunc.NextEnt( pEntity->BaseEntity() ));
	}
}

//-----------------------------------------------------------------------------
LINK_ENTITY_TO_CUSTOM_CLASS( squadinsignia, item_sodacan, CSquadInsignia );

void CSquadInsignia::Spawn()
{
	CAI_NPC *pOwner = ( GetOwnerEntity() ) ? GetOwnerEntity()->MyNPCPointer() : NULL;
	...
	if ( pOwner )
	{
		...
		if ( attachment )
		{
			...
			SetParent( GetOwnerEntity()->BaseEntity(), attachment );
