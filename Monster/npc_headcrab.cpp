...
void CBaseHeadcrab::OnChangeActivity( Activity NewActivity )
{
	...
	// If this crab is starting to walk or idle, pick a enginerandom point within
...
// Input  : bRandomJump - Just hop in a enginerandom direction.
	...
	if ( pEvent->event == AE_HEADCRAB_JUMPATTACK )
	{
		...
		if ( pEnemy )
		{
			...
			else
			{
				...
				Vector temp = pEnemy->EyePosition();
				temp.z -= 5.0f;
				JumpAttack( false, temp );
			}
...
void CBaseHeadcrab::BeginClimbFromCanister()
{
	...
	{
		// Choose a enginerandom direction (forward, left, or right)
	...
	case TASK_HEADCRAB_HOP_OFF_NPC:
		{
			...
			if( ground )
			{
				...
				// Jump in some enginerandom direction. This way if the person I'm standing on is
...
int CBaseHeadcrab::SelectSchedule( void )
{
	...
	{
		if ( ( HasCondition( COND_CAN_RANGE_ATTACK1 ) || HasCondition( COND_NEW_ENEMY ) ) )
...
//			*ptr - 
//-----------------------------------------------------------------------------
void CBaseHeadcrab::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
	...
	BaseClass::TraceAttack( newInfo, vecDir, ptr, NULL );
}
...
bool CBaseHeadcrab::HandleInteraction(int interactionType, void *data, CBaseEntity* sourceEnt)
{
				...
				)
	{
		...
		Vector vHitDir = GetLocalOrigin() - cent->GetLocalOrigin();
	...
	case TASK_HEADCRAB_HOP_ASIDE:
		{
			...
			if( tr.fraction == 1.0 )
			{
				...
				GetMoveProbe()->MoveLimit( NAV_JUMP, GetAbsOrigin(), tr.endpos, MASK_NPCSOLID, GetEnemy_CBase(), &moveTrace );
...
//-----------------------------------------------------------------------------
// Purpose: Does a spastic hop in a enginerandom or provided direction.
// Input  : pvecDir - 2D direction to hop, NULL picks a enginerandom direction.
//-----------------------------------------------------------------------------
void CBlackHeadcrab::JumpFlinch( const Vector *pvecDir )
{
	...
	//
	// Jump in a enginerandom direction.
	//
	Vector up;
...
void CBlackHeadcrab::HandleAnimEvent( animevent_t *pEvent )
{
	...
	{
		//
		// Hop in a enginerandom direction, then run and hide. If we're already running
