...
void CNPC_Houndeye::AlertSound ( void )
{
	...
	if ( GetSquad() && !GetSquad()->IsLeader( this ) )
	{
		return; 
	}
...
{
	if (GetSquad() && enginerandom->RandomInt(0,10) == 10)
	{
...
void CNPC_Houndeye::SonicAttack ( void )
{
	...
	te->BeamRingPoint(filter, 0.0, pos, 16, HOUNDEYE_MAX_ATTACK_RADIUS, g_sModelIndexLaser, 0, 0, 2, 0.3f, 20, 16, 0, 0, 255, 0, 255, 128);
	te->BeamRingPoint(filter, 0.1, pos, 16, HOUNDEYE_MAX_ATTACK_RADIUS, g_sModelIndexLaser, 0, 0, 2, 0.3f, 15, 16, 0, 0, 255, 0, 255, 128);
	te->BeamRingPoint(filter, 0.2, pos, 16, HOUNDEYE_MAX_ATTACK_RADIUS, g_sModelIndexLaser, 0, 0, 2, 0.3f, 10, 16, 0, 0, 255, 0, 255, 128);
	te->BeamRingPoint(filter, 0.3, pos, 16, HOUNDEYE_MAX_ATTACK_RADIUS, g_sModelIndexLaser, 0, 0, 2, 0.3f, 5, 16, 0, 0, 255, 0, 255, 128);
...
void CNPC_Houndeye::StartTask( const Task_t *pTask )
{
	...
	case TASK_HOUND_GET_PATH_TO_CIRCLE:
	{
		...
		else 
		{
			...
			/*if (GetNavigator()->SetRadialGoal(vTargetPos, enginerandom->RandomInt(50,500), 90, 175, m_bLoopClockwise))
			{
				TaskComplete();
				return;
			}*/
...
void CNPC_Houndeye::PrescheduleThink ( void )
{
	...
	// at enginerandom, initiate a blink if not already blinking or sleeping
	if ( !m_fDontBlink )
	{
		/*//<<TEMP>>//<<TEMP>>
		if ( ( pev->skin == 0 ) && enginerandom->RandomInt(0,0x7F) == 0 )
		{// start blinking!
...
bool CNPC_Houndeye::IsAnyoneInSquadAttacking( void )
{
	...
	for (CBaseEntity *pSquadMember = GetSquad()->GetFirstMember( &iter ); pSquadMember; pSquadMember = GetSquad()->GetNextMember( &iter ) )
	{
		...
		if (npc->IsCurSchedule(SCHED_HOUND_RANGE_ATTACK1))
		{
			return true;
		}
	}
	...
	case NPC_STATE_COMBAT:
		{
			...
			{
				if (GetSquad() && enginerandom->RandomInt(0,4) == 0)
				{
			...
			{
				if (GetSquad() && enginerandom->RandomInt(0,5) == 0)
				{
					if (!IsAnyoneInSquadAttacking())
					{
...
//-----------------------------------------------------------------------------
bool CNPC_Houndeye::HandleInteraction(int interactionType, void *data, CBaseEntity* sourceEnt)
{
