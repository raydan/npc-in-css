...
public:
	...
	CFakeHandle m_Advisor; ///< handle to the advisor.
...
// The advisor class.
//-----------------------------------------------------------------------------
class CNPC_Advisor : public CE_Cycler_Fix
{
public:
	CE_DECLARE_CLASS( CNPC_Advisor, CE_Cycler_Fix );
	...
	bool DidThrow(const CEntity *pEnt);
#else
...
protected:
...
#if NPC_ADVISOR_HAS_BEHAVIOR
	Vector GetThrowFromPos( CEntity *pEnt ); ///< Get the position in which we shall hold an object prior to throwing it
#endif

	bool CanLevitateEntity( CEntity *pEntity, int minMass, int maxMass );
	void StartLevitatingObjects( void );


#if NPC_ADVISOR_HAS_BEHAVIOR
	// void PurgeThrownObjects(); ///< clean out the recently thrown objects array
	void AddToThrownObjects(CEntity *pEnt); ///< add to the recently thrown objects array

	void HurlObjectAtPlayer( CEntity *pEnt, const Vector &leadVel );
	void PullObjectToStaging( CEntity *pEnt, const Vector &stagingPos );
	CEntity *ThrowObjectPrepare( void );

	CEntity *PickThrowable( bool bRequireInView ); ///< choose an object to throw at the player (so it can get stuffed in the handle array)

	/// push everything out of the way between an object I'm about to throw and the player.
	void PreHurlClearTheWay( CEntity *pThrowable, const Vector &toPos );
#endif
	...
	CFakeHandle m_hLevitateGoal1;
	CFakeHandle m_hLevitateGoal2;
	CFakeHandle m_hLevitationArea;
	...
	void Write_BeamOn(  CEntity *pEnt ); 	///< write a message turning a beam on
	void Write_BeamOff( CEntity *pEnt );   	///< write a message turning a beam off
	void Write_AllBeamsOff( void );				///< tell client to kill all beams

	// for the pin-the-player-to-something behavior
	CFakeHandle m_hPlayerPinPos;
	...
	CFakeHandle m_haRecentlyThrownObjects[kMaxThrownObjectsTracked]; 
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_advisor, cycler, CNPC_Advisor );
...
void CNPC_Advisor::Activate()
{
	...
	CEntity *cent;
	cent = g_helpfunc.FindEntityGeneric( NULL, STRING( m_iszLevitateGoal1 ),  BaseEntity() );
	m_hLevitateGoal1.Set((cent)?cent->BaseEntity():NULL);
	
	cent = g_helpfunc.FindEntityGeneric( NULL, STRING( m_iszLevitateGoal2 ),  BaseEntity() );
	m_hLevitateGoal2.Set((cent)?cent->BaseEntity():NULL);
		
	cent = g_helpfunc.FindEntityGeneric( NULL, STRING( m_iszLevitationArea ), BaseEntity() );
	m_hLevitationArea.Set((cent)?cent->BaseEntity():NULL);


	m_levitateCallback.m_Advisor.Set(BaseEntity());
	...
	while ( pEnt = g_helpfunc.FindEntityByName(pEnt,m_iszStagingEntities) )
	{
		m_hvStagingPositions.AddToTail(pEnt->BaseEntity());
	}
...
void CNPC_Advisor::StartLevitatingObjects()
{
	...
	{
		CEntity *pEnt = CEntity::Instance(m_physicsObjects.Element( i ));
		if ( !pEnt )
			continue;
...
bool CNPC_Advisor::CanLevitateEntity( CEntity *pEntity, int minMass, int maxMass )
{
...
CEntity *CNPC_Advisor::ThrowObjectPrepare()
{

	CEntity *pThrowable = NULL;
	while (m_hvStagedEnts.Count() > 0)
	{
		pThrowable = CEntity::Instance(m_hvStagedEnts[0]);
...
void CNPC_Advisor::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_ADVISOR_FIND_OBJECTS:
		{
			...
			CEntity *pVolume = m_hLevitationArea;
				...
				for ( touchlink_t *link = touchroot->nextLink; link != touchroot; link = link->nextLink )
				{
					...
					if ( CanLevitateEntity( pTouch, 10, 220 ) )
					{
						if ( pTouch->GetMoveType() == MOVETYPE_VPHYSICS )
						{
							...
							m_physicsObjects.AddToTail( pTouch->BaseEntity() );
						}
					}
				}
			}
		...
		case TASK_ADVISOR_STAGE_OBJECTS:
		{
			...
			*/
			...
			m_OnPickingThrowable.FireOutput((CBaseEntity *)NULL,BaseEntity());
		...
		case TASK_ADVISOR_BARRAGE_OBJECTS:
		{
			...
			CEntity *pThrowable = ThrowObjectPrepare();
...
void CNPC_Advisor::RunTask( const Task_t *pTask )
{
	...
	switch ( pTask->iTask )
	{
		...
		// Pick a enginerandom object that we are levitating. If we have a clear LOS from that object
...
// advisor_staging_num
			...
			{	
				...
				CEntity *pThrowable = PickThrowable(!bDesperate);
				if (pThrowable)
				{
					...
					m_hvStagedEnts.AddToTail( pThrowable->BaseEntity() );
			...
			for (int ii = m_hvStagedEnts.Count() - 1 ; ii >= 0 ; --ii)
			{
				...
				CEntity *pThrowable = CEntity::Instance(m_hvStagedEnts[ii]);
				if (pThrowable)
				{
					PullObjectToStaging(pThrowable, CEntity::Instance(m_hvStagingPositions[ii])->GetAbsOrigin());
				}
			}
		...
		case TASK_ADVISOR_BARRAGE_OBJECTS:
		{
			...
			if ( !GetEnemy() )
			{
				...
				for (int ii = m_hvStagedEnts.Count() - 1 ; ii >=0 ; --ii)
				{
					...
					IPhysicsObject *pPhys = CEntity::Instance(m_hvStagedEnts[ii])->VPhysicsGetObject();
					if ( pPhys )
					{  
						pPhys->SetGameFlags(pPhys->GetGameFlags() & (~FVPHYSICS_NO_PLAYER_PICKUP) );
					}
			...
			CEntity *pThrowable = CEntity::Instance(m_hvStagedEnts[0]);
			while (!pThrowable) 
			...
			if (gpGlobals->curtime > m_flThrowPhysicsTime)
			{
				...
				Write_BeamOff(CEntity::Instance(m_hvStagedEnts[0]));
				m_hvStagedEnts.Remove(0);
...
static bool AdvisorCanPickObject(CPlayer *pPlayer, CEntity *pEnt)
{
	Assert( pPlayer != NULL );

	// Is the player carrying something?
	CEntity *pHeldObject = GetPlayerHeldEntity(pPlayer);
...
//-----------------------------------------------------------------------------
CEntity *CNPC_Advisor::PickThrowable( bool bRequireInView )
{
	...
	if (!!m_iszPriorityEntityGroupName) // if the string isn't null
	{
		...
		{
			CEntity *pThrowEnt = CEntity::Instance(m_physicsObjects[ii]);
			// Assert(pThrowEnt); 
			if (!pThrowEnt)
				continue;
	...
	// pick a enginerandom candidate.
	int nRandomIndex = enginerandom->RandomInt( 0, numCandidates - 1 );
	return CEntity::Instance(m_physicsObjects[candidates[nRandomIndex]]);
...
//-----------------------------------------------------------------------------
void CNPC_Advisor::HurlObjectAtPlayer( CEntity *pEnt, const Vector &leadVel )
{
	IPhysicsObject *pPhys = pEnt->VPhysicsGetObject();
	...
	//
	Vector vecObjOrigin = pEnt->CollisionProp_Actual()->WorldSpaceCenter();
...
//-----------------------------------------------------------------------------
void CNPC_Advisor::PreHurlClearTheWay( CEntity *pThrowable, const Vector &toPos )
{
	...
	{
		CEntity *obstruction = CEntity::Instance(list[i]);
		if (  obstruction != pThrowable                  &&
			m_physicsObjects.HasElement( obstruction->BaseEntity() ) && // if it's floating
			 !m_hvStagedEnts.HasElement( obstruction->BaseEntity() )   && // and I'm not staging it
...
//-----------------------------------------------------------------------------
bool CNPC_Advisor::DidThrow(const CEntity *pEnt)
{
	...
	for (int ii = 0 ; ii < kMaxThrownObjectsTracked ; ++ii)
	{
		...
		CEntity *pTestEnt = CEntity::Instance(m_haRecentlyThrownObjects[ii]);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Advisor::AddToThrownObjects(CEntity *pEnt)
{
	Assert(pEnt);
	...
	m_haRecentlyThrownObjects[oldestThrownObject].Set(pEnt->BaseEntity());
...
//-----------------------------------------------------------------------------
void CNPC_Advisor::PullObjectToStaging( CEntity *pEnt, const Vector &stagingPos )
{
	IPhysicsObject *pPhys = pEnt->VPhysicsGetObject();
	Assert(pPhys);

	Vector curPos = pEnt->CollisionProp_Actual()->WorldSpaceCenter();
...
int	CNPC_Advisor::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( oldLastDamageTime != gpGlobals->curtime )
	{
		...
		m_OnHealthIsNow.Set( GetHealth(), newInfo.GetAttacker(), BaseEntity());
	}
...
//-----------------------------------------------------------------------------
Vector CNPC_Advisor::GetThrowFromPos( CEntity *pEnt )
{
	Assert(pEnt);
	Assert(pEnt->VPhysicsGetObject());
	const CECollisionProperty *cProp = pEnt->CollisionProp();
	Assert(cProp);
...
bool CNPC_Advisor::QueryHearSound( CSound *pSound )
{
	...
	CEntity *pOwner = CEntity::Instance(pSound->m_hOwner);
...
void CNPC_Advisor::InputPinPlayer( inputdata_t &inputdata )
{
	...
	CEntity *pEnt = g_helpfunc.FindEntityByName((CBaseEntity *)NULL,targetname);

	if (pEnt)
	{
		m_hPlayerPinPos.Set(pEnt->BaseEntity());
	}
	else
	{
...
void CNPC_Advisor::InputWrenchImmediate( inputdata_t &inputdata )
{
	...
	// conditional assignment: find an entity by name and save it into pEnt. Bail out when none are left.
	while ( ( pEnt = g_helpfunc.FindEntityByName(pEnt,groupname) ) != NULL )
	{
		// if I'm not already levitating it, and if I didn't just throw it
		if (!m_physicsObjects.HasElement(pEnt->BaseEntity()) )
		{
			// add to levitation
			IPhysicsObject *pPhys = pEnt->VPhysicsGetObject();
			if ( pPhys )
			{
				...
				objectToMe *= enginerandom->RandomFloat(0.25f,1.0f);

				pPhys->SetVelocity( &objectToMe, NULL );

				// add it to tracked physics objects
				m_physicsObjects.AddToTail( pEnt->BaseEntity() );
			...
			{
				Warning( "Advisor tried to wrench %s, but it is not moveable!", pEnt->GetEntityName());
			}
		}
	}
...
//-----------------------------------------------------------------------------
void CNPC_Advisor::Write_BeamOn(  CEntity *pEnt )
{
	Assert( pEnt );
	//CE_TODO
...
//-----------------------------------------------------------------------------
void CNPC_Advisor::Write_BeamOff( CEntity *pEnt )
{
	Assert( pEnt );
	//CE_TODO
	/*EntityMessageBegin( this, true );
...
void CNPC_Advisor::InputTurnBeamOn( inputdata_t &inputdata )
{
	...
	CEntity *pTarget = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, inputdata.value.StringID() );
	if ( pTarget )
	{
...
CAdvisorLevitate::simresult_e	CAdvisorLevitate::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	...
	{	// independent movement of all objects
		...
		CBaseEntity *cbase = static_cast<CBaseEntity *>(pObject->GetGameData());
		if (pAdvisor->DidThrow(CEntity::Instance(cbase)))
		{
