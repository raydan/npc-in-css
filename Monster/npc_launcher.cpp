...
//	>> CNPC_Launcher
//=========================================================
class CNPC_Launcher : public CE_Cycler_Fix
{

public:
	CE_DECLARE_CLASS( CNPC_Launcher, CE_Cycler_Fix );
	...
	void				LaunchGrenade(CEntity* pLauncher );
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_launcher, cycler, CNPC_Launcher );
...
void CNPC_Launcher::Precache( void )
{
	...
	g_helpfunc.UTIL_PrecacheOther( "grenade_homer");
	BaseClass::Precache();
}
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_Launcher::LaunchGrenade( CEntity* pEnemy )
{
	...
	if (m_bSmokeLaunch)
	{
		UTIL_Smoke(GetAbsOrigin(), enginerandom->RandomInt(20,30), enginerandom->RandomInt(10,15));
	}
...
//------------------------------------------------------------------------------
bool CNPC_Launcher::FInViewCone_Entity( CBaseEntity *pEntity )
{
	return true;
}
...
bool CNPC_Launcher::IsValidEnemy( CBaseEntity *pTarget )
{
	...
	//  Check range
	// ---------------------------------
	float flTargetDist = (GetAbsOrigin() - cent->GetAbsOrigin()).Length();
	...
	Vector vEndPos	 = cent->GetAbsOrigin();
	UTIL_TraceLine( vStartPos, vEndPos, MASK_SHOT, pTarget, COLLISION_GROUP_NONE, &tr );
...
void CNPC_Launcher::LauncherThink( void )
{
	if (gpGlobals->curtime > m_flNextAttack)
	{
		...
		else
		{
			...
			CEntity* pBestEnemy = CEntity::Instance(BestEnemy());
