...
{ 
	...
	{
		if (m_iKeySkin == 0)
		{	// select a "enginerandom" skin -- rather than being truly enginerandom, use a static variable
...
void CNPC_FloorTurret::OnPhysGunPickup( CBaseEntity *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if ( reason != PUNTED_BY_CANNON )
	{
		...
		Vector vecToTurret = CEntity::Instance(pPhysGunUser)->GetAbsOrigin() - GetAbsOrigin();
		vecToTurret.z = 0;
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_FloorTurret::OnAttemptPhysGunPickup( CBaseEntity *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if ( reason == PICKED_UP_BY_CANNON && IRelationType(pPhysGunUser ) == D_HT )
	{
		...
		Vector vecForce = (CEntity::Instance(pPhysGunUser)->GetAbsOrigin() - GetAbsOrigin());
...
void CNPC_FloorTurret::ActiveThink( void )
{
	...
	bool bEnemyVisible = FVisible_Entity( GetEnemy_CBase() ) && GetEnemy()->IsAlive();	
...
void CNPC_FloorTurret::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy, bool bStrict )
{
	...
	if(cent)
	{
		pAttacker = cent->BaseEntity();
	} else {
		pAttacker = BaseEntity();
	}
	...
	info.m_flDamage = (m_fBulletDamage <= 0.0f) ? sk_npc_turret_floor_bullet_damage.GetFloat() : m_fBulletDamage;
