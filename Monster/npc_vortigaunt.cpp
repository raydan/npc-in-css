...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt::PlayerBelowHealthPercentage( CPlayer *pPlayer, float flPerc )
{
	if ( pPlayer == NULL )
		return false;
...
void CNPC_Vortigaunt::StartTask( const Task_t *pTask )
{
	...
	case TASK_VORTIGAUNT_GET_HEAL_TARGET:
	{
		...
		{
			SetTarget( m_hHealTarget->BaseEntity() );
			TaskComplete();
		}
	...
	case TASK_VORTIGAUNT_HEAL:
	{
		...
		CPlayer *pPlayer = ToBasePlayer( m_hHealTarget );
		if ( pPlayer == NULL )
		{
...
int CNPC_Vortigaunt::NumAntlionsInRadius( float flRadius )
{
	...
	{
		CEntity *cent = CEntity::Instance(sEnemySearch[i]);
		// We only care about antlions
		if ( cent == NULL || cent->Classify() != CLASS_ANTLION )
			continue;
...
void CNPC_Vortigaunt::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_VORTIGAUNT_ZAP_SHOOT )
	{
		...
		{
			// Spawn bugbait!
			CCombatWeapon *pWeapon = Weapon_Create( "weapon_bugbait" );
			if ( pWeapon )
			{
				...
				{
					trace_t tr;
					CTraceFilterSkipTwoEntities traceFilter( GetTarget()->BaseEntity(), BaseEntity(), COLLISION_GROUP_NONE );
					UTIL_TraceLine( vecSpawnOrigin, vecSpawnOrigin, MASK_SHOT, &traceFilter, &tr );

					if ( tr.fraction == 1.0 && !tr.m_pEnt )
					{
						// Make sure it can fit there
						UTIL_TraceHull( vecSpawnOrigin, vecSpawnOrigin, -Vector(16,16,16), Vector(16,16,48), MASK_SHOT, &traceFilter, &tr );
						if ( tr.fraction == 1.0 && !tr.m_pEnt )
							break;
					}
			...
			g_helpfunc.DispatchEffect( "AntlionGib", data );
		}
...
void CNPC_Vortigaunt::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( HealBehaviorAvailable() )
	{
		...
		CEntity *pHealTarget = FindHealTarget();
		if ( pHealTarget != NULL )
		{
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump)
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt::SetHealTarget( CEntity *pTarget, bool bPlayerRequested )
{
	SetCondition( COND_VORTIGAUNT_CAN_HEAL );
	OccupyStrategySlot( SQUAD_SLOT_HEAL_PLAYER );
	m_hHealTarget.Set((pTarget)?pTarget->BaseEntity():NULL);
...
//-----------------------------------------------------------------------------
CEntity *CNPC_Vortigaunt::FindHealTarget( void )
{
	...
	CEntity *pEntity = PlayerInRange( GetAbsOrigin(), HEAL_SEARCH_RANGE );
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt::ShouldHealTarget( CEntity *pTarget )
{
	...
	if ( m_eHealState == HEAL_STATE_HEALING )
	{
		...
		if ( HasCondition( COND_VORTIGAUNT_HEAL_VALID ) )
		{
			if ( m_flNextHealTokenTime < gpGlobals->curtime )
			{
				...
				m_flNextHealTokenTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.5f, 1.0f );
				m_nNumTokensToSpawn--;
...
void CNPC_Vortigaunt::ZapBeam( int nHand )
{
	...
	{
		CE_CRagdollProp *pTest = dynamic_cast< CE_CRagdollProp *>( GetTarget() );
		...
		if ( pTest )
		{
			...
			if ( m_ragdoll )
			{
				...
				UTIL_TraceLine( vecSrc, vOrigin, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr);
			}

			CE_CRagdollBoogie::Create( pTest, 200, gpGlobals->curtime, 1.0f );
		}
	} else {
	...
	if ( pEntity != NULL && *(m_takedamage) )
	{
		...
		pEntity->DispatchTraceAttack( dmgInfo, vecAim, &tr, NULL );
	}
...
{
	CEntity *pTarget = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, data.value.String(), NULL, data.pActivator, data.pCaller );
	...
	int playerArmor = (pTarget->IsPlayer()) ? ((CPlayer *)pTarget)->ArmorValue() : 0;
	...
	m_hHealTarget.Set(pTarget->BaseEntity());
...
int CNPC_Vortigaunt::IRelationPriority( CBaseEntity *pTarget )
{
	...
	if ( m_FollowBehavior.GetFollowTarget() && 
		m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().DistToSqr( cent_pTarget->GetAbsOrigin() ) < Square(25*12) )
	{
		priority++;
	}
...
bool CNPC_Vortigaunt::HealGestureHasLOS( void )
{
	...
	CEntity *pTargetEnt = UTIL_GetNearestVisiblePlayer(this);
	if ( pTargetEnt == NULL )
		return false;
	...
	trace_t tr;
	CTraceFilterSkipTwoEntities filter( BaseEntity(), pTargetEnt->BaseEntity(), COLLISION_GROUP_NONE );
...
void CNPC_Vortigaunt::DispelAntlions( const Vector &vecOrigin, float flRadius, bool bDispel /*= true*/ )
{
	...
	for ( int i = 0; i < nNumEnemies; i++ )
	{
		CEntity *cent = CEntity::Instance(pEnemySearch[i]);
		// We only care about antlions
		if ( IsAntlion( cent ) == false )
			continue;

		CNPC_Antlion *pAntlion = static_cast<CNPC_Antlion *>(cent);
...
// attached to enginerandom debris chunks while inside scripted sequences.
//-----------------------------------------------------------------------------
