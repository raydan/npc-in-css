...
{
public:
	CE_DECLARE_CLASS( CNPC_Zombine, CAI_BlendingHost<CNPC_BaseZombie> );
	DECLARE_DATADESC();
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_zombine, cycler, CNPC_Zombine );
...
void CNPC_Zombine::PrescheduleThink( void )
{
	...
	if( gpGlobals->curtime > m_flNextMoanSound )
	{
		if( CanPlayMoanSound() )
		{
			...
			m_flNextMoanSound = gpGlobals->curtime + enginerandom->RandomFloat( 10.0, 15.0 );
		}
		else
		{
			m_flNextMoanSound = gpGlobals->curtime + enginerandom->RandomFloat( 2.5, 5.0 );
		}
	}

	if ( HasGrenade () )
	{
		g_helpfunc.CSoundEnt_InsertSound ( SOUND_DANGER, GetAbsOrigin() + GetSmoothedVelocity() * 0.5f , 256, 0.1, BaseEntity(), SOUNDENT_CHANNEL_ZOMBINE_GRENADE );
...
void CNPC_Zombine::GatherGrenadeConditions( void )
{
	...
	if ( GetEnemy_CBase() == NULL )
		return;

	if ( FVisible_Entity( GetEnemy_CBase() ) == false )
		return;
	...
	if ( pPlayer && pPlayer->FVisible_Entity( BaseEntity() ) )
	{
		...
		if ( flLengthToPlayer <= GRENADE_PULL_MAX_DISTANCE && flLengthToEnemy <= GRENADE_PULL_MAX_DISTANCE )
		{
			...
			if ( flPullChance >= enginerandom->RandomFloat( 0.0f, 1.0f ) )
			{
...
//-----------------------------------------------------------------------------
bool CNPC_Zombine::HandleInteraction( int interactionType, void *data, CBaseEntity *sourceEnt )
{
...
void CNPC_Zombine::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
...
void CNPC_Zombine::HandleAnimEvent( animevent_t *pEvent )
{
	if ( pEvent->event == AE_ZOMBINE_PULLPIN )
	{
		...
		CE_Grenade *pGrenade = Fraggrenade_Create( vecStart, vec3_angle, vec3_origin, AngularImpulse( 0, 0, 0 ), this, 3.5f, true );
		...
		if ( pGrenade )
		{
			...
			if ( pPhysicsObject )
			{
				...
				pGrenade->SetParent( BaseEntity(), iAttachment );

				pGrenade->SetDamage( 200.0f );
				m_hGrenade.Set(pGrenade->BaseEntity());
				...
				for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
				{
					...
					if( pNPC->Classify() == CLASS_PLAYER_ALLY || pNPC->Classify() == CLASS_PLAYER_ALLY_VITAL && pNPC->FVisible_Entity(BaseEntity()) )
					{
						int priority;
						Disposition_t disposition;

						priority = pNPC->IRelationPriority(BaseEntity());
						disposition = pNPC->IRelationType(BaseEntity());

						pNPC->AddEntityRelationship( BaseEntity(), disposition, priority + 1 );
					}
				}
			}
...
bool CNPC_Zombine::AllowedToSprint( void )
{
	...
	if ( ( GetHealth() > GetMaxHealth() * 0.5f ) )
	{
		...
		if ( enginerandom->RandomInt( 0, 100 ) > iChance )
			return false;
...
void CNPC_Zombine::StopSprint( void )
{
	...
	m_flSprintRestTime = m_flSprintTime + enginerandom->RandomFloat( 2.5f, 5.0f );
}
...
void CNPC_Zombine::Sprint( bool bMadSprint )
{
	...
	float flSprintTime = enginerandom->RandomFloat( MIN_SPRINT_TIME, MAX_SPRINT_TIME );
...
//-----------------------------------------------------------------------------
// Purpose: Play a enginerandom attack hit sound
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Play a enginerandom attack miss sound
//-----------------------------------------------------------------------------
void CNPC_Zombine::AttackMissSound( void )
{
	// Play a enginerandom attack miss sound
...
//-----------------------------------------------------------------------------
// Purpose: Play a enginerandom idle sound.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Play a enginerandom attack sound.
//-----------------------------------------------------------------------------
...
{
	CEntity *pGrenade = m_hGrenade;
	ReleaseGrenade( vPhysgunPos );
	return (pGrenade)?pGrenade->BaseEntity():NULL;
}
