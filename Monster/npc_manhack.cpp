...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Manhack::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
...
void CNPC_Manhack::Bump( CEntity *pHitEntity, float flInterval, trace_t &tr )
{
	...
	if (GetNavigator()->IsGoalActive() && !(GetNavigator()->GetPath()->CurWaypointFlags() & bits_WP_TO_PATHCORNER) )
	{
		...
		GetMoveProbe()->MoveLimit( NAV_GROUND, GetAbsOrigin(), GetNavigator()->GetCurWaypointPos(), 
			MoveCollisionMask(), GetEnemy_CBase(), &moveTrace );
...
void CNPC_Manhack::MoveExecute_Alive(float flInterval)
{
	...
	{
		/*
		// If I'm stalled add enginerandom noise
...
//		UTIL_Smoke(GetAbsOrigin(), enginerandom->RandomInt(10, 15), 10);
	...
	// ------------------------------------------
	// If I'm dying, add enginerandom banking noise
	// ------------------------------------------
