	...
	if ( !bIgnoreSolidEntities )
	{
		...
		if ( count )
		{
			...
			for ( int i = 0; i < count; i++ )
			{
				...
				if ( ( cent->GetSolidFlags() & FSOLID_NOT_SOLID ) == false )
				{
...
CNPCSpawnDestination *CTemplateNPCMaker::FindSpawnDestination()
{
	...
	{
		// Pretty lame way to pick randomly. Try a few times to find a enginerandom
...
void CTemplateNPCMaker::MakeNPCInRadius( void )
{
	...
	if ( !PlaceNPCInRadius( pent ) )
	{
		...
		UTIL_RemoveImmediate( pent->BaseEntity() );
		return;
	}
