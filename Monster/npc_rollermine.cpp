...
void CNPC_RollerMine::WakeNeighbors()
{
	...
	{
		boxCount--;
		CEntity *cent = CEntity::Instance(entityList[boxCount]);
...
int CNPC_RollerMine::CountRollersOnMyVehicle( CUtlVector<CNPC_RollerMine*> *pRollerList )
{
	...
	{
		CEntity *cent = CEntity::Instance(entityList[i]);
		CAI_NPC *pNPC = cent->MyNPCPointer();
		if ( pNPC && *(pNPC->m_iClassname) == *(m_iClassname) && pNPC != this )
		{
...
//			*ptr - 
//-----------------------------------------------------------------------------
void CNPC_RollerMine::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
