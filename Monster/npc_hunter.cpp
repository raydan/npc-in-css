...
extern short g_sModelIndexBlueLight;
extern short g_sModelIndexBlueLaser;
...
//-----------------------------------------------------------------------------
bool IsStriderBuster( CEntity *pEntity )
{
	if ( !pEntity )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool HateThisStriderBuster( CEntity *pTarget )
{
...
class CHunterFlechette : public CE_CPhysicsProp, public IParentPropInteraction
{
public:
	CE_DECLARE_CLASS( CHunterFlechette, CE_CPhysicsProp );
...
public:
	...
	void SetSeekTarget( CEntity *pTargetEntity );
	void Explode();
	...
	void OnParentPhysGunDrop( CBaseEntity *pPhysGunUser, PhysGunDrop_t Reason );

protected:

	void SetupGlobalModelData();

	void StickTo( CEntity *pOther, trace_t &tr );

	void BubbleThink();
	void BubbleThink_CBE();
	...
	void FlechetteTouch( CEntity *pOther );

	Vector m_vecShootPosition;
	CFakeHandle m_hSeekTarget;
	bool m_bThrownBack;
...
LINK_ENTITY_TO_CUSTOM_CLASS( hunter_flechette, physics_prop, CHunterFlechette );
...
void CC_Hunter_Shoot_Flechette( const CCommand& args )
{
	...
	CHunterFlechette *entity = CHunterFlechette::FlechetteCreate( pPlayer->EyePosition(), angEye, pPlayer->BaseEntity() );
	if ( entity )
	{
		entity->Precache();
		DispatchSpawn( entity->BaseEntity() );
...
void CHunterFlechette::PostConstructor()
{
...
//-----------------------------------------------------------------------------
void CHunterFlechette::SetSeekTarget( CEntity *pTargetEntity )
{
	if ( pTargetEntity )
	{
		m_hSeekTarget.Set(pTargetEntity->BaseEntity());
		SetContextThink( &CHunterFlechette::SeekThink_CBE, gpGlobals->curtime, s_szHunterFlechetteSeekThink );
	}
}
...
void CHunterFlechette::OnParentPhysGunDrop( CBaseEntity *pPhysGunUser, PhysGunDrop_t Reason )
{
	m_bThrownBack = true;
}
...
bool CHunterFlechette::CreateSprites( bool bBright )
{
	if ( bBright )
	{
		...
		te->BeamFollow(filter,0.0, entindex(), g_sModelIndexBlueLaser, 0, 0.2f, 1.0f, 1.0f, 1.0f, 255,255,255,255);
	}
	else
	{
		//DispatchParticleEffect( "hunter_flechette_trail", PATTACH_ABSORIGIN_FOLLOW, this );
		CPVSFilter filter( GetAbsOrigin() );
		te->BeamFollow(filter,0.0, entindex(), g_sModelIndexBlueLight, 0, 0.2f, 1.0f, 1.0f, 1.0f, 255,255,255,255);
	}
	return true;
}
...
void CHunterFlechette::Spawn()
{
	...
	UTIL_SetSize( BaseEntity(), -Vector(1,1,1), Vector(1,1,1) );
	SetSolid( SOLID_BBOX );
	SetGravity( 0.05f );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHunterFlechette::StickTo( CEntity *pOther, trace_t &tr )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHunterFlechette::FlechetteTouch( CEntity *pOther )
{
	...
	if ( pOther->m_takedamage != DAMAGE_NO )
	{
		...
		CE_CBreakable *pBreak = dynamic_cast <CE_CBreakable *>(pOther);
		...
		CTakeDamageInfo	dmgInfo( BaseEntity(), (cent)?cent->BaseEntity():NULL, flDamage, DMG_DISSOLVE | DMG_NEVERGIB );
		CalculateMeleeDamageForce( &dmgInfo, vecNormalizedVel, tr->endpos, 0.7f );
		dmgInfo.SetDamagePosition( tr->endpos );
		pOther->DispatchTraceAttack( dmgInfo, vecNormalizedVel, tr, NULL );
		...
		trace_t	tr2;
		UTIL_TraceLine( GetAbsOrigin(),	GetAbsOrigin() + vForward * 128, MASK_BLOCKLOS, pOther->BaseEntity(), COLLISION_GROUP_NONE, &tr2 );
		...
		if ( tr2.fraction != 1.0f )
		{
			...
			CEntity *cent = CEntity::Instance(tr2.m_pEnt);
			if ( cent == NULL || ( cent && cent->GetMoveType() == MOVETYPE_NONE ) )
			{
				CEffectData	data;
		...
		{
			CE_CPhysicsProp *pProp = dynamic_cast<CE_CPhysicsProp *>( pOther );
			if ( pProp )
			{
...
void CHunterFlechette::SeekThink_CBE()
{
	CHunterFlechette *cent = (CHunterFlechette *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->SeekThink();
	}
...
void CHunterFlechette::BubbleThink_CBE()
{
	CHunterFlechette *cent = (CHunterFlechette *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
...
void CHunterFlechette::Shoot( Vector &vecVelocity, bool bBrightFX )
{
	...
	SetContextThink( &CHunterFlechette::BubbleThink_CBE, gpGlobals->curtime + 0.1, s_szHunterFlechetteBubbles );
}
...
void CHunterFlechette::DangerSoundThink()
{
	...
	g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER|SOUND_CONTEXT_EXCLUDE_COMBINE, GetAbsOrigin(), 150, 0.5, BaseEntity() );
...
void CHunterFlechette::Explode()
{
	...
	DispatchParticleEffect( "blood_impact_synth_01", vecFuse, GetAbsAngles(), NULL );
	...
	g_helpfunc.USE_HL2_RADIUS_DAMAGE = true;
	RadiusDamage( CTakeDamageInfo( BaseEntity(), (cent)?cent->BaseEntity():NULL, sk_hunter_flechette_explode_dmg.GetFloat(), nDamageType ), GetAbsOrigin(), sk_hunter_flechette_explode_radius.GetFloat(), CLASS_NONE, NULL );
	g_helpfunc.USE_HL2_RADIUS_DAMAGE = false;
...
//-----------------------------------------------------------------------------
void Hunter_ApplyChargeDamage( CEntity *pHunter, CEntity *pTarget, float flDamage )
{
	...
	// Deal the damage
	CTakeDamageInfo	info( pHunter->BaseEntity(), pHunter->BaseEntity(), vecForce, offset, flDamage, DMG_CLUB );
	...
	virtual bool ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
	{
		...
		if ( pEntity )
		{
			...
			if ( !pEntity->IsNPC() && pEntity->GetMoveType() == MOVETYPE_VPHYSICS )
			{
				...
				{
					if ( entMass < m_minMass * 0.666f || pEntity->CollisionProp()->BoundingRadius() < (assert_cast<const CAI_NPC *>(CE_EntityFromEntityHandle( m_pPassEnt )))->GetHullHeight() )
					{
						return false;
					}
				}
			...
			{
				CEntity *pHunter = (CEntity *)CE_EntityFromEntityHandle( m_pPassEnt );
...
{
public:
	CE_DECLARE_CLASS( CNPC_Hunter, CAI_BaseActor );
	...
	bool			HandleInteraction(int interactionType, void *data, CBaseEntity *pSourceEnt);
	...
	int				MeleeAttack1ConditionsVsEnemyInVehicle( CCombatCharacter *pEnemy, float flDot );
	...
	CEntity *		MeleeAttack( float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch, int BloodOrigin );
...
private:
	...
	bool ShootFlechette( CEntity *pTargetEntity, bool bSingleShot );
	bool ShouldSeekTarget( CEntity *pTargetEntity, bool bStriderBuster );
	void GetShootDir( Vector &vecDir, const Vector &vecSrc, CEntity *pTargetEntity, bool bStriderbuster, int nShotNum, bool bSingleShot );
	...
	bool IsCorporealEnemy( CEntity *pEnemy );

	void PhysicsDamageEffect( const Vector &vecPos, const Vector &vecDir );
	bool PlayerFlashlightOnMyEyes( CPlayer *pPlayer );
	...
	// buster fu	
	CUtlVector< CFakeHandle >	m_hAttachedBusters;		// List of busters attached to us
	...
	CUtlVector<CFakeHandle> m_pSiegeTargets;
	string_t	m_iszSiegeTargetName;
	float		m_flTimeNextSiegeTargetAttack;
	CFakeHandle		m_hCurrentSiegeTarget;
	
	CFakeHandle		m_hHitByVehicle;
};


LINK_ENTITY_TO_CUSTOM_CLASS( npc_hunter, cycler, CNPC_Hunter );
LINK_ENTITY_TO_CUSTOM_CLASS( npc_alien_grunt, cycler, CNPC_Hunter );
...
void CNPC_Hunter::Precache()
{
	...
	g_helpfunc.PrecacheInstancedScene( "scenes/npc/hunter/hunter_scan.vcd" );
	g_helpfunc.PrecacheInstancedScene( "scenes/npc/hunter/hunter_eyeclose.vcd" );
	g_helpfunc.PrecacheInstancedScene( "scenes/npc/hunter/hunter_roar.vcd" );
	g_helpfunc.PrecacheInstancedScene( "scenes/npc/hunter/hunter_pain.vcd" );
	g_helpfunc.PrecacheInstancedScene( "scenes/npc/hunter/hunter_eyedarts_top.vcd" );
	g_helpfunc.PrecacheInstancedScene( "scenes/npc/hunter/hunter_eyedarts_bottom.vcd" );
	
	PrecacheMaterial( "effects/water_highlight" );

	g_helpfunc.UTIL_PrecacheOther( "hunter_flechette" );
...
{
	if ( enginerandom->RandomInt(0, 99) == 0 && !HasSpawnFlags( SF_NPC_GAG ) )
		return true;
...
void CNPC_Hunter::Activate()
{
	...
	CMissile::AddCustomDetonator( BaseEntity(), ( GetHullMaxs().AsVector2D() - GetHullMins().AsVector2D() ).Length() * 0.5, GetHullHeight() );
...
{
	CMissile::RemoveCustomDetonator( BaseEntity() );
...
{ 
	...
	if ( bNoisy )
	{
		float rand1 = enginerandom->RandomFloat( 0, gm_flHeadRadius ) + enginerandom->RandomFloat( 0, gm_flHeadRadius );
...
void CNPC_Hunter::UpdateEyes()
{
	...
	if ( m_EyeSwitchTimer.Expired() )
	{
		...
		m_EyeSwitchTimer.Set( enginerandom->RandomFloat( 1.0f, 3.0f ) );
	}
...
void CNPC_Hunter::PrescheduleThink()
{
	...
	if ( m_bFlashlightInEyes )
	{
		...
		{
 			CPlayer *pPlayer = UTIL_GetNearestPlayer(GetAbsOrigin());
 			if ( pPlayer && !pPlayer->IsIlluminatedByFlashlight( BaseEntity(), NULL ) || !PlayerFlashlightOnMyEyes( pPlayer ) )
			{
...
void CNPC_Hunter::GatherChargeConditions()
{
	...
	if ( m_EscortBehavior.GetEscortTarget() && GetEnemy()->MyCombatCharacterPointer() && !GetEnemy()->MyCombatCharacterPointer()->FInViewCone_Entity( BaseEntity() ) )
		return;
		...
		if ( ( pVehicle ) && ( GlobalEntity_GetCounter( s_iszHuntersToRunOver ) <= 0 ) )
		{
			...
			if ( pVehicle->GetSmoothedVelocity().LengthSqr() > Square( 200 ) )
			{
				...
				if ( DotProduct2D( vDirMovement, vDirToHunter ) > hunter_dodge_warning_cone.GetFloat() && 
					 ...
					 t > 0.0 && t < 1.0 )
				{
					if ( fabs( predictTime - hunter_dodge_warning.GetFloat() ) < .05 || enginerandom->RandomInt( 0, 3 ) )
					{
...
{
	CEntity *pTarget = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_iszSiegeTargetName );
	...
	while( pTarget != NULL )
	{
		...
		{
			CFakeHandle hndl;
			hndl.Set(pTarget->BaseEntity());
			m_pSiegeTargets.AddToTail(hndl);
		}

		pTarget = g_helpfunc.FindEntityByName( pTarget, m_iszSiegeTargetName );
	};
...
void CNPC_Hunter::ManageSiegeTargets()
{
	...
	if( bCreateSiegeTarget )
	{
		...
		CEntity *pSiegeTargetLocation = NULL;
		int iTraces = 0;
		for( int i = 0 ; i < m_pSiegeTargets.Count() ; i++ )
		{
			CEntity *pCandidate = m_pSiegeTargets[i];
			if ( pCandidate == NULL )
				continue;
			...
			if( flDistSqr < flClosestDistSqr )
			{
				...
				if( !pBullseye->FInViewCone_Entity(BaseEntity()) )
					continue;

				if( pPlayer->FVisible_Entity(BaseEntity()) )
				{
		...
		if( pSiegeTargetLocation != NULL )
		{
			...
			CEntity *pSiegeTarget = CEntity::Instance(CreateCustomTarget( pSiegeTargetLocation->GetAbsOrigin(), 20.0f ));
			pSiegeTarget->SetName( "siegetarget" );

			m_hCurrentSiegeTarget.Set( pSiegeTarget->BaseEntity() );

			AddEntityRelationship( pSiegeTarget->BaseEntity(), D_HT, 1 );
			GetEnemies()->UpdateMemory( GetNavigator()->GetNetwork(), pSiegeTarget->BaseEntity(), pSiegeTarget->GetAbsOrigin(), 0.0f, true );
			AI_EnemyInfo_t *pMemory = GetEnemies()->Find( pSiegeTarget->BaseEntity() );
...
void CNPC_Hunter::KillCurrentSiegeTarget()
{
	...
	{
		GetEnemies()->ClearMemory( m_hCurrentSiegeTarget->BaseEntity() );
...
void CNPC_Hunter::GatherIndoorOutdoorConditions()
{
	...
	UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() + Vector( 0, 0, 40.0f * 12.0f ), MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	if( tr.fraction < 1.0f )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
static bool IsMovablePhysicsObject( CEntity *pEntity )
{
...
bool CNPC_Hunter::ShouldCharge( const Vector &startPos, const Vector &endPos, bool useTime, bool bCheckForCancel )
{
	...
	GetMoveProbe()->MoveLimit( NAV_GROUND, startPos, vecTargetPos, MASK_NPCSOLID_BRUSHONLY, GetEnemy_CBase(), &moveTrace );
	...
	if ( IsMoveBlocked( moveTrace ) )
	{
		...
		if ( moveTrace.pObstruction != NULL )
		{
			...
			CEntity *pObstruction = CEntity::Instance(moveTrace.pObstruction);
			// Hit things that will take damage
			if ( pObstruction->m_takedamage != DAMAGE_NO )
				return true;

			// Hit things that will move
			if ( pObstruction->GetMoveType() == MOVETYPE_VPHYSICS )
				return true;
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::HandleInteraction(int interactionType, void *data, CBaseEntity *pSourceEnt)
{
	if ( ( pSourceEnt != BaseEntity() ) && ( interactionType == g_interactionHunterFoundEnemy ) )
	{
...
int CNPC_Hunter::SelectCombatSchedule()
{
	...
	{
		AI_EnemyInfo_t *pEnemyInfo = GetEnemies()->Find( pEnemy->BaseEntity() );
	...
	{
		if ( hunter_charge_test.GetBool() || enginerandom->RandomInt( 1, 100 ) < hunter_charge_pct.GetInt() )
		{
...
/*
	if ( gpGlobals->curtime > m_flNextSideStepTime )
	{
		...
		{
			m_flNextSideStepTime = gpGlobals->curtime + enginerandom->RandomFloat( 1.0f, 3.0f );
		...
		if ( m_RundownDelay.Expired() )
		{
			...
			if ( iRundownCounter % 2 == 0 )
			{
				...
				if ( HasCondition( COND_SEE_ENEMY ) )
				{
					...
					else if ( enginerandom->RandomInt( 0, 1 ) )
					{
						return SCHED_HUNTER_CHARGE_ENEMY;
					}
					else
...
void CNPC_Hunter::TaskFindDodgeActivity()
{
	...
	else if ( enginerandom->RandomInt( 0, 1 ) == 0 )
	{
		bDodgeLeft = true;
	}
	...
	while ( !bFoundDir && ( nTries < 2 ) )
	{
		...
		trace_t tr;
		HunterTraceHull_SkipPhysics( GetAbsOrigin(), GetAbsOrigin() + vecDelta, testHullMins, GetHullMaxs(), MASK_NPCSOLID, BaseEntity(), GetCollisionGroup(), &tr, VPhysicsGetObject()->GetMass() * 0.5f );
	...
	switch ( pTask->iTask )
	{
		case TASK_HUNTER_FINISH_RANGE_ATTACK:
		{
			if( GetEnemy() != NULL && GetEnemy()->Classify() == CLASS_PLAYER_ALLY_VITAL )
			{
				...
				GetEnemies()->SetTimeValidEnemy( GetEnemy_CBase(), gpGlobals->curtime + 10.0f );
			}
		...
		case TASK_HUNTER_BEGIN_FLANK:
		{
			if ( IsInSquad() && GetSquad()->NumMembers() > 1 )
			{
				...
				AISquadIter_t iter;
				CAI_NPC *pNPC = (CAI_NPC *)CEntity::Instance(GetSquad()->GetFirstMember( &iter ));
				while ( pNPC == this )
				{
					pNPC = (CAI_NPC *)CEntity::Instance(GetSquad()->GetNextMember( &iter ));
				}
		...
		case TASK_RANGE_ATTACK2:
		{
			if ( GetEnemy() )
			{
				...
				if ( bIsBuster )
				{
					AddFacingTarget_E_V_F_F_F( GetEnemy_CBase(), GetEnemy()->GetAbsOrigin() + GetEnemy()->GetSmoothedVelocity() * .5, 1.0, 0.8 );
				}
		...
		case TASK_HUNTER_FIND_SIDESTEP_POSITION:
		{
			...
			else
			{
				...
				Vector vecPos = GetAbsOrigin() + Vector( 0, 0, 64 ) + enginerandom->RandomFloat( 120, 200 ) * vecDir;
...
void CNPC_Hunter::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_RANGE_ATTACK2:
		{
			...
			{
				if( !IsValidEnemy(GetEnemy_CBase()) )
				{
			...
			if ( bIsBuster )
			{
				...
				AddFacingTarget_E_V_F_F_F( GetEnemy_CBase(), vFuturePosition, 1.0, 0.8 );
		...
		case TASK_HUNTER_CHARGE:
		{
			...
			if ( eActivity == ACT_HUNTER_CHARGE_STOP || eActivity == ACT_HUNTER_CHARGE_CRASH ) 
			{
				if ( IsActivityFinished() )
				{
					m_flNextChargeTime = gpGlobals->curtime + hunter_charge_min_delay.GetFloat() + enginerandom->RandomFloat( 0, 2.5 ) + enginerandom->RandomFloat( 0, 2.5 );
					float delayMultiplier = ( g_helpfunc.GameRules_IsSkillLevel( SKILL_EASY ) ) ? 1.5 : 1.0;
					float groupDelay = gpGlobals->curtime +  ( 2.0  + enginerandom->RandomFloat( 0, 2 ) ) * delayMultiplier;
					for ( int i = 0; i < g_Hunters.Count(); i++ )
					{
			...
			if ( AutoMovement( GetEnemy(), &moveTrace ) == false )
			{
				...
				if ( HandleChargeImpact( moveTrace.vEndPosition, CEntity::Instance(moveTrace.pObstruction) ) )
				{
				...
				else if ( moveTrace.pObstruction )
				{
					...
					if ( pObstruction->Classify() == CLASS_COMBINE_HUNTER )
					{
						// Crash unless we're trying to stop already
						if ( eActivity != ACT_HUNTER_CHARGE_STOP )
						{
					...
					else if (pObstruction->Classify() == CLASS_ANTLION )
					{
						if ( FClassnameIs( pObstruction, "npc_antlionguard" ) )
						{
							// Crash unless we're trying to stop already
							if ( eActivity != ACT_HUNTER_CHARGE_STOP )
							{
						...
						{
							Hunter_ApplyChargeDamage( this, pObstruction, pObstruction->GetHealth() );
						}
					}
				}
			}
		...
		case TASK_HUNTER_WAIT_FOR_MOVEMENT_FACING_ENEMY:
		{
			...
			{
				Vector vecEnemyLKP = GetEnemyLKP();
				AddFacingTarget_E_V_F_F_F( GetEnemy_CBase(), vecEnemyLKP, 1.0, 0.8 );
			}
...
//-----------------------------------------------------------------------------
bool CNPC_Hunter::EnemyIsRightInFrontOfMe( CEntity **pEntity )
{
	if ( !GetEnemy() )
		return false;
	...
	if ( (GetEnemy()->WorldSpaceCenter() - WorldSpaceCenter()).LengthSqr() < (156*156) )
	{
		...
		if ( DotProduct( vecLOS, vBodyDir ) > 0.8 )
		{
			...
			trace_t tr;
			UTIL_TraceHull( WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(), GetHullMins() * 0.5, GetHullMaxs() * 0.5, MASK_SOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
float CNPC_Hunter::ChargeSteer()
{
	...
	// Probe
	HunterTraceHull_SkipPhysics( GetAbsOrigin(), testPos, testHullMins, GetHullMaxs(), MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr, VPhysicsGetObject()->GetMass() * 0.5f );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hunter::ChargeDamage( CEntity *pTarget )
{
	if ( pTarget == NULL )
		return;
...
//-----------------------------------------------------------------------------
bool CNPC_Hunter::HandleChargeImpact( Vector vecImpact, CEntity *pEntity )
{
	...
	if ( IRelationType( pEntity->BaseEntity() ) == D_HT && ( GetNextAttack() < gpGlobals->curtime ) )
	{
	...
	{		
		CE_CBreakable *pBreakable = dynamic_cast<CE_CBreakable *>(pEntity);
		...
		if ( pPhysics )
		{
			...
			if ( entMass < minMass )
			{
				if ( entMass < minMass * 0.666f || pEntity->CollisionProp()->BoundingRadius() < GetHullHeight() )
				{
					...
					{
						CE_CBreakableProp *pBreakable = dynamic_cast<CE_CBreakableProp *>(pEntity);
...
void CNPC_Hunter::Explode()
{
	...
	PropBreakableCreateAll( GetModelIndex(), NULL, EyePosition(), GetAbsAngles(), velocity, angVelocity, 1.0, 150, COLLISION_GROUP_NPC, BaseEntity() );

	ExplosionCreate( EyePosition(), GetAbsAngles(), BaseEntity(), 500, 256, (SF_ENVEXPLOSION_NOPARTICLES|SF_ENVEXPLOSION_NOSPARKS|SF_ENVEXPLOSION_NODLIGHTS|SF_ENVEXPLOSION_NODAMAGE|SF_ENVEXPLOSION_NOSMOKE), false );
...
void CNPC_Hunter::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_HUNTER_SPRAY_BLOOD )
	{
		...
		for ( int i = 0 ; i < 3 ; i++ )
		{
			...
			vecTraceDir.x += enginerandom->RandomFloat( -0.1, 0.1 );
			vecTraceDir.y += enginerandom->RandomFloat( -0.1, 0.1 );
			vecTraceDir.z += enginerandom->RandomFloat( -0.1, 0.1 );

			trace_t tr;
			UTIL_TraceLine( vecOrigin, vecOrigin + ( vecTraceDir * 192.0f ), MASK_SOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, &tr );
			if ( tr.fraction != 1.0 )
			{
...
{
	AISquadIter_t iter;
	CBaseEntity *cent = GetSquad() ? GetSquad()->GetFirstMember( &iter ) : NULL;
	CAI_NPC *pSquadmate = (CAI_NPC *)CEntity::Instance(cent);
...
void CNPC_Hunter::FollowStrider( const char *szStrider )
{
	...
	CEntity *pEnt = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, szStrider, BaseEntity() );
...
void CNPC_Hunter::FollowStrider( CNPC_Strider * pStrider )
{
	...
	if ( pStrider )
	{
		...
		{
			m_iszFollowTarget = pStrider->GetEntityName_String();
			if ( m_iszFollowTarget == NULL_STRING )
			{
...
void CAI_HunterEscortBehavior::SetEscortTarget( CNPC_Strider *pStrider, bool fFinishCurSchedule )
{
	...
	if ( !GetOuter()->GetSquad() )
	{
		GetOuter()->AddToSquad( AllocPooledString( CFmtStr( "%s_hunter_squad",  pStrider->GetEntityName() ) ) );
	}
...
void CNPC_Hunter::InputUseSiegeTargets( inputdata_t &inputdata )
{
	...
	m_flTimeNextSiegeTargetAttack = gpGlobals->curtime + enginerandom->RandomFloat( 1, hunter_siege_frequency.GetFloat() );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::FVisible_Entity( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
...
bool CNPC_Hunter::IsValidEnemy( CBaseEntity *pTarget )
{
	...
	if ( IsStriderBuster( cent_pTarget) )
	{
		...
		if ( cent_pTarget->VPhysicsGetObject() )
		{
			if ( ( cent_pTarget->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD ) &&
				hunter_hate_held_striderbusters.GetBool() )
			{
				if ( gpGlobals->curtime - StriderBuster_GetPickupTime( cent_pTarget ) > hunter_hate_held_striderbusters_delay.GetFloat())
				{
					if ( StriderBuster_NumFlechettesAttached( cent_pTarget ) <= 2 )
					{
						if ( m_EscortBehavior.GetEscortTarget() && 
							( m_EscortBehavior.GetEscortTarget()->GetAbsOrigin().AsVector2D() - cent_pTarget->GetAbsOrigin().AsVector2D() ).LengthSqr() < Square( hunter_hate_held_striderbusters_tolerance.GetFloat() ) )
						{
							return true;
						}
					}
			...
			bool bThrown = ( cent_pTarget->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_WAS_THROWN ) != 0;
			bool bAttached = StriderBuster_IsAttachedStriderBuster( cent_pTarget );
			...
			if ( ( bThrown && !bAttached ) && hunter_hate_thrown_striderbusters.GetBool() )
			{
				...
				float dist = CalcDistanceSqrToLineSegment2D( m_EscortBehavior.GetEscortTarget()->GetAbsOrigin().AsVector2D(), 
					cent_pTarget->GetAbsOrigin().AsVector2D(), 
					cent_pTarget->GetAbsOrigin().AsVector2D() + cent_pTarget->GetSmoothedVelocity().AsVector2D(), &t );
			...
			if ( bAttached && StriderBuster_IsAttachedStriderBuster( cent_pTarget, m_EscortBehavior.GetEscortTarget() ) && hunter_hate_attached_striderbusters.GetBool() )
			{
				return true;
			}
		}
...
Disposition_t CNPC_Hunter::IRelationType( CBaseEntity *pTarget )
{
	...
	if ( IsStriderBuster( cent_pTarget ) )
	{
		if ( HateThisStriderBuster( cent_pTarget ) )
			return D_HT;
	...
	{
		if ( cent_pTarget->IsPlayer() && (m_hAttachedBusters.Count() > 0) )
		{
			return D_FR;
		}
	}
...
void CNPC_Hunter::OnSeeEntity( CBaseEntity *pEntity )
{
	...
	if ( IsStriderBuster( cent_pEntity ) && IsValidEnemy( pEntity ) )
	{
...
bool CNPC_Hunter::CanPlantHere( const Vector &vecPos )
{
	...
	trace_t tr;
	UTIL_TraceHull( vecPos, vecPos, vecMins, vecMaxs, MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	if ( tr.startsolid )
	{
		...
		UTIL_TraceHull( vecStart, vecPos, vecMins, vecMaxs, MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Hunter::MeleeAttack1ConditionsVsEnemyInVehicle( CCombatCharacter *pEnemy, float flDot )
{
	...
	VectorNormalize( vecDelta );
	trace_t	tr;
	UTIL_TraceHull( WorldSpaceCenter(), WorldSpaceCenter() + (vecDelta * 64), -Vector(8,8,8), Vector(8,8,8), MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
int CNPC_Hunter::MeleeAttack1Conditions ( float flDot, float flDist )
{
	...
	trace_t	tr;
	UTIL_TraceHull( WorldSpaceCenter(), WorldSpaceCenter() + forward * HUNTER_MELEE_REACH, vecMins, vecMaxs, MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	...
	if ( cent == GetEnemy() || cent->IsNPC() || (cent->m_takedamage == DAMAGE_YES && (dynamic_cast<CE_CBreakableProp*>(cent))) )
	{
	...
	if ( !cent->IsWorld() && GetEnemy() && GetEnemy()->GetGroundEntity() == cent )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::IsCorporealEnemy( CEntity *pEnemy ) 
{
	if( !pEnemy )
		return false;
...
{
	CEntity *pTargetEnt = GetEnemy();

	trace_t tr;
	Vector vFrom = ownerPos + GetViewOffset();
	UTIL_TraceLine( vFrom, targetPos, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );

	CEntity *cent = CEntity::Instance(tr.m_pEnt);

	if ( ( pTargetEnt && cent == pTargetEnt) || tr.fraction == 1.0 || CanShootThrough( tr, targetPos ) )
	{
		...
		UTIL_TraceHull( vFrom - Vector( 0, 0, 18 ), targetPos, vMins, vMaxs, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr);
...
//-----------------------------------------------------------------------------
CEntity *CNPC_Hunter::MeleeAttack( float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch, int BloodOrigin  )
{
	...
	CEntity *pHurt = CEntity::Instance(CheckTraceHullAttack_Float( flDist, vecMins, vecMaxs, iDamage, DMG_SLASH ));
			...
			if ( IsMovablePhysicsObject( pHurt ) )
			{
				...
				if ( pPhysics )
				{
					...
					if ( pPhysics->GetGameFlags() & FVPHYSICS_PLAYER_HELD )
					{
						...
						{
							CE_CBreakableProp *pBreak = dynamic_cast<CE_CBreakableProp*>(pHurt);
							if ( pBreak )
							{
								CTakeDamageInfo info( BaseEntity(), BaseEntity(), 20, DMG_SLASH );
								pBreak->Break( BaseEntity(), info );
							}
						}
					}
				}
...
bool CNPC_Hunter::CanShootThrough( const trace_t &tr, const Vector &vecTarget )
{
	...
	CAI_NPC *pNPC = cent->MyNPCPointer();
	if ( pNPC && ( IRelationType( pNPC->BaseEntity() ) == D_LI ) )
	{
		return false;
	}
...
void CNPC_Hunter::PainSound( const CTakeDamageInfo &info )
{
	if ( gpGlobals->curtime > m_flNextDamageTime )
	{
		...
		m_flNextDamageTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.5, 1.2 ); 
	}
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hunter::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
...
void CNPC_Hunter::TeslaThink_CBE()
{
	CNPC_Hunter *cent = (CNPC_Hunter *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->TeslaThink();
	}
...
void CNPC_Hunter::TeslaThink()
{
	...
	{
		SetContextThink( &CNPC_Hunter::TeslaThink_CBE, gpGlobals->curtime + enginerandom->RandomFloat( 0.1f, 0.3f ), HUNTER_ZAP_THINK ); 
	}
}



void CNPC_Hunter::BleedThink_CBE()
{
	CNPC_Hunter *cent = (CNPC_Hunter *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
...
{
	// Spurt blood from enginerandom points on the hunter's head.
	Vector vecOrigin;
	QAngle angDir;
	...
	SetNextThink( gpGlobals->curtime + enginerandom->RandomFloat( 0.6, 1.5 ), HUNTER_BLEED_THINK );
}
...
void CNPC_Hunter::ConsiderFlinching( const CTakeDamageInfo &info )
{
	if ( !m_FlinchTimer.Expired() )
	{
		...
		m_FlinchTimer.Set( enginerandom->RandomFloat( 0.3 ) );
		return;
	}
	...
	if ( !IsPlayingGesture( eGesture ) )
	{
		...
		m_FlinchTimer.Set( enginerandom->RandomFloat( 0.3, 1.0 ) );
	}
}



void CNPC_Hunter::JostleVehicleThink_CBE()
{
	CNPC_Hunter *cent = (CNPC_Hunter *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->JostleVehicleThink();
	}
}
...
{
	CEntity *pInflictor = m_hHitByVehicle;
	if ( !pInflictor )
		return;
	...
	if ( ( info.GetDamageType() & DMG_CRUSH ) && !( info.GetDamageType() & DMG_VEHICLE ) )
	{
		...
		else
		{
			...
			for (int i = 0; i < children.Count(); i++ )
			{
				...
				{
					myInfo.SetInflictor( pent->BaseEntity() );
...
int CNPC_Hunter::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( pInflictor )
	{
		if ( IsStriderBuster( pInflictor ) )
		{
			...
			SetContextThink( &CNPC_Hunter::TeslaThink_CBE, gpGlobals->curtime, HUNTER_ZAP_THINK ); 
		...
		else
		{
			...
			if ( ( info.GetDamageType() & DMG_VEHICLE ) || 
				 ( pInflictor && pInflictor->GetServerVehicle() && 
				   ( ( bHitByUnoccupiedCar = ( dynamic_cast<CE_CPropVehicleDriveable *>(pInflictor) && static_cast<CE_CPropVehicleDriveable *>(pInflictor)->GetDriver() == NULL ) )  == false ) ) )
			{
				...
				if ( vecVelDir.Length() >= hunter_jostle_car_min_speed.GetFloat() )
				{
					...
					m_hHitByVehicle.Set(pInflictor->BaseEntity());
					SetContextThink( &CNPC_Hunter::JostleVehicleThink_CBE, gpGlobals->curtime, HUNTER_JOSTLE_VEHICLE_THINK );
				}
			}
	...
	if ( flScale != 0 )
	{
		...
		if( pAttacker && pAttacker->IsPlayer() )
		{
			...
			if( !GetEnemy() || !GetEnemy()->IsPlayer() )
			{
				UpdateEnemyMemory( pAttacker->BaseEntity(), pAttacker->GetAbsOrigin(), BaseEntity() );

				if( GetEnemy() )
				{
					// Gotta forget about this person for a little bit.
					GetEnemies()->SetTimeValidEnemy( GetEnemy_CBase(), gpGlobals->curtime + HUNTER_IGNORE_ENEMY_TIME );
				}
			}
		}
	...
	pAttacker = CEntity::Instance(info.GetAttacker());
	if ( IsUsingSiegeTargets() && pAttacker != NULL && pAttacker->IsPlayer() )
	{
...
void CNPC_Hunter::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( m_EscortBehavior.GetFollowTarget() )
	{
		if ( AIGetNumFollowers( m_EscortBehavior.GetFollowTarget(), m_iClassname ) == 1 )
		{
			...
			if ( pAttacker && pAttacker->IsPlayer() )
			{
				m_EscortBehavior.GetEscortTarget()->UpdateEnemyMemory( pAttacker->BaseEntity(), pAttacker->GetAbsOrigin(), BaseEntity() );
			}
		}
	}
...
void CNPC_Hunter::StartBleeding()
{
	...
	SetContextThink( &CNPC_Hunter::BleedThink_CBE, gpGlobals->curtime + 0.1, HUNTER_BLEED_THINK );
}
...
bool CNPC_Hunter::IsJumpLegal(const Vector &startPos, const Vector &apex, const Vector &endPos) const
{
	...
	trace_t tr;	
	UTIL_TraceHull( startPos, startPos, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	if ( tr.startsolid )
	{
...
void CNPC_Hunter::DoMuzzleFlash( int nAttachment )
{
	...
	DispatchParticleEffect( "muzzle_pistols", PATTACH_POINT_FOLLOW, this, nAttachment );

	// Dispatch the elight	
	CEffectData data;
	data.m_nAttachmentIndex = nAttachment;
	data.m_nEntIndex = entindex();
	g_helpfunc.DispatchEffect( "HunterMuzzleFlash", data );
}
...
{
	float delayMultiplier = ( g_helpfunc.GameRules_IsSkillLevel( SKILL_EASY ) ) ? 1.25 : 1.0;
	if ( !m_bEnableSquadShootDelay && !bForced )
	{
		m_flNextRangeAttack2Time = gpGlobals->curtime + enginerandom->RandomFloat( minDelay, maxDelay ) * delayMultiplier;
		return;
	}
	...
	for ( int i = 0; i < g_Hunters.Count(); i++ )
	{
		...
		if ( pOtherHunter->GetEnemy() == pEnemy )
		{
			float nextTime = gpGlobals->curtime + enginerandom->RandomFloat( minDelay, maxDelay ) * delayMultiplier;
...
//-----------------------------------------------------------------------------
void CNPC_Hunter::GetShootDir( Vector &vecDir, const Vector &vecSrc, CEntity *pTargetEntity, bool bStriderBuster, int nShotNum, bool bSingleShot )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::ShouldSeekTarget( CEntity *pTargetEntity, bool bStriderBuster )
{
	bool bSeek = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::ShootFlechette( CEntity *pTargetEntity, bool bSingleShot )
{
	if ( !pTargetEntity )
	{
		Assert( false );
	...
	CHunterFlechette *pFlechette = CHunterFlechette::FlechetteCreate( vecSrc, angShoot, BaseEntity() );
	...
	if( nShotNum == 1 && pTargetEntity->Classify() == CLASS_PLAYER_ALLY_VITAL )
	{
		...
		g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER|SOUND_CONTEXT_REACT_TO_SOURCE|SOUND_CONTEXT_EXCLUDE_COMBINE, pTargetEntity->EyePosition(), 180, 2.0f, BaseEntity() );
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CEntity *CNPC_Hunter::GetEnemyVehicle()
{
	if ( GetEnemy() == NULL )
		return false;
	...
	CCombatCharacter *pCCEnemy = GetEnemy()->MyCombatCharacterPointer();
	if ( pCCEnemy != NULL )
		return CEntity::Instance(pCCEnemy->GetVehicleEntity());
...
//-----------------------------------------------------------------------------
void CNPC_Hunter::PlayerHasIlluminatedNPC( CBaseEntity *pPlayer, float flDot )
{
	if ( m_bFlashlightInEyes )
		return;

	CPlayer *player = (CPlayer *)CEntity::Instance(pPlayer);
	// Ignore the flashlight if it's not shining at my eyes
	if ( PlayerFlashlightOnMyEyes( player ) )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::PlayerFlashlightOnMyEyes( CPlayer *pPlayer )
{
...
//-----------------------------------------------------------------------------
// Return a enginerandom expression for the specified state to play over 
...
const char *CNPC_Hunter::SelectRandomExpressionForState( NPC_STATE state )
{
	...
	int nIndex = enginerandom->RandomInt( 0, 3 );
...
void CNPC_Hunter::StriderBusterAttached( CBaseEntity *pAttached )
{
	...
	CFakeHandle hndl;
	hndl.Set(pAttached);
	m_hAttachedBusters.AddToTail( hndl );
...
void CNPC_Hunter::StriderBusterDetached( CBaseEntity *pAttached )
{
	CFakeHandle hndl;
	hndl.Set(pAttached);
	int elem = m_hAttachedBusters.Find(hndl);
	if (elem >= 0)
	{
...
{
	CEntity *pAttacker = CEntity::Instance(info.GetAttacker());
	if ( info.GetDamage() > 0 && pAttacker->IsPlayer() &&
		...
		( GetOuter()->GetSquad()->GetSquadSoundWaitTime() <= gpGlobals->curtime ) ) // && !FarFromFollowTarget()
	{
		...
		m_flTimeEscortReturn = gpGlobals->curtime + enginerandom->RandomFloat( 15.0f, 25.0f );
...
void CAI_HunterEscortBehavior::CheckBreakEscort()
{
	if ( m_flTimeEscortReturn != 0 && ( FarFromFollowTarget() || gpGlobals->curtime >= m_flTimeEscortReturn ) )
	{
		...
		if ( GetOuter()->GetSquad() )
		{
			GetOuter()->GetSquad()->SetSquadSoundWaitTime( gpGlobals->curtime + enginerandom->RandomFloat( 5.0f, 12.0f ) );
		}
	}
}
...
void CAI_HunterEscortBehavior::GatherConditions( void )
{
	...
	{
		if ( GetOuter()->GetSquad()->GetSquadSoundWaitTime() <= gpGlobals->curtime && ((CPlayer *)GetEnemy())->IsInAVehicle() )
		{
			m_flTimeEscortReturn = gpGlobals->curtime + enginerandom->RandomFloat( 15.0f, 25.0f );
			GetOuter()->GetSquad()->SetSquadSoundWaitTime( m_flTimeEscortReturn + 1.0 ); // prevent others from breaking escort
		}
	...
	case TASK_MOVE_TO_FOLLOW_POSITION:
		{
			if ( GetEnemy() )
			{
				if ( GetOuter()->OccupyStrategySlot( SQUAD_SLOT_RUN_SHOOT ) )
				{
					...
					{
						GetOuter()->BeginVolley( NUM_FLECHETTE_VOLLEY_ON_FOLLOW, gpGlobals->curtime + 1.0 + enginerandom->RandomFloat( 0, .25 ) + enginerandom->RandomFloat( 0, .25 ) );
					}
					else
					{
	...
	case TASK_MOVE_TO_FOLLOW_POSITION:
		{
			...
			else
			{
				if ( GetEnemy() )
				{
					...
					pHunter->AddFacingTarget_E_V_F_F_F( pHunter->GetEnemy_CBase(), vecEnemyLKP, 1.0, 0.8 );
					bool bVacate = false;
							...
							if ( gpGlobals->curtime >= pHunter->m_flNextFlechetteTime )
							{
								...
								if ( bHasSlot )
								{
									...
									else
									{
										...
										bVacate = true;
										pHunter->BeginVolley( NUM_FLECHETTE_VOLLEY_ON_FOLLOW, gpGlobals->curtime + 1.0 + enginerandom->RandomFloat( 0, .25 ) + enginerandom->RandomFloat( 0, .25 ) );
									}
								}
							}
						}
...
void CAI_HunterEscortBehavior::DistributeFreeHunters()
{
	...
	CAI_NPC **ppNPCs = g_AI_Manager.AccessAIs();
	...
	for ( int i = 0; i < nAIs; i++ )
	{
		...
		if ( pNPC && ( pNPC->Classify() == CLASS_COMBINE_HUNTER ) && pNPC->m_lifeState == LIFE_ALIVE )
		{
			if ( !pNPC->GetEnemy() || !IsStriderBuster( pNPC->GetEnemy() ) )
			{
				...
				{
					pNPC->SetEnemy( pBuster->BaseEntity() );
					pNPC->SetState( NPC_STATE_COMBAT );
					pNPC->UpdateEnemyMemory( pBuster->BaseEntity(), pBuster->GetAbsOrigin() );
...
void Hunter_StriderBusterAttached( CEntity *pHunter, CEntity *pAttached )
{
	...
	static_cast<CNPC_Hunter *>(pHunter)->StriderBusterAttached((pAttached)?pAttached->BaseEntity():NULL);
}
...
void Hunter_StriderBusterDetached( CEntity *pHunter, CEntity *pAttached )
{
	...
	static_cast<CNPC_Hunter *>(pHunter)->StriderBusterDetached((pAttached)?pAttached->BaseEntity():NULL);
}
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_hunter_maker, info_target, CHunterMaker );
