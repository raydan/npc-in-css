...
{
public:
	CE_DECLARE_CLASS( CNPC_Monk, CAI_PlayerAlly );
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_monk, cycler, CNPC_Monk );
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
WeaponProficiency_t CNPC_Monk::CalcWeaponProficiency( CBaseEntity *pWeapon )
{
...
Vector CNPC_Monk::GetActualShootTrajectory( const Vector &shootOrigin )
{
	if( GetEnemy() && GetEnemy()->Classify() == CLASS_ZOMBIE )
	{
		...
		if( m_bPerfectAccuracy || enginerandom->RandomInt( 1, monk_headshot_freq.GetInt() ) == 1 )
		{
...
bool CNPC_Monk::IsValidEnemy( CBaseEntity *pEnemy )
{
	if ( BaseClass::IsValidEnemy( pEnemy ) && GetActiveWeapon() )
	{
		...
		flDist = ( GetAbsOrigin() - cent->GetAbsOrigin() ).Length();
	...
	case TASK_RELOAD:
		{
			...
			if( m_iNumZombies >= 2 && enginerandom->RandomInt( 1, 3 ) == 1 )
			{
...
void CNPC_Monk::GatherConditions()
{
	...
	while( pSightEnt )
	{
		if( pSightEnt->Classify() == CLASS_ZOMBIE && pSightEnt->IsAlive() )
		{
			...
			CAI_NPC *pZombie = dynamic_cast<CAI_NPC*>(pSightEnt);
...
bool CNPC_Monk::PassesDamageFilter( const CTakeDamageInfo &info )
{
	...
	if ( attacker->ClassMatches( "npc_headcrab_black" ) || attacker->ClassMatches( "npc_headcrab_poison" ) )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Monk::OnKilledNPC( CBaseEntity *pKilled )
{
	...
	if ( cent_pKilled->Classify() == CLASS_ZOMBIE )
	{
		...
		if( m_iNumZombies == 1 || enginerandom->RandomInt( 1, 3 ) == 1 )
		{
