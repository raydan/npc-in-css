...
{
public:	
	CE_DECLARE_CLASS( CNPC_Dog, CAI_BaseActor );
	DECLARE_DATADESC();
	...
	bool FindPhysicsObject( const char *pPickupName, CEntity *pIgnore = NULL );
...
protected:
	...
	CEFakeHandle<CE_CSprite> m_hGlowSprites[EFFECT_COUNT];
	CEFakeHandle<CE_CBeam>  m_hBeams[EFFECT_COUNT]; //This is temp.
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_dog, cycler, CNPC_Dog );
...
Disposition_t CNPC_Dog::IRelationType( CBaseEntity *pTarget )
{
	...
	if ( NPC_Rollermine_IsRollermine( cent ) )
	{
		if ( cent->HasSpawnFlags( SF_ROLLERMINE_FRIENDLY ) )
			 return D_LI;
	}
	...
	if (m_bPerformAvoidance || ( ShouldPlayerAvoid() && bIsMoving ) )
	{
		...
		if ( pLocalPlayer )
		{
			...
			for ( i = 0; i < m_BoneFollowerManager.GetNumBoneFollowers(); i++ )
			{
				...
				if ( pBone && pBone->hFollower )
				{
					pBone->hFollower->CollisionProp_Actual()->WorldSpaceSurroundingBounds( &vMins, &vMaxs );
...
void CNPC_Dog::PullObject( bool bMantain )
{
	...
	if ( bMantain == false )
	{
		...
		{
			m_hPhysicsEnt->SetOwnerEntity( BaseEntity() );
...
void CNPC_Dog::InputPlayerPickupObject ( inputdata_t &inputdata )
{
	if ( m_bDoWaitforObjectBehavior == true )
	{
		if ( m_hPhysicsEnt != inputdata.pCaller )
		{
			...
			m_hPhysicsEnt.Set(inputdata.pCaller);
		}
	}
...
void CNPC_Dog::InputSetThrowTarget( inputdata_t &inputdata )
{
	...
	m_hThrowTarget.Set((cent)?cent->BaseEntity():NULL);
}
	...
	if ( m_hPhysicsEnt )
	{
		...
		if ( pPhysObj )
		{
			...
			if ( m_hThrowTarget == NULL ) {
				...
				m_hThrowTarget.Set((player)?player->BaseEntity():NULL);
			}
			...
			if ( m_hThrowTarget )
			{
				...
				if ( m_hThrowTarget->IsPlayer() )
					 vThrowOrigin = vThrowOrigin + Vector( enginerandom->RandomFloat( -128, 128 ), enginerandom->RandomFloat( -128, 128 ), 0 );
			...
			if ( m_bBeamEffects == true )
			{
				...
				CE_CBeam *pBeam = CE_CBeam::BeamCreate(  "sprites/orangelight1.vmt", 1.8 );
...
{
	CEntity *cent_m_hPhysicsEnt = m_hPhysicsEnt;
	if ( cent_m_hPhysicsEnt )
	{
		InvalidateBoneCache();
		...
		IPhysicsObject *pPhysicsObject = cent_m_hPhysicsEnt->VPhysicsGetObject();
		if ( pPhysicsObject )
		{
		...
		m_iContainerMoveType = cent_m_hPhysicsEnt->GetMoveType();
		cent_m_hPhysicsEnt->SetMoveType( MOVETYPE_NONE );
		
		cent_m_hPhysicsEnt->SetParent( BaseEntity(), iAttachment );
	
		cent_m_hPhysicsEnt->SetLocalOrigin( vec3_origin );
		cent_m_hPhysicsEnt->SetLocalAngles( vec3_angle );

		cent_m_hPhysicsEnt->SetGroundEntity( NULL );
		

		if ( cent_m_hPhysicsEnt->GetOwnerEntity() == NULL )
			 cent_m_hPhysicsEnt->SetOwnerEntity( BaseEntity() );
...
void CNPC_Dog::CreateSprites( void )
{
	...
	for ( int i = 0; i < EFFECT_COUNT; i++ )
	{
		...
		CE_CSprite *sprite = CE_CSprite::SpriteCreate( "sprites/glow04_noz.vmt", GetAbsOrigin(), false );
		m_hGlowSprites[i].Set(sprite->BaseEntity());
		sprite->SetAttachment( BaseEntity(), LookupAttachment( attachNames[i] ) );
		sprite->SetTransparency( kRenderGlow, 255, 128, 0, 64, kRenderFxNoDissipation );
		sprite->SetBrightness( 255, 0.2f );
		sprite->SetScale( 0.55f, 0.2f );
	}
}
...
void CNPC_Dog::CreateBeams( void )
{
	...
	for ( int i = 0; i < EFFECT_COUNT; i++ )
	{
		...
		CE_CBeam *beam = CE_CBeam::BeamCreate( "sprites/physcannon_bluelight2.vmt", 5.0 );
		m_hBeams[i].Set(beam->BaseEntity());
		beam->EntsInit( m_hPhysicsEnt, this );
		beam->SetEndAttachment( LookupAttachment( attachNames[i] ) );
...
bool CNPC_Dog::FindPhysicsObject( const char *pPickupName, CEntity *pIgnore )
{
	CEntity		*pEnt = NULL;
	...
	{
		pEnt = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, pPickupName );
		
		if ( m_hUnreachableObjects.Find( (pEnt)?pEnt->BaseEntity():NULL ) == -1  )
		{
			m_bHasObject = false;
			m_hPhysicsEnt.Set((pEnt)?pEnt->BaseEntity():NULL);
			return true;
		}
	}
	
	while ( ( pEnt = g_helpfunc.FindEntityByClassname( pEnt, "prop_physics" ) ) != NULL )
	{
		//We don't want this one.
		if ( pEnt == pIgnore )
			 continue;

		if ( m_hUnreachableObjects.Find( pEnt->BaseEntity() ) != -1 )
			 continue;
		...
		if ( FVisible_Entity( pEnt->BaseEntity() ) == false )
			 continue;
...
{
	CEntity *pEntity = m_hThrowTarget;
	...
	if ( pEntity )
	{
		...
		if ( pPlayer )
		{
			if ( m_hPhysicsEnt )
			{
				if ( pPlayer->FVisible_Entity( m_hPhysicsEnt->BaseEntity() ) == false )
					return false;
			}
	...
	case TASK_DOG_GET_PATH_TO_PHYSOBJ:
		{
			...
			vecDir = GetLocalOrigin() - cent_m_hPhysicsEnt->WorldSpaceCenter();
			...
			vecGoalPos = cent_m_hPhysicsEnt->WorldSpaceCenter() + (vecDir * DOG_PHYSOBJ_MOVE_TO_DIST );
	...
	case TASK_DOG_WAIT_FOR_OBJECT:
		{
			...
			if ( cent_m_hPhysicsEnt != NULL )
			{
				if ( FVisible_Entity( cent_m_hPhysicsEnt->BaseEntity() ) == false )
				{
					m_flTimeToCatch = 0.0f;
					ClearBeams();
					TaskFail( "Lost sight of the object!" );
					cent_m_hPhysicsEnt->SetOwnerEntity( NULL );
					return;
				}
				...
				Vector vToObject = cent_m_hPhysicsEnt->WorldSpaceCenter() - vGunPos;
				float flDistance = vToObject.Length();

				VectorNormalize( vToObject );

				SetAim( cent_m_hPhysicsEnt->WorldSpaceCenter() - GetAbsOrigin() );
				...
				if ( pPlayer )
				{
					flDistanceToPlayer = (pPlayer->GetAbsOrigin() - cent_m_hPhysicsEnt->WorldSpaceCenter()).Length();
				}
			
				IPhysicsObject *pPhysObj = cent_m_hPhysicsEnt->VPhysicsGetObject();
				if ( !pPhysObj )
				{
...
void CNPC_Dog::SetupThrowTarget( void )
{
	...
	SetTarget( (m_hThrowTarget)?m_hThrowTarget->BaseEntity():NULL );
}
	...
	case TASK_DOG_GET_PATH_TO_PHYSOBJ:
		{
			...
			{
				if ( m_hUnreachableObjects.Find( cent_m_hPhysicsEnt->BaseEntity() ) == -1 )
					m_hUnreachableObjects.AddToTail( cent_m_hPhysicsEnt->BaseEntity() );
					
				 FindPhysicsObject( NULL, cent_m_hPhysicsEnt );
