...
class CNPC_CeilingTurret : public CE_Cycler_Fix
{
public:
	CE_DECLARE_CLASS( CNPC_CeilingTurret, CE_Cycler_Fix );
...
//-----------------------------------------------------------------------------
bool CNPC_CeilingTurret::FVisible_Entity( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	CBaseEntity	*pHitEntity = NULL;
	if ( BaseClass::FVisible_Entity( pEntity, traceMask, &pHitEntity ) )
		return true;

	CEntity *cent_pHitEntity = CEntity::Instance(pHitEntity);
	// If we hit something that's okay to hit anyway, still fire
	if ( cent_pHitEntity && cent_pHitEntity->MyCombatCharacterPointer() )
	{
...
void CNPC_CeilingTurret::ActiveThink( void )
{
	...
	UpdateEnemyMemory( GetEnemy_CBase(), vecMidEnemy );

	//Look for our current enemy
	bool bEnemyVisible = FInViewCone_Entity( GetEnemy_CBase() ) && FVisible_Entity( GetEnemy_CBase() ) && GetEnemy()->IsAlive();
...
void CNPC_CeilingTurret::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy )
{
	...
	info.m_flDamage = (m_fBulletDamage <= 0.0f) ? sk_npc_turret_ceiling_bullet_damage.GetFloat() : m_fBulletDamage;
...
bool CNPC_CeilingTurret::CanBeAnEnemyOf( CBaseEntity *pEnemy )
{
	...
	if ( m_spawnflags & SF_CEILING_TURRET_OUT_OF_AMMO )
	{
		if ( CEntity::Instance(pEnemy)->Classify() == CLASS_PLAYER_ALLY_VITAL )
			return false;
	} 
