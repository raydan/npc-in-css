...
ConVar sk_npc_turret_ground_bullet_damage("sk_npc_turret_ground_bullet_damage", "5");


// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

LINK_ENTITY_TO_CUSTOM_CLASS( npc_turret_ground, cycler, CNPC_GroundTurret );
...
BEGIN_DATADESC( CNPC_GroundTurret )
	...
	DEFINE_KEYFIELD( m_fBulletDamage, FIELD_FLOAT, "BulletDamage" ),
...
int CNPC_GroundTurret::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if( inflictor != this && inflictor->Classify() != CLASS_MISSILE )
	{
		return 0;
	}
...
void CNPC_GroundTurret::Event_Killed( const CTakeDamageInfo &info )
{
	...
	m_pSmoke = CSmokeTrail::CreateSmokeTrail();
	...
	m_iDeathSparks = enginerandom->RandomInt( 6, 12 );
...
void CNPC_GroundTurret::DeathEffects()
{
	...
	else
	{
		...
		SetNextThink( gpGlobals->curtime + enginerandom->RandomFloat( 0.5, 2.5 ) );
	}
}
...
void CNPC_GroundTurret::MakeTracer( const Vector &vecTracerSrc, const trace_t &tr, int iTracerType )
{
	...
	pBeam->SetColor( 0, 145+enginerandom->RandomInt( -16, 16 ), 255 );
	pBeam->RelinkBeam();
	pBeam->LiveForTime( enginerandom->RandomFloat( 0.2f, 0.5f ) );
#endif
}
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_GroundTurret::FVisible_Entity( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	if ( BaseClass::FVisible_Entity( pEntity, traceMask, ppBlocker ) )
		return true;

	CEntity *cent = CEntity::Instance(pEntity);
	if ( ( cent->GetAbsOrigin().AsVector2D() - GetAbsOrigin().AsVector2D() ).LengthSqr() < Square(10*12) &&
		 FInViewCone_Vector( cent->GetAbsOrigin() ) &&
		BaseClass::FVisible_Vector( cent->GetAbsOrigin() + Vector( 0, 0, 1 ), traceMask, ppBlocker ))
		return true;
	return false;
}
...
bool CNPC_GroundTurret::QuerySeeEntity( CBaseEntity *pEntity, bool bOnlyHateOrFearIfNPC)
{
	...
	CEntity *cent = CEntity::Instance(pEntity);
	flDist = (cent->GetAbsOrigin() - EyePosition()).Length2DSqr();
...
void CNPC_GroundTurret::Shoot()
{
	...
	for( int i = 0 ; i < 1 ; i++ )
	{
		...
		{
			// Subsequent shots or shots at non-players enginerandom
		...
		info.m_flDamage = (m_fBulletDamage <= 0.0f) ? sk_npc_turret_ground_bullet_damage.GetFloat() : m_fBulletDamage;
...
{
	CE_CBeam *pBeam;
	pBeam = CE_CBeam::BeamCreate( GROUNDTURRET_BEAM_SPRITE, width );
	if ( !pBeam )
		return;

	trace_t tr;
	UTIL_TraceLine( vecStart, vecStart + vecDir * m_flSensingDist, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
