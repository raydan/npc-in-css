...
int g_fCombineQuestion = 0;				// true if an idle grunt asked a question. Cleared when someone answers. YUCK old global from grunt code
...
{
	s_iszShotgunClassname = FindPooledString( WEAPON_SHOTGUN_REPLACE_NAME );
	BaseClass::Activate();
}
...
void CNPC_Combine::PostNPCInit()
{
	if( IsElite() )
	{
		...
		// an AR2. 
		if( !GetActiveWeapon() || !FClassnameIs( GetActiveWeapon(), WEAPON_AR2_REPLACE_NAME ) )
		{
...
#define MIN_SIGNAL_DIST	256
	...
	case SCHED_COMBINE_PATROL:
		{
			// If I have an enemy, don't go off into enginerandom patrol mode.
		...
		case COMBINE_AE_GREN_LAUNCH:
			{
				...
				pGrenade->DispatchKeyValue( "velocity", m_vecTossVelocity );
				pGrenade->Spawn();
				

				if ( g_helpfunc.GameRules_IsSkillLevel(SKILL_HARD) )
					m_flNextGrenadeCheck = gpGlobals->curtime + enginerandom->RandomFloat( 2, 5 );// wait a enginerandom amount of time before shooting again
				else
...
WeaponProficiency_t CNPC_Combine::CalcWeaponProficiency( CBaseEntity *pWeapon )
{
	...
	if( FClassnameIs( cent, WEAPON_AR2_REPLACE_NAME ) )
	{
	...
	else if( FClassnameIs( cent, WEAPON_SHOTGUN_REPLACE_NAME )	)
	{
	...
	else if( FClassnameIs( cent, WEAPON_SMG1_REPLACE_NAME ) )
	{
		return WEAPON_PROFICIENCY_GOOD;
	}
