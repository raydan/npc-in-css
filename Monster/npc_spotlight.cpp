...
class CNPC_Spotlight : public CE_Cycler_Fix
{
public:
		CE_DECLARE_CLASS( CNPC_Spotlight, CE_Cycler_Fix );
	...
	public:
		...
		// ------------------------------
		CE_CScriptedTarget*	m_pScriptedTarget;		// My current scripted target
		void				SetScriptedTarget( CE_CScriptedTarget *pScriptedTarget );
		...
		void				SetInspectTargetToEntity(CEntity *pEntity, float fInspectDuration);
		void				SetInspectTargetToEnemy(CEntity *pEntity);
		...
		CE_CBeam*				m_pSpotlight;
		CE_CSpotlightEnd*		m_pSpotlightTarget;
...
LINK_ENTITY_TO_CUSTOM_CLASS(npc_spotlight, cycler, CNPC_Spotlight);
...
int CNPC_Spotlight::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	{
		m_flYaw		+= enginerandom->RandomInt(10,20);
	}
	else
	{
		m_flYaw		-= enginerandom->RandomInt(10,20);
...
void CNPC_Spotlight::Event_Killed( const CTakeDamageInfo &info )
{
	...
	g_helpfunc.CBaseCombatCharacter_Event_Killed(BaseEntity(), info);
}
...
//-----------------------------------------------------------------------------
bool CNPC_Spotlight::FValidateHintType(CBaseEntity *pHint)
{
	CE_AI_Hint *hint = (CE_AI_Hint *)CEntity::Instance(pHint);
	if (hint->HintType() == HINT_WORLD_WINDOW)
	{
		Vector vHintPos;
...
// Output  :
//------------------------------------------------------------------------------
CEntity* CNPC_Spotlight::BestInspectTarget(void)
{
	...
	CEntity*	pBestEntity	= CEntity::Instance(BestEnemy());
	if (pBestEntity)
	{
		// If the enemy isn't visibile
		if (!FVisible_Entity(pBestEntity->BaseEntity()))
		{
			...
			float flTimeLastSeen = GetEnemies()->LastTimeSeen(pBestEntity->BaseEntity());
			if (!GetEnemies()->HasEludedMe(pBestEntity->BaseEntity()) &&
				flTimeLastSeen + 0.5 < gpGlobals->curtime)
			{
				GetEnemies()->MarkAsEluded(pBestEntity->BaseEntity());
				m_pOutputSquadLost.Set(*((EHANDLE *)pBestEntity->BaseEntity()),BaseEntity(),BaseEntity());
			}
			pBestEntity = NULL;
		}

		// If he has eluded me or isn't in the legal range of my spotligth reject
		else if (GetEnemies()->HasEludedMe(pBestEntity->BaseEntity())									||
			!SpotlightIsPositionLegal(GetEnemies()->LastKnownPosition(pBestEntity->BaseEntity()))	)
		{
			pBestEntity = NULL;
		}
	}
	...
	for ( CEntitySphereQuery sphere( vSearchOrigin, SPOTLIGHT_ENTITY_SEARCH_DIST ); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
	{
		if (pEntity->GetFlags() & (FL_CLIENT|FL_NPC))
		{
			...
			CAI_Squad *squard = GetSquad();
			if (squard && squard->SquadIsMember(pEntity))
			{
				continue;
			}
			...
			if (!GetEnemies()->HasMemory(pEntity->BaseEntity())	|| 
				GetEnemies()->HasEludedMe(pEntity->BaseEntity())	)
			{
				m_pOutputSquadDetect.Set(*((EHANDLE *)pEntity),BaseEntity(),BaseEntity());
			}
			UpdateEnemyMemory(pEntity->BaseEntity(),pEntity->GetAbsOrigin());

			CCombatCharacter* pBCC	= (CCombatCharacter*)pEntity;
			float	fTestDistance		= (GetAbsOrigin() - pBCC->EyePosition()).Length();
			int		nTestRelationship	= IRelationType(pBCC->BaseEntity());
			int		nTestPriority		= IRelationPriority ( pBCC->BaseEntity() );
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Spotlight::SetInspectTargetToEntity(CEntity *pEntity, float fInspectDuration)
{
	ClearInspectTarget();
	SetTarget((pEntity)?pEntity->BaseEntity():NULL);
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Spotlight::SetInspectTargetToEnemy(CEntity *pEntity)
{
	ClearInspectTarget();
	SetEnemy( (pEntity)?pEntity->BaseEntity():NULL );
	m_bHadEnemy			= true;
	m_flInspectEndTime	= 0;
	SetState(NPC_STATE_COMBAT);

	EHANDLE hEnemy;
	hEnemy.Set( GetEnemy_CBase() );
	m_pOutputDetect.Set(hEnemy, NULL, BaseEntity());
}
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Spotlight::SetInspectTargetToHint(CE_AI_Hint *pHintNode, float fInspectDuration)
{
	ClearInspectTarget();
	...
	trace_t tr;
	UTIL_TraceLine ( vHintOrigin, vHintEnd, MASK_OPAQUE, BaseEntity(), COLLISION_GROUP_NONE, &tr);
	if (tr.fraction == 1.0)
	{
...
void CNPC_Spotlight::ClearInspectTarget(void)
{
	...
	if (m_bHadEnemy)
	{
		...
		m_pOutputLost.Set(hEnemy,BaseEntity(),BaseEntity());
	}
	...
	m_flYawDir				= enginerandom->RandomInt(0,1) ? 1 : -1;
	m_flPitchDir			= enginerandom->RandomInt(0,1) ? 1 : -1;
}
...
void CNPC_Spotlight::UpdateTargets(void)
{
	if (m_fSpotlightFlags & BITS_SPOTLIGHT_TRACK_ON)
	{
		...
		{
			// If it's an enemy
			if (IRelationType(pBestEntity->BaseEntity()) == D_HT)
			{
				...
				if (GetEnemy() == NULL)
				{
					...
					{
						AISquadIter_t iter;
						for (CBaseEntity *pSquadMember = squad->GetFirstMember( &iter ); pSquadMember; pSquadMember = squad->GetNextMember( &iter ) )
						{
							CAI_NPC *npc = (CAI_NPC *)CEntity::Instance(pSquadMember);
							// reset members who aren't activly engaged in fighting
							if (npc->GetEnemy() != pBestEntity && !npc->HasCondition( COND_SEE_ENEMY))
							{
								// give them a new enemy
								npc->SetLastAttackTime( 0 );
								npc->SetCondition ( COND_NEW_ENEMY );
							}
						}
					}
				}
				...
				{
					if (IRelationPriority(pBestEntity->BaseEntity()) > IRelationPriority(GetEnemy_CBase()))
					{
						SetInspectTargetToEnemy(pBestEntity);
					}
				}
			...
			else
			{
				...
				{
					if (IRelationPriority(pBestEntity->BaseEntity()) > IRelationPriority(GetTarget()->BaseEntity()))
					{
						SetInspectTargetToEntity(pBestEntity,SPOTLIGHT_ENTITY_INSPECT_LENGTH);
					}
				}
			...
			if (HasCondition(COND_HEAR_COMBAT) || HasCondition(COND_HEAR_DANGER) )
			{
				...
				if (pSound)
				{
					...
					if (SpotlightIsPositionLegal(vSoundPos))
					{
						...
						// Fire alert output
						m_pOutputAlert.FireOutput((CBaseEntity *)NULL,BaseEntity());
		...
		if (GetTarget())
		{
			if (!SpotlightIsPositionLegal(GetEnemies()->LastKnownPosition(GetTarget()->BaseEntity())))
			{
				ClearInspectTarget();
			}
			else if (!FVisible_Entity(GetTarget()->BaseEntity()))
			{
				ClearInspectTarget();
			}
		}
		if (GetEnemy())
		{
			if (!FVisible_Entity(GetEnemy_CBase()))
			{
				ClearInspectTarget();
			}
			...
			{
				UpdateEnemyMemory(GetEnemy_CBase(),GetEnemy()->GetAbsOrigin());
			}
		}
...
void CNPC_Spotlight::SpotlightCreate(void)
{
	...
	UTIL_TraceLine ( GetAbsOrigin(), GetAbsOrigin() + m_vSpotlightDir * m_flSpotlightMaxLength, 
		MASK_OPAQUE, BaseEntity(), COLLISION_GROUP_NONE, &tr);

	m_pSpotlightTarget				= (CE_CSpotlightEnd*)CreateEntityByName( "spotlight_end" );
	m_pSpotlightTarget->Spawn();
	m_pSpotlightTarget->SetLocalOrigin( tr.endpos );
	m_pSpotlightTarget->SetOwnerEntity( BaseEntity() );
	*(m_pSpotlightTarget->m_clrRender) = m_clrRender;
	...
	m_pSpotlight = CE_CBeam::BeamCreate( "sprites/spotlight.vmt", 2.0 );
...
Vector CNPC_Spotlight::SpotlightCurrentPos(void)
{
	...
	{
		// Make enginerandom movement frame independent
		if (enginerandom->RandomInt(0,10) == 0)
		{
			m_flYawDir *= -1;
		}
		if (enginerandom->RandomInt(0,10) == 0)
	...
	// ---------------------------------------------
	trace_t tr;
	UTIL_TraceLine ( GetAbsOrigin(), GetAbsOrigin() + (m_vSpotlightDir * 2 * m_flSpotlightMaxLength), 
		(CONTENTS_SOLID|CONTENTS_MOVEABLE|CONTENTS_MONSTER|CONTENTS_DEBRIS),
		BaseEntity(), COLLISION_GROUP_NONE, &tr);
...
void CNPC_Spotlight::SpotlightUpdate(void)
{
	...
	m_pOutputPosition.Set(m_pSpotlightTarget->GetLocalOrigin(),BaseEntity(),BaseEntity());
...
//------------------------------------------------------------------------------
void CNPC_Spotlight::SetScriptedTarget( CE_CScriptedTarget *pScriptedTarget )
{
	if (pScriptedTarget)
	{
...
//-----------------------------------------------------------------------------
bool CNPC_Spotlight::HandleInteraction(int interactionType, void *data, CBaseEntity* sourceEnt)
{
	if (interactionType == *(g_interactionScriptedTarget))
	{
		...
		else
		{
			...
			SetScriptedTarget((CE_CScriptedTarget*)cent);
			return true;
		}
	}
	return false;
