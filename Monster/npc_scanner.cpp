...
void CNPC_CScanner::Gib( void )
{
	...
	if( m_bIsClawScanner )
	{
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 500, 250, "models/gibs/Shield_Scanner_Gib1.mdl");
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 500, 250, "models/gibs/Shield_Scanner_Gib2.mdl");
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 500, 250, "models/gibs/Shield_Scanner_Gib3.mdl");
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 500, 250, "models/gibs/Shield_Scanner_Gib4.mdl");
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 500, 250, "models/gibs/Shield_Scanner_Gib5.mdl");
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 500, 250, "models/gibs/Shield_Scanner_Gib6.mdl");
	}
	else
	{
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 500, 250, "models/gibs/scanner_gib01.mdl" );
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 500, 250, "models/gibs/scanner_gib02.mdl" );
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 500, 250, "models/gibs/scanner_gib04.mdl" );
		CE_CGib::SpawnSpecificGibs( BaseEntity(), 1, 500, 250, "models/gibs/scanner_gib05.mdl" );
	}

	// Add a enginerandom chance of spawning a battery...
	if ( !HasSpawnFlags(SF_NPC_NO_WEAPON_DROP) && enginerandom->RandomFloat( 0.0f, 1.0f) < 0.3f )
	{
		CItem<CSoda_Fix> *pBattery = (CItem<CSoda_Fix> *)CreateEntityByName("item_battery");
		if ( pBattery )
		{
...
bool CNPC_CScanner::FValidateHintType(CBaseEntity *pHint)
{
	...
	return( hint->HintType() == HINT_WORLD_WINDOW );
}
...
void CNPC_CScanner::RequestInspectSupport(void)
{
	...
	if (squad)
	{
		...
		{
			CAI_NPC *pSquadNPC = (CAI_NPC *)CEntity::Instance(pSquadMember);
			if (pSquadNPC != this)
			{
				if (GetTarget())
				{
					pSquadNPC->DispatchInteraction(g_interactionScannerSupportEntity,((void *)((CBaseEntity*)(GetTarget()->BaseEntity()))),BaseEntity());
				}
				else
				{
					pSquadNPC->DispatchInteraction(g_interactionScannerSupportPosition,((void *)m_vInspectPos.Base()),BaseEntity());
				}
			}
		}
	}
...
bool CNPC_CScanner::IsValidInspectTarget(CEntity *pEntity)
{
	...
	if (squad)
	{
		...
		{
			CAI_NPC *pSquadNPC = (CAI_NPC *)CEntity::Instance(pSquadMember);
			if (pSquadNPC->GetTarget() == pEntity)
			{
				return false;
			}
		}
...
void CNPC_CScanner::DeployMine()
{
	...
	for ( child = FirstMoveChild(); child != NULL; child = child->NextMovePeer() )
	{
		if( FClassnameIs( child, "combine_mine" ) )
		{
			...
			child->SetOwnerEntity( BaseEntity() );
...
void CNPC_CScanner::InputEquipMine(inputdata_t &inputdata)
{
	...
	CEntity *pEnt = CreateEntityByName( "combine_mine" );
	...
	if( m_bIsClawScanner )
	{
		...
		if( attachment > -1 )
		{
			...
			pEnt->SetParent( BaseEntity(), attachment );
...
{
	CEntity *pEnt = g_helpfunc.FindEntityGeneric( (CBaseEntity *)NULL, inputdata.value.String(), BaseEntity(), inputdata.pActivator );
	
	if ( pEnt != NULL )
	{
		// Set and begin to inspect our target
		SetInspectTargetToEnt( pEnt->BaseEntity(), SCANNER_CIT_INSPECT_LENGTH );
...
void CNPC_CScanner::GatherConditions( void )
{
		...
		if ( pSound )
		{
			...
			if ( pSound->m_hOwner != NULL )
			{
				...
				if ( IRelationType( pSound->m_hOwner ) != D_LI )
				{
					...
					if ( FVisible_Entity( pSound->m_hOwner ) )
					{
	...
	if ( gpGlobals->curtime	> m_fCheckCitizenTime && HaveInspectTarget() == false )
	{
		...
		if ( pBestEntity != NULL )
		{
			SetInspectTargetToEnt( pBestEntity->BaseEntity(), SCANNER_CIT_INSPECT_LENGTH );
	...
	if ( m_nFlyMode == SCANNER_FLY_PHOTO )
	{
		...
		if ( gpGlobals->curtime > m_fNextPhotographTime && HaveInspectTarget() && GetCurrentVelocity().LengthSqr() < (64*64) )
		{
			...
			if ( fInspectDist > SCANNER_PHOTO_NEAR_DIST && fInspectDist < SCANNER_PHOTO_FAR_DIST )
			{
				...
				{
					CEntity *cent = GetTarget();
					trace_t tr;
					UTIL_TraceLine ( GetAbsOrigin(), InspectTargetPosition(), MASK_BLOCKLOS, (cent)?cent->BaseEntity():NULL, COLLISION_GROUP_NONE, &tr);
...
Disposition_t CNPC_CScanner::IRelationType(CBaseEntity *pTarget)
{
	...
	if ( cent && cent->Classify() == CLASS_PLAYER )
	{
...
int CNPC_CScanner::SelectSchedule(void)
{
	...
	if ( GetEnemy() != NULL && GetEnemy()->IsAlive() && m_bShouldInspect )
	{
		...
		SetInspectTargetToEnt( GetEnemy_CBase(), 9999 );
...
void CNPC_CScanner::SpotlightCreate(void)
{
	...
	beam->SetEndWidth( beam->GetWidth() );
	beam->SetBeamFlags( (FBEAM_SHADEOUT|FBEAM_NOTILE) );
	beam->SetBrightness( 32 );
	beam->SetNoise( 0 );
	beam->EntsInit( this, light );
	beam->SetHDRColorScale( 0.75f );	// Scale this back a bit on HDR maps
	// attach to light
	beam->SetStartAttachment( LookupAttachment( SCANNER_ATTACHMENT_LIGHT ) );
...
void CNPC_CScanner::StartTask( const Task_t *pTask )
{
	...
	case TASK_GET_PATH_TO_TARGET:
	{
		...
		else 
		{
			...
			goal.pTarget = GetTarget()->BaseEntity();
...
bool CNPC_CScanner::OverrideMove( float flInterval )
{
	...
	else
	{
		...
		CBaseEntity *cbase_pMoveTarget = (pMoveTarget)?pMoveTarget->BaseEntity():NULL;
		...
		if ( pMoveTarget || HaveInspectTarget() )
		{
			...
			if ( ( tr.m_pEnt == cbase_pMoveTarget ) || ( fTargetDist < 50 ) )
			{
		...
		{
			if ( OverridePathMove( cbase_pMoveTarget, flInterval ) )
			{
...
//-----------------------------------------------------------------------------
bool CNPC_CScanner::HandleInteraction(int interactionType, void *data, CBaseEntity* pSourceEnt)
{
	CCombatCharacter *cent_pSourceEnt = (CCombatCharacter *)CEntity::Instance(pSourceEnt);
	...
	if (interactionType ==	g_interactionScannerSupportEntity)
	{
		...
		if (GetEnemy() == NULL && !HaveInspectTarget())
		{
			...
			{
				float fInspectTime = (((CNPC_CScanner*)cent_pSourceEnt)->m_fInspectEndTime - gpGlobals->curtime);
				SetInspectTargetToEnt(pTarget->BaseEntity(),fInspectTime);
