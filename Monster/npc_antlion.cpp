...
// Number of times the antlions will attempt to generate a enginerandom chase position
...
bool CNPC_Antlion::ShouldJump( void )
{
	...
	GetMoveProbe()->MoveLimit( NAV_JUMP, GetAbsOrigin(), targetPos, MASK_NPCSOLID, (cent)?cent->BaseEntity():NULL, &moveTrace );
...
void CNPC_Antlion::CascadePush( const Vector &vecForce )
{
	...
	for ( int i = 0; i < nNumEnemies; i++ )
	{
		...
		if ( cent == NULL || cent->Classify() != CLASS_ANTLION || cent == this )
			continue;

		CNPC_Antlion *pAntlion = dynamic_cast<CNPC_Antlion *>(cent);
		if ( pAntlion != NULL )
		{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Antlion::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr , CDmgAccumulator *dump)
{
	CTakeDamageInfo newInfo = info;
