...
private:
	...
	void ExplodeThink();
	void RampSoundThink_CBE();
	void RampSoundThink();
	void WarningBlinkerThink_CBE();
	void WarningBlinkerThink();
	void StopWarningBlinker();
	void AnimateThink_CBE();
	void AnimateThink();
...
private:
	...
	// Blink lights
	void BlinkLightsThink_CBE();
	void BlinkLightsThink();

	// Spotlights
	void SpotlightThink_CBE();
	void SpotlightThink();
...
void CNPC_AttackHelicopter::Startup()
{
	...
	if ( HasSpawnFlags( SF_HELICOPTER_LIGHTS ) )
	{
		...
		SetContextThink( &CNPC_AttackHelicopter::BlinkLightsThink_CBE, gpGlobals->curtime + CHOPPER_LIGHT_BLINK_TIME_SHORT, s_pBlinkLightThinkContext );
	}
}

void CNPC_AttackHelicopter::BlinkLightsThink_CBE()
{
	CNPC_AttackHelicopter *cent = (CNPC_AttackHelicopter *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->BlinkLightsThink();
	}
}
...
void CNPC_AttackHelicopter::BlinkLightsThink()
{
	...
	SetContextThink( &CNPC_AttackHelicopter::BlinkLightsThink_CBE, gpGlobals->curtime + flTime, s_pBlinkLightThinkContext );
}
...
void CNPC_AttackHelicopter::SpotlightThink_CBE( void )
{
	CNPC_AttackHelicopter *cent = (CNPC_AttackHelicopter *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
...
void CNPC_AttackHelicopter::SpotlightThink()
{
	...
	m_Spotlight.Update();
	SetContextThink( &CNPC_AttackHelicopter::SpotlightThink_CBE, gpGlobals->curtime + TICK_INTERVAL, s_pSpotlightThinkContext );
}
...
int CNPC_AttackHelicopter::BuildMissTargetList( int nCount, CEntity **ppMissCandidates )
{
	...
	for ( int i = 0; i < numEnts; i++ )
	{
		...
		CEntity *cent = CEntity::Instance(pEnts[i]);
...
void CNPC_AttackHelicopter::FireElectricityGun( )
{
	...
	for ( i = 0; i < nEntCount; i++ )
	{
		...
		CEntity *cent = CEntity::Instance(ppEnts[i]);
...
void CGrenadeHelicopter::Spawn( void )
{
	...
	if( hl2_episodic->GetBool() )
	{
		SetContextThink( &CGrenadeHelicopter::AnimateThink_CBE, gpGlobals->curtime, s_pAnimateThinkContext );
	}
}
...
void CGrenadeHelicopter::BecomeActive()
{
	...
	if ( !bMegaBomb )
	{
		SetContextThink( &CGrenadeHelicopter::RampSoundThink_CBE, gpGlobals->curtime + GetBombLifetime() - BOMB_RAMP_SOUND_TIME, s_pRampSoundContext );
	...
	SetContextThink( &CGrenadeHelicopter::WarningBlinkerThink_CBE, gpGlobals->curtime + (GetBombLifetime() - 2.0f), s_pWarningBlinkerContext );
...
void CGrenadeHelicopter::RampSoundThink_CBE()
{
	CGrenadeHelicopter *cent = (CGrenadeHelicopter *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->RampSoundThink();
	}
...
void CGrenadeHelicopter::WarningBlinkerThink_CBE()
{
	CGrenadeHelicopter *cent = (CGrenadeHelicopter *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->WarningBlinkerThink();
	}
}
...
*/
	...
	{
		SetContextThink( &CGrenadeHelicopter::WarningBlinkerThink_CBE, gpGlobals->curtime + 0.1f, s_pWarningBlinkerContext );
	}
	else
	{
		SetContextThink( &CGrenadeHelicopter::WarningBlinkerThink_CBE, gpGlobals->curtime + 0.2f, s_pWarningBlinkerContext );
	}
#endif//HL2_EPISODIC
}
...
void CGrenadeHelicopter::AnimateThink_CBE()
{
	CGrenadeHelicopter *cent = (CGrenadeHelicopter *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
...
{
	StudioFrameAdvance();
	SetContextThink( &CGrenadeHelicopter::AnimateThink_CBE, gpGlobals->curtime + 0.1f, s_pAnimateThinkContext );
}
