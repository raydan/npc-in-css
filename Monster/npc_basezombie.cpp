...
bool CNPC_BaseZombie::FindNearestPhysicsObject( int iMaxMass )
{
	...
	for( i = 0 ; i < count ; i++ )
	{
		...
		pPhysObj = cent->VPhysicsGetObject();

		Assert( !( !pPhysObj || pPhysObj->GetMass() > iMaxMass || !pPhysObj->IsAsleep() ) );

		Vector center = cent->WorldSpaceCenter();
		...
		vecDirToObject = cent->WorldSpaceCenter() - GetAbsOrigin();
		...
		if ( (center.z + cent->BoundingRadius()) < vecZombieKnees.z )
			continue;
		...
		vcollide_t *pCollide = modelinfo->GetVCollide( cent->GetModelIndex() );
		
		Vector objMins, objMaxs;
		physcollision->CollideGetAABB( &objMins, &objMaxs, pCollide->solids[0], cent->GetAbsOrigin(), cent->GetAbsAngles() );

		if ( objMaxs.z < vecZombieKnees.z )
			continue;

		if ( !FVisible_Entity( pList[i] ) )
			continue;

		// Make this the last check, since it makes a string.
		// Don't swat server ragdolls!
		if ( FClassnameIs( cent, "physics_prop_ragdoll" ) )
			continue;
			
		if ( FClassnameIs( cent, "prop_ragdoll" ) )
			continue;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_BaseZombie::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *dump )
{
	...
	BaseClass::TraceAttack( infoCopy, vecDir, ptr, dump );
}
...
{
	//g_helpfunc.CSoundEnt_InsertSound( SOUND_COMBAT, EyePosition(), (int)volume, duration, BaseEntity(), SOUNDENT_CHANNEL_SPOOKY_NOISE );
}
...
void CNPC_BaseZombie::MoanSound( envelopePoint_t *pEnvelope, int iEnvelopeSize )
{
	...
	float duration = 0.0f;
	if(m_pMoanSound) {
		duration = g_SoundController->SoundPlayEnvelope( m_pMoanSound, SOUNDCTRL_CHANGE_VOLUME, pEnvelope, iEnvelopeSize );
	}
...
void CNPC_BaseZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_SWATITEM )
	{
		...
		if ( pEnemy )
		{
			...
			PhysicsImpactSound( pEnemy->BaseEntity(), pPhysObj, CHAN_BODY, pPhysObj->GetMaterialIndex(), physprops->GetSurfaceIndex("flesh"), 0.5, 800 );
...
{
	g_helpfunc.UTIL_PrecacheOther(GetHeadcrabClassname());
