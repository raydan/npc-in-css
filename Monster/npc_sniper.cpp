...
//=========================================================
//=========================================================
class CSniperTarget : public CE_InfoTarget_Fix
{
public:
	CE_DECLARE_CLASS( CSniperTarget, CE_InfoTarget_Fix );
	DECLARE_DATADESC();
...
//=========================================================
//=========================================================
class CSniperBullet : public CE_InfoTarget_Fix
{
public:
	CE_DECLARE_CLASS( CSniperBullet, CE_InfoTarget_Fix );
	...
	bool Start( const Vector &vecOrigin, const Vector &vecTarget, CEntity *pOwner, bool bDirectShot );
	void Stop( void );
...
//=========================================================
//=========================================================
class CProtoSniper : public CE_Cycler_Fix
{
public:
	CE_DECLARE_CLASS( CProtoSniper, CE_Cycler_Fix );
	...
	Vector  DesiredBodyTarget( CEntity *pTarget );
	Vector	LeadTarget( CEntity *pTarget );
	CEntity *PickDeadPlayerTarget();
...
private:
	...
	void AddOldDecoy( CEntity *pDecoy );
	bool HasOldDecoy( CEntity *pDecoy );
	bool FindFrustratedShot( float flNoise );

	bool VerifyShot( CEntity *pTarget );
...
private:
	...
	/// A enginerandom number from 0 to this is added to m_flKeyfieldPaintTime
	...
	float						m_flPatience;
	int							m_iMisses;
	CFakeHandle					m_hDecoyObject;
	CFakeHandle					m_hSweepTarget;
	...
	CEntity						*m_pGroupTarget[ SNIPER_MAX_GROUP_TARGETS ];
	...
	// Protection
	CFakeHandle					m_hProtectTarget;			// Entity that this sniper is supposed to protect
...
bool CProtoSniper::QuerySeeEntity( CBaseEntity *pEntity, bool bOnlyHateOrFearIfNPC )
{
	...
	if( VerifyShot( CEntity::Instance(pEntity) ) )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CProtoSniper::FInViewCone_Entity ( CBaseEntity *pEntity )
{
	CEntity *cent = CEntity::Instance(pEntity);
	if( cent->GetFlags() & FL_CLIENT )
	{
	...
	return BaseClass::FInViewCone_Vector( cent->EyePosition() );
}
...
void CProtoSniper::LaserOn( const Vector &vecTarget, const Vector &vecDeviance )
{
	if (!m_pBeam)
	{
		m_pBeam = CE_CBeam::BeamCreate( "effects/bluelaser1.vmt", 1.0f );
	...
	vecInitialAim.x += enginerandom->RandomFloat( -vecDeviance.x, vecDeviance.x );
	vecInitialAim.y += enginerandom->RandomFloat( -vecDeviance.y, vecDeviance.y );
	vecInitialAim.z += enginerandom->RandomFloat( -vecDeviance.z, vecDeviance.z );
...
void CProtoSniper::PaintTarget( const Vector &vecTarget, float flPaintTime )
{
	...
	if( P > 0.25f && GetEnemy() && GetEnemy()->IsNPC() && HasCondition(COND_SEE_ENEMY) && !m_bWarnedTargetEntity )
	{
		...
		if( GetEnemy()->Classify() == CLASS_PLAYER_ALLY_VITAL && GetEnemy()->MyNPCPointer()->FVisible_Entity(BaseEntity()) )
		{
			g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER | SOUND_CONTEXT_REACT_TO_SOURCE, GetEnemy()->EarPosition(), 16, 1.0f, BaseEntity() );
		}
	}
...
#define THRESHOLD 0.8f
	...
	UTIL_TraceLine( vecStart, vecStart + vecCurrentDir * 8192, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
bool CProtoSniper::IsPlayerAllySniper()
{
	...
	return IRelationType( (pPlayer)?pPlayer->BaseEntity():NULL ) == D_LI;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CProtoSniper::DispatchKeyValue( const char *szKeyName, const char *szValue )
{
...
LINK_ENTITY_TO_CUSTOM_CLASS( npc_sniper, cycler, CProtoSniper );
LINK_ENTITY_TO_CUSTOM_CLASS( proto_sniper, cycler, CProtoSniper );
LINK_ENTITY_TO_CUSTOM_CLASS( sniperbullet, info_target, CSniperBullet );
...
void CProtoSniper::Precache( void )
{
	...
	g_helpfunc.UTIL_PrecacheOther( "sniperbullet" );
...
void CProtoSniper::Spawn( void )
{
	...
	UTIL_SetSize( BaseEntity(), Vector( -16, -16 , 0 ), Vector( 16, 16, 64 ) );
...
void CProtoSniper::SetSweepTarget( const char *pszTarget )
{
	...
	// In case the sniper was sweeping a enginerandom set of targets when asked to sweep a normal chain.
	ClearTargetGroup();

	pTarget = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, pszTarget );

	if( !pTarget )
	{
		DevMsg( "**Sniper %s cannot find sweep target %s\n", GetClassname(), pszTarget );
		m_hSweepTarget.Set(NULL);
		return;
	}

	m_hSweepTarget.Set(pTarget->BaseEntity());
}
...
void CProtoSniper::InputSweepGroupRandomly( inputdata_t &inputdata )
{
	...
	// Gary: Yes, this sucks. :)
	pEnt = CEntity::Instance(g_helpfunc.FirstEnt());
	...
	do
	{
		...
		pEnt = CEntity::Instance(g_helpfunc.NextEnt( pEnt->BaseEntity() ));

	} while( pEnt );

	CEntity *cent = m_pGroupTarget[ enginerandom->RandomInt( 0, m_iNumGroupTargets - 1 ) ];
	m_hSweepTarget.Set((cent)?cent->BaseEntity():NULL);
}
...
{
	CEntity *cent = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, inputdata.value.String(), NULL, inputdata.pActivator, inputdata.pCaller );
	m_hProtectTarget.Set((cent)?cent->BaseEntity():NULL);
...
int CProtoSniper::IRelationPriority( CBaseEntity *pTarget )
{
	...
	if ( m_hProtectTarget )
	{
		...
		float flDistance = (cent->GetAbsOrigin() - m_hProtectTarget->GetAbsOrigin()).LengthSqr();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CProtoSniper::HasOldDecoy( CEntity *pDecoy )
{
#if 0
	int i;
...
//-----------------------------------------------------------------------------
void CProtoSniper::AddOldDecoy( CEntity *pDecoy )
{
#if 0
...
int CProtoSniper::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() == DMG_GENERIC && info.GetInflictor() == BaseEntity() )
	...
	float flDist = GetAbsOrigin().DistTo( inf->GetAbsOrigin() );
...
void CProtoSniper::Event_Killed( const CTakeDamageInfo &info )
{
	if( !(m_spawnflags & SF_SNIPER_NOCORPSE) )
	{
		...
		float flForce = enginerandom->RandomFloat( 500, 700 ) * 10;
	...
	if( attacker )
	{
		attacker->Event_KilledOther(BaseEntity(), info);
		g_CEventQueue->AddEvent( info.GetAttacker(), "KilledNPC", 0.3, BaseEntity(), BaseEntity() );
	}
...
int CProtoSniper::SelectSchedule ( void )
{
	...
	CEntity *player = UTIL_GetNearestVisiblePlayer(this);

	if( player && !player->IsAlive() && m_bKilledPlayer )
	{
	...
	if( HasCondition( COND_HEAR_DANGER ) )
	{
		...
		if( pSound && pSound->IsSoundType( SOUND_DANGER ) && BaseClass::FVisible_Vector( pSound->GetSoundReactOrigin() ) )
		{
...
#define SEARCH_DEPTH	50
	...
	CEntity *pDecoys[ SNIPER_NUM_DECOYS ];
	...
	for( i = 0 ; i < count ; i++ )
	{
		CEntity *cent = CEntity::Instance(pList[i]);
		pCurrent = cent;
		...
		if( FClassnameIs( pCurrent, "func_breakable" ) || FClassnameIs( pCurrent, "prop_physics" ) || FClassnameIs( pCurrent, "func_physbox" ) )
		{
			...
			if( pCurrent->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD )
			{
				...
				if( FVisible_Entity( pCurrent->BaseEntity() ) )
				{
					m_hDecoyObject.Set(pCurrent->BaseEntity());
	...
	// try 4 times to pick a enginerandom object from the list
	...
	for( i = 0 ; i < 4 ; i++ )
	{
		...
		// Pick one of the decoys at enginerandom.
		pProspect = pDecoys[ enginerandom->RandomInt( 0, iIterator - 1 ) ];
		...
		UTIL_TraceLine( vecBulletOrigin, vecDecoyTarget + vecDirToDecoy * 32, 
			MASK_BLOCKLOS, BaseEntity(), COLLISION_GROUP_NONE, &tr);

		if( CEntity::Instance(tr.m_pEnt) == pProspect || tr.fraction == 1.0 )
		{
			// Great! A shot will hit this object.
			m_hDecoyObject.Set(pProspect->BaseEntity());
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CProtoSniper::VerifyShot( CEntity *pTarget )
{
	trace_t tr;

	Vector vecTarget = DesiredBodyTarget( pTarget );
	UTIL_TraceLine( GetBulletOrigin(), vecTarget, MASK_SHOT, pTarget->BaseEntity(), COLLISION_GROUP_NONE, &tr );
	...
	if( tr.fraction != 1.0 )
	{
		if( pTarget->IsPlayer() )
		{
			...
			// head in full view.
			UTIL_TraceLine( GetBulletOrigin(), pTarget->EyePosition(), MASK_SHOT, pTarget->BaseEntity(), COLLISION_GROUP_NONE, &tr );
		...
		if( cent )
		{
			if( cent->m_takedamage == DAMAGE_YES )
			{
...
void CProtoSniper::ScopeGlint()
{
	...
	g_helpfunc.DispatchEffect( "CommandPointer", data );
}
...
bool CProtoSniper::FireBullet( const Vector &vecTarget, bool bDirectShot )
{
	...
	pBullet->SetOwnerEntity( BaseEntity() );
...
void CProtoSniper::StartTask( const Task_t *pTask )
{
	...
	case TASK_SNIPER_PLAYER_DEAD:
		{
			CEntity *cent = UTIL_GetNearestVisiblePlayer(this);
			m_hSweepTarget.Set((cent)?cent->BaseEntity():NULL);
			SetWait( 4.0f );
			if(m_hSweepTarget)
			{
		...
		else
		{
			...
			else
			{
				m_flPaintTime = m_flKeyfieldPaintTimeNoise > 0									 ? 
					m_flKeyfieldPaintTime + enginerandom->RandomFloat( 0, m_flKeyfieldPaintTimeNoise ) :
	...
	case TASK_SNIPER_PAINT_FRUSTRATED:
		m_flPaintTime = SNIPER_PAINT_FRUSTRATED_TIME + enginerandom->RandomFloat( 0, SNIPER_PAINT_FRUSTRATED_TIME );
		SetWait( m_flPaintTime );
		if( FindFrustratedShot( pTask->flTaskData ) )
		{
			LaserOff();
...
void CProtoSniper::RunTask( const Task_t *pTask )
{
	...
	case TASK_SNIPER_PLAYER_DEAD:
		if( IsWaitFinished() )
		{
			CEntity *cent = PickDeadPlayerTarget();;
			m_hSweepTarget.Set((cent)?cent->BaseEntity():NULL);
	...
	case TASK_SNIPER_PAINT_SWEEP_TARGET:
		...
		if( IsWaitFinished() )
		{
			...
			CEntity *pNext;
			pNext = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_hSweepTarget->m_target );
			...
			if( pNext || IsSweepingRandomly() )
			{
				...
				else
				{
					...
					m_hSweepTarget.Set((pNext)?pNext->BaseEntity():NULL);
				}
...
void CProtoSniper::PrescheduleThink( void )
{
	...
	if( GetEnemy() != NULL )
	{
		if( gpGlobals->curtime - GetEnemies()->LastTimeSeen( GetEnemy_CBase() ) > 30 )
		{
...
//---------------------------------------------------------
//---------------------------------------------------------
Vector CProtoSniper::DesiredBodyTarget( CEntity *pTarget )
{
	...
	{
		if( !BaseClass::FVisible_Vector( vecTarget ) )
		{
...
//---------------------------------------------------------
//---------------------------------------------------------
Vector CProtoSniper::LeadTarget( CEntity *pTarget )
{
	float targetTime;
	float targetDist;
	...
	*/
	...
	else
	{
		...
		else
		{
			...
			CAnimating *pAnimating;

			pAnimating = (CAnimating *)pTarget;
...
//---------------------------------------------------------
CEntity *CProtoSniper::PickDeadPlayerTarget()
{
	const int iSearchSize = 32;
	CEntity *pTarget = UTIL_GetNearestVisiblePlayer(this);
	...
	if( iNumEntities > 0 )
	{
		...
		for( i = 0 ; i < 10 ; i++ )
		{
			CEntity *pCandidate = CEntity::Instance(pEntities[ enginerandom->RandomInt(0, iNumEntities - 1)]);
			if( !pCandidate->IsNPC() && FInViewCone_Entity(pCandidate->BaseEntity()) )
			{
				return pCandidate;
			}
		}
...
#define MAX_TRIES	15
	for( int i = 0 ; i < MAX_TRIES ; i++ )
	{
		...
		vecSpot.x += enginerandom->RandomFloat( -64, 64 );
		vecSpot.y += enginerandom->RandomFloat( -64, 64 );
		vecSpot.z += enginerandom->RandomFloat( -40, 40 );
		...
		if( GetEnemy()->FVisible_Vector( vecSpot ) || i == MAX_TRIES - 1 )
		{
			trace_t tr;
			UTIL_TraceLine(vecSrc, vecSrc + vecDir * 8192, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr);

			if( !GetEnemy()->FVisible_Vector( tr.endpos ) )
			{
...
bool CProtoSniper::FVisible_Entity( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	if( m_spawnflags & SF_SNIPER_VIEWCONE )
	{
	...
	if( fabs( GetAbsOrigin().z - cent->WorldSpaceCenter().z ) <= 120.f )
	{
	...
	AngleVectors( cent->GetLocalAngles(), NULL, &vecRight, NULL );

	vecEye = vecRight * SNIPER_EYE_DIST - vecVerticalOffset;
	UTIL_TraceLine( EyePosition(), cent->EyePosition() + vecEye, MASK_BLOCKLOS, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
void CSniperBullet::BulletThink( void )
{
	...
	if( gpGlobals->curtime >= m_SoundTime )
	{
		...
		if( GetOwnerEntity() )
		{
			...
			if( pSniper && pSniper->GetEnemy() )
			{
				...
				if( pEnemyNPC && GetOwnerEntity() && pEnemyNPC->FVisible_Vector( GetOwnerEntity()->WorldSpaceCenter() ) )
				{
					CEntity *cent = GetOwnerEntity();
					g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER | SOUND_CONTEXT_FROM_SNIPER, pSniper->GetEnemy()->EarPosition(), 16, 1.0f, (cent)?cent->BaseEntity():NULL );
				}
			}
		}
	...
	trace_t tr;
	UTIL_TraceLine( vecStart, vecEnd, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );

	if( tr.fraction != 1.0 )
	{
		// This slice of bullet will hit something.
		GetOwnerEntity()->FireBullets( 1, vecStart, m_vecDir, vec3_origin, flDist, m_AmmoType, 0, -1, -1, sk_plr_dmg_sniper_round.GetFloat() );
		m_iImpacts++;

		CEntity *cent = CEntity::Instance(tr.m_pEnt);
#ifdef HL2_EPISODIC
		if( cent->IsNPC() || m_iImpacts == NUM_PENETRATIONS )
#else	 
		if( cent->m_takedamage == DAMAGE_YES || m_iImpacts == NUM_PENETRATIONS )
#endif//HL2_EPISODIC
		{
			...
			{
				if( cent->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD )
				{
					Pickup_ForcePlayerToDropThisObject(cent);
				}
			}
		...
		else
		{
			...
			for( int i = 0 ; i < NUM_STEPS ; i++ )
			{
				...
				if( UTIL_PointContents( vecCursor ) != CONTENTS_SOLID )
				{
					...
					// Fire another tracer.
					UTIL_TraceLine( vecCursor, vecCursor + m_vecDir * 8192, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
//=========================================================
//=========================================================
bool CSniperBullet::Start( const Vector &vecOrigin, const Vector &vecTarget, CEntity *pOwner, bool bDirectShot )
{
	...
	trace_t tr;
	UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() + m_vecDir * 8192, MASK_SOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CSniperTarget::DispatchKeyValue( const char *szKeyName, const char *szValue )
{
...
LINK_ENTITY_TO_CUSTOM_CLASS( info_snipertarget, info_target, CSniperTarget );
