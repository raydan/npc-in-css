...
//-----------------------------------------------------------------------------
class CEnvHeadcrabCanister : public CSoda_Fix
{
	CE_DECLARE_CLASS( CEnvHeadcrabCanister, CSoda_Fix );
	DECLARE_DATADESC();
...
void CEnvHeadcrabCanister::Precache( void )
{
	...
	g_helpfunc.UTIL_PrecacheOther( s_pHeadcrabClass[m_nHeadcrabType] );
}
...
void CEnvHeadcrabCanister::SetTransmit( CCheckTransmitInfo *pInfo, bool bAlways )
{
	...
	{
		//CE_FIX
		//m_hSmokeTrail->m_bInSetTransmit = false;
