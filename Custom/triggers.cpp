...
void CTriggerWeaponDissolve::Activate( void )
{
	...
	SetContextThink( &CTriggerWeaponDissolve::DissolveThink_CBE, gpGlobals->curtime + 0.1f, s_pDissolveThinkContext );
}
...
void CTriggerWeaponDissolve::DissolveThink_CBE( void )
{
	CTriggerWeaponDissolve *cent = (CTriggerWeaponDissolve *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->DissolveThink();
	}
...
void CTriggerWeaponDissolve::DissolveThink( void )
{
	...
	for ( int i = 0; i < numWeapons; i++ )
	{
		...
		m_pWeapons.Remove( i );
		SetContextThink( &CTriggerWeaponDissolve::DissolveThink_CBE, gpGlobals->curtime + enginerandom->RandomFloat( 0.5f, 1.5f ), s_pDissolveThinkContext );
		return;
	}
