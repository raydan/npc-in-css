...
void CPropJeep::FireChargedCannon( void )
{
	...
	else if ( pHit != NULL )
	{
		...
		pHit->DispatchTraceAttack( dmgInfo, aimDir, &tr, NULL );
	}
...
{
	bool ret = ( !m_bEnterAnimOn && !m_bExitAnimOn && !m_bLocked && (m_nSpeed <= g_jeepexitspeed->GetFloat() ) );
	return MonsterTools::CanExitVehicle(pEntity, BaseEntity(), ret);
}
...
void CPropJeep::SetupMove( CBaseEntity *player, CUserCmd *ucmd, IMoveHelper *pHelper, CMoveData *move )
{
	...
	if ( m_flOverturnedTime > OVERTURNED_EXIT_WAITTIME )
	{
		if ( (ucmd->buttons & (IN_FORWARD|IN_BACK|IN_MOVELEFT|IN_MOVERIGHT|IN_SPEED|IN_JUMP|IN_ATTACK|IN_ATTACK2) ) && !m_bExitAnimOn )
		{
			...
			{
				MonsterTools::OnVehicleOverturned(cent_m_hPlayer->BaseEntity(), BaseEntity());
			}
...
void CPropJeep::ExitVehicle( int nRole )
{
	...
	if ( GlobalEntity_GetState( "no_seagulls_on_jeep" ) == GLOBAL_OFF )
	{
		...
		if ( pHint )
		{
			...
			SetContextThink( &CPropJeep::JeepSeagullThink_CBE, gpGlobals->curtime + JEEP_SEAGULL_THINK_INTERVAL, g_pJeepThinkContext );
		}
	}
}

void CPropJeep::JeepSeagullThink_CBE()
{
	CPropJeep *cent = (CPropJeep *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->JeepSeagullThink();
	}
}
...
void CPropJeep::JeepSeagullThink( void )
{
	...
	if ( cent )
	{
		...
		if ( pSeagull )
		{
			...
			if ( pSeagull->m_bOnJeep == true )
			{
				...
				SetContextThink( &CPropJeep::JeepSeagullThink_CBE, gpGlobals->curtime + JEEP_SEAGULL_POOP_INTERVAL, g_pJeepThinkContext );
			}
			else
			{
	...
	SetContextThink( &CPropJeep::JeepSeagullThink_CBE, gpGlobals->curtime + 0.2, g_pJeepThinkContext );
...
{
	bool ret = BaseClass::IsPassengerVisible(nRole);
	return MonsterTools::IsPassengerVisible( nRole, ret);
}
