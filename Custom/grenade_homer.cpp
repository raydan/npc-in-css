...
LINK_ENTITY_TO_CUSTOM_CLASS( grenade_homer, grenade, CGrenadeHomer );
...
void CGrenadeHomer::SetSpin(float flSpinMagnitude, float flSpinSpeed)
{
	...
	m_flSpinOffset		= enginerandom->RandomInt((int)-m_flSpinSpeed, (int)m_flSpinSpeed);
}
...
{
	CRocketTrail *pRocketTrail =  CRocketTrail::CreateRocketTrail();
	if(pRocketTrail)
	{
		...
		m_hRocketTrail[0].Set(pRocketTrail->BaseEntity());
	}
	/*
...
// Output  :
//------------------------------------------------------------------------------
void CGrenadeHomer::Launch( CEntity*		pOwner, 
							CEntity*		pTarget, 
							...
							int					nRocketTrailType)
{
	...
	if ( pTarget )
	{
		...
		g_helpfunc.CSoundEnt_InsertSound ( SOUND_DANGER, m_hTarget->GetAbsOrigin(), 300, flTime, pOwner->BaseEntity() );
	}
}
...
void CGrenadeHomer::GrenadeHomerTouch( CEntity *pOther )
{
	Assert( pOther );
...
{ 
	...
	if (m_hTarget != NULL)
	{
		...
		if ((vTargetPos - GetAbsOrigin()).Length() > 200)
		{
			...
			trace_t tr;
			UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() + vTravelDir, MASK_SHOT, m_hTarget->BaseEntity(), COLLISION_GROUP_NONE, &tr );
			if (tr.fraction != 1.0)
			{
				// Head off in normal 
...
{
	// Don't take damage from other homing grenades so can shoot in vollies
	if (FClassnameIs( CEntity::Instance(info.GetInflictor()), "grenade_homer"))
	{
		return 0;
	}
