...
LINK_ENTITY_TO_CUSTOM_CLASS( grenade_pathfollower, grenade, CGrenadePathfollower );
...
void CGrenadePathfollower::GrenadeTouch( CEntity *pOther )
{
	// ----------------------------------
...
void CGrenadePathfollower::Detonate(void)
{
	...
	trace_t		tr;
	UTIL_TraceLine ( GetAbsOrigin(), GetAbsOrigin() + 60*vecForward,  MASK_SOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, & tr);

	UTIL_DecalTrace( &tr, "Scorch" );

	UTIL_ScreenShake( GetAbsOrigin(), 25.0, 150.0, 1.0, 750, SHAKE_START );
	g_helpfunc.CSoundEnt_InsertSound ( SOUND_DANGER, GetAbsOrigin(), 400, 0.2 );

	CEntity *cent = GetThrower();

	g_helpfunc.USE_HL2_RADIUS_DAMAGE = true;
	RadiusDamage ( CTakeDamageInfo( BaseEntity(), (cent)?cent->BaseEntity():NULL, m_flDamage, DMG_BLAST ), GetAbsOrigin(),  m_DmgRadius, CLASS_NONE, NULL );
...
{
	m_pPathTarget = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, sPathCornerName );
	if (m_pPathTarget)
	{
	...
	// Make the trail
	CRocketTrail *rocktrail = CRocketTrail::CreateRocketTrail();
	m_hRocketTrail.Set((rocktrail)?rocktrail->BaseEntity():NULL);

	if ( rocktrail )
	{
		rocktrail->m_Opacity = 0.2f;
		rocktrail->m_SpawnRate = 100;
		rocktrail->m_ParticleLifetime = 0.5f;
		rocktrail->m_StartColor->Init( 0.65f, 0.65f , 0.65f );
		rocktrail->m_EndColor->Init( 0.0, 0.0, 0.0 );
		rocktrail->m_StartSize = 8;
		rocktrail->m_EndSize = 16;
		rocktrail->m_SpawnRadius = 4;
		rocktrail->m_MinSpeed = 2;
		rocktrail->m_MaxSpeed = 16;
		
		rocktrail->SetLifetime( 999 );
		rocktrail->FollowEntity( this, "0" );
	}
}
...
void CGrenadePathfollower::AimThink( void )
{
	...
	if (m_pPathTarget)
	{
		...
		{
			m_pPathTarget = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_pPathTarget->m_target );
			if (!m_pPathTarget)
			{	
				SetGravity( 1.0 );
			}
...
CGrenadePathfollower* CGrenadePathfollower::CreateGrenadePathfollower( string_t sModelName, string_t sFlySound, const Vector &vecOrigin, const QAngle &vecAngles, edict_t *pentOwner )
{
	...
	if ( pGrenade->edict() )
	{
		CEntity *cent = Instance( pentOwner );
		pGrenade->m_sFlySound	= sFlySound;
		pGrenade->SetOwnerEntity( (cent)?cent->BaseEntity():NULL);
