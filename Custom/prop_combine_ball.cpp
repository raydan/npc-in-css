...
ConVar	sk_npc_dmg_combineball( "sk_npc_dmg_combineball","15", 0);
ConVar	sk_combineball_guidefactor( "sk_combineball_guidefactor","0.5", 0);
ConVar	sk_combine_ball_search_radius( "sk_combine_ball_search_radius", "512", 0);
ConVar	sk_combineball_seek_angle( "sk_combineball_seek_angle","15.0", 0);
ConVar	sk_combineball_seek_kill( "sk_combineball_seek_kill","0", 0);
...
CEntity *CreateCombineBall( const Vector &origin, const Vector &velocity, float radius, float mass, float lifetime, CEntity *pOwner )
{
	...
	pBall->SetOwnerEntity( (pOwner)?pOwner->BaseEntity():NULL );
...
void CPropCombineBall::Spawn( void )
{
	...
	{
		m_pGlowTrail->FollowEntity( BaseEntity() );
...
{
	// Start our animation cycle. Use the enginerandom to avoid everything thinking the same frame
	SetContextThink( &CPropCombineBall::AnimThink_CBE, gpGlobals->curtime + enginerandom->RandomFloat( 0.0f, 0.1f), s_pAnimThinkContext );
...
void CPropCombineBall::CaptureBySpawner_CBE()
{
	CPropCombineBall *cent = (CPropCombineBall *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->CaptureBySpawner();
	}
}
...
void CPropCombineBall::CaptureBySpawner( )
{
	...
	if ( flSpeed > 25.0f )
	{
		...
		SetContextThink( &CPropCombineBall::CaptureBySpawner_CBE, gpGlobals->curtime + 0.01f,	s_pCaptureContext );
		return;
	}
...
{
	SetContextThink( &CPropCombineBall::ExplodeThink_CBE, gpGlobals->curtime + flDuration, s_pExplodeTimerContext );
}
...
void CPropCombineBall::ExplodeThink_CBE()
{
	CPropCombineBall *cent = (CPropCombineBall *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
...
{
		m_hSpawner.Set((pSpawner)?pSpawner->BaseEntity():NULL);
}


void CPropCombineBall::DieThink_CBE()
{
	CPropCombineBall *cent = (CPropCombineBall *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
...
{
	SetContextThink( &CPropCombineBall::WhizSoundThink_CBE, gpGlobals->curtime + 2.0f * TICK_INTERVAL, s_pWhizThinkContext );
}

void CPropCombineBall::WhizSoundThink_CBE()
{
	CPropCombineBall *cent = (CPropCombineBall *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
	...
	if ( pPlayer )
	{
		...
		if ( DotProduct( vecDelta, vecVelocity ) > 0.5f )
		{
			...
			if ( flDist < 200.0f )
			{
				...
				SetContextThink( &CPropCombineBall::WhizSoundThink_CBE, gpGlobals->curtime + 0.5f, s_pWhizThinkContext );
				return;
			}
		}
	}
...
void CPropCombineBall::OnPhysGunPickup( CBaseEntity *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if ( reason != PUNTED_BY_CANNON )
	{
		...
		SetContextThink( &CPropCombineBall::DissolveRampSoundThink_CBE, gpGlobals->curtime + GetBallHoldSoundRampTime(), s_pHoldDissolveContext );
...
// Activate death-spin!
//-----------------------------------------------------------------------------
void CPropCombineBall::OnPhysGunDrop( CBaseEntity *pPhysGunUser, PhysGunDrop_t Reason )
{
	...
	{
		CEntity *cent = CEntity::Instance(pPhysGunUser);
		// Choose a enginerandom direction (forward facing)
		Vector vecForward;
		cent->GetVectors( &vecForward, NULL, NULL );
...
void CPropCombineBall::DissolveRampSoundThink_CBE()
{
	CPropCombineBall *cent = (CPropCombineBall *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->DissolveRampSoundThink();
	}
}
...
void CPropCombineBall::DissolveRampSoundThink( )
{
	...
	SetContextThink( &CPropCombineBall::DissolveThink_CBE, gpGlobals->curtime + dt, s_pHoldDissolveContext );
}

void CPropCombineBall::DissolveThink_CBE( )
{
	CPropCombineBall *cent = (CPropCombineBall *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
...
float CPropCombineBall::GetBallHoldDissolveTime()
{
	...
	if( g_helpfunc.GameRules_IsSkillLevel( 1 ) && hl2_episodic->GetBool() )
	{
...
void CPropCombineBall::CQ_DoExplosion( )
{
	CPropCombineBall *cent = (CPropCombineBall *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
...
void CPropCombineBall::DoExplosion( )
{
	...
	if ( PhysIsInCallback() )
	{
		VALVE_BASEPTR valve_func;
		void (CEntity::*cent_func)() = static_cast <void (CEntity::*)()> (&CPropCombineBall::CQ_DoExplosion);
		memcpy(&valve_func, &cent_func, sizeof(void *));
		g_PostSimulationQueue->QueueCall( BaseEntity(), valve_func );
		return;
	}
	...
	if( hl2_episodic->GetBool() )
	{
		g_helpfunc.CSoundEnt_InsertSound( SOUND_COMBAT | SOUND_CONTEXT_EXPLOSION, WorldSpaceCenter(), 180, 0.25, BaseEntity() );
	}
	...
	SetContextThink( &CPropCombineBall::SUB_Remove_CBE, gpGlobals->curtime + 0.5f, s_pRemoveContext );
	StopLoopingSounds();
}
...
bool CPropCombineBall::DissolveEntity( CEntity *pEntity )
{
	...
	if( !pEntity->IsNPC() && !(dynamic_cast<CE_CRagdollProp*>(pEntity)) )
		return false;
...
void CPropCombineBall::OnHitEntity( CEntity *pHitEntity, float flSpeed, int index, gamevcollisionevent_t *pEvent )
{
	...
	CTakeDamageInfo info( BaseEntity(), (owner)?owner->BaseEntity():NULL, GetAbsVelocity(), GetAbsOrigin(), sk_npc_dmg_combineball.GetFloat(), DMG_DISSOLVE );
	...
	if ( pBCC )
	{
		bShouldHit = pBCC->IRelationType( (owner)?owner->BaseEntity():NULL ) != D_LI;
	}
	...
	if ( !bIsDissolving && bShouldHit == true )
	{
		if ( pHitEntity->PassesDamageFilter( info ) )
		{
			...
			{
				if ( (m_nState == STATE_THROWN) && (pHitEntity->IsNPC() || dynamic_cast<CE_CRagdollProp*>(pHitEntity) ))
				{
					EmitSound( "NPC_CombineBall.KillImpact" );
				}
...
void CPropCombineBall::DeflectTowardEnemy( float flSpeed, int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( bSeekKill )
	{
		...
		for ( int i = 0; i < nCount; i++ )
		{
			...
			if ( !IsAttractiveTarget( cent ) )
				continue;

			VectorSubtract( cent->WorldSpaceCenter(), vecStartPoint, vecDelta );
...
void CPropCombineBall::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( IsInField() )
	{
		...
		g_CEventQueue->AddEvent( BaseEntity(), "FadeAndRespawn", 0.01, NULL, NULL );
		return;
	}
	...
	SetContextThink( &CPropCombineBall::WhizSoundThink_CBE, gpGlobals->curtime + 0.01, s_pWhizThinkContext );
...
void CPropCombineBall::AnimThink_CBE()
{
	CPropCombineBall *cent = (CPropCombineBall *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
...
{
	StudioFrameAdvance();
	SetContextThink( &CPropCombineBall::AnimThink_CBE, gpGlobals->curtime + 0.1f, s_pAnimThinkContext );
}
...
void CFuncCombineBallSpawner::Spawn()
{
	...
	float flWidth = CollisionProp_Actual()->OBBSize().x;
	float flHeight = CollisionProp_Actual()->OBBSize().y;
	...
	{
		flDeltaT = (CollisionProp_Actual()->OBBSize().z - 2 * m_flBallRadius) / ((m_flMinSpeed + m_flMaxSpeed) * 0.5f);
...
{
	float flXBoundary = ( CollisionProp_Actual()->OBBSize().x != 0 ) ? m_flBallRadius / CollisionProp_Actual()->OBBSize().x : 0.0f;
	float flYBoundary = ( CollisionProp_Actual()->OBBSize().y != 0 ) ? m_flBallRadius / CollisionProp_Actual()->OBBSize().y : 0.0f;
	if ( flXBoundary > 0.5f )
	{
		flXBoundary = 0.5f;
	}
...
{
	float flXRange = m_flRadius / CollisionProp_Actual()->OBBSize().x;
	float flYRange = m_flRadius / CollisionProp_Actual()->OBBSize().y;
...
//-----------------------------------------------------------------------------
// Choose a enginerandom point on the 
//-----------------------------------------------------------------------------
...
LINK_ENTITY_TO_CUSTOM_CLASS( point_combine_ball_launcher, info_target, CPointCombineBallLauncher );
...
void CPointCombineBallLauncher::SpawnBall()
{
	...
	qAngle = qAngle + QAngle ( enginerandom->RandomFloat( -m_flConeDegrees, m_flConeDegrees ), enginerandom->RandomFloat( -m_flConeDegrees, m_flConeDegrees ), 0 );
	...
	DispatchSpawn(pBall->BaseEntity());
	pBall->Activate();
	...
	if( GetSpawnFlags() & SF_COMBINE_BALL_LAUNCHER_ATTACH_BULLSEYE )
	{
		...
		if( pBullseye )
		{
			...
			pBullseye->DispatchKeyValue( "solid", "6" );
			pBullseye->DispatchKeyValue( "targetname", STRING(m_iszBullseyeName) );
			pBullseye->Spawn();

			DispatchSpawn(pBullseye->BaseEntity());
			pBullseye->Activate();

			pBullseye->SetParent(pBall->BaseEntity());
...
//	> FilterClass
// ###################################################################
class CFilterCombineBall : public CE_CBaseFilter
{
public:
	CE_DECLARE_CLASS( CFilterCombineBall, CE_CBaseFilter );
	DECLARE_DATADESC();
	...
	{
		CPropCombineBall *pBall = dynamic_cast<CPropCombineBall*>(CEntity::Instance(pEntity) );
		if ( pBall )
		{
...
LINK_ENTITY_TO_CUSTOM_CLASS( filter_combineball_type, filter_base, CFilterCombineBall );
