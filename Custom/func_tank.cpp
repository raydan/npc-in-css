...
void CFuncTank::InputFindNPCToManTank( inputdata_t &inputdata )
{
	...
	if ( pEntity )
	{
		...
		if ( pNPC )
		{
			...
			{
				m_hController.Set(pNPC->BaseEntity());
	...
	for ( int iAI = 0; iAI < nAICount; ++iAI )
	{
		...
		if ( pNPC->GetBehavior( &pBehavior ) )
		{
			...
			if ( pEnemy )
			{
				...
				if ( !pNPC->FVisible_Vector( vecMountPos + pNPC->GetViewOffset() ) )
					continue;
			}

			trace_t tr;
			UTIL_TraceEntity( pNPC, vecMountPos, vecMountPos, MASK_NPCSOLID, BaseEntity(), pNPC->GetCollisionGroup(), &tr );
...
LINK_ENTITY_TO_CUSTOM_CLASS( func_tankrocket, info_target, CFuncTankRocket );
...
void CFuncTankRocket::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CEntity *pAttacker, bool bIgnoreSpread )
{
	CMissile *pRocket = (CMissile *) CEntity::Create( "rpg_missile", barrelEnd, GetAbsAngles(), BaseEntity() );
...
LINK_ENTITY_TO_CUSTOM_CLASS( func_tankapcrocket, info_target, CFuncTankAPCRocket );

void CFuncTankAPCRocket::Precache( void )
{
	g_helpfunc.UTIL_PrecacheOther( "apc_missile" );
...
void CFuncTankAPCRocket::Spawn( void )
{
	...
	m_nSide = 0;
	m_bDying = false;
	CEntity *cent = CreateLaserDot( GetAbsOrigin(), BaseEntity(), false );
	m_hLaserDot.Set((cent)?cent->BaseEntity():NULL);
...
void CFuncTankAPCRocket::FireDying( const Vector &barrelEnd )
{
	...
	Vector vecVelocity;
	VectorMultiply( vecDir, m_flRocketSpeed * enginerandom->RandomFloat( 0.75f, 1.25f ), vecVelocity );

	QAngle angles;
	VectorAngles( vecDir, angles );

	CAPCMissile *pRocket = (CAPCMissile *) CAPCMissile::Create( barrelEnd, angles, vecVelocity, this );
	float flDeathTime = enginerandom->RandomFloat( 0.3f, 0.5f );
	if ( enginerandom->RandomFloat( 0.0f, 1.0f ) < 0.3f )
	{
	...
	// Make erratic firing
	m_fireRate = enginerandom->RandomFloat( DEATH_VOLLEY_MIN_FIRE_RATE, DEATH_VOLLEY_MAX_FIRE_RATE ); 
	if ( --m_nBulletCount <= 0 )
	{
		UTIL_Remove( this );
	}
...
void CFuncTankAPCRocket::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CEntity *pAttacker, bool bIgnoreSpread )
{
	...
	if ( --m_nBulletCount <= 0 )
	{
		...
		m_fireLast += enginerandom->RandomFloat( 2.0f, 3.0f );
	}
...
void CFuncTankAPCRocket::Think()
{
	...
	CEntity *cent = m_hFuncTankTarget;
	SetLaserDotTarget( m_hLaserDot, (cent)?cent->BaseEntity():NULL );
	EnableLaserDot( m_hLaserDot, cent != NULL );
