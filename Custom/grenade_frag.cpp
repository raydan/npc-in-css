...
void CGrenadeFrag::UpdateOnRemove()
{
	if(m_pMainGlow)
	{
		UTIL_Remove(m_pMainGlow);
		m_pMainGlow.Set(NULL);
	}

	if(m_pGlowTrail)
	{
		UTIL_Remove(m_pGlowTrail);
		m_pGlowTrail.Set(NULL);
	}
...
void CGrenadeFrag::CreateEffects( void )
{
	...
	CE_CSprite *glow = CE_CSprite::SpriteCreate( "sprites/redglow1.vmt", GetLocalOrigin(), false );

	int	nAttachment = LookupAttachment( "fuse" );

	if ( glow != NULL )
	{
		m_pMainGlow.Set(glow->BaseEntity());
		glow->FollowEntity( BaseEntity() );
		glow->SetAttachment( BaseEntity(), nAttachment );
		glow->SetTransparency( kRenderGlow, 255, 255, 255, 200, kRenderFxNoDissipation );
		glow->SetScale( 0.2f );
		glow->SetGlowProxySize( 4.0f );
	}

	// Start up the eye trail
	CE_CSpriteTrail *trail = CE_CSpriteTrail::SpriteTrailCreate( "sprites/bluelaser1.vmt", GetLocalOrigin(), false );

	if ( trail != NULL )
	{
		m_pGlowTrail.Set(trail->BaseEntity());
		trail->FollowEntity( BaseEntity() );
		trail->SetAttachment( BaseEntity(), nAttachment );
		trail->SetTransparency( kRenderTransAdd, 255, 0, 0, 255, kRenderFxNone );
		trail->SetStartWidth( 8.0f );
		trail->SetEndWidth( 1.0f );
		trail->SetLifeTime( 0.5f );
	}
}
...
void CGrenadeFrag::VPhysicsUpdate( IPhysicsObject *pPhysics )
{
	...
	if ( tr.DidHit() )
	{
		...
		CTakeDamageInfo info( BaseEntity(), (thrower)?thrower->BaseEntity():NULL, pPhysics->GetMass() * vel, GetAbsOrigin(), 0.1f, DMG_CRUSH );
...
{
	CCombatCharacter *cent = ToBaseCombatCharacter(pOwner);

	// Don't set the owner here, or the player can't interact with grenades he's thrown
	CGrenadeFrag *pGrenade = (CGrenadeFrag *)CEntity::Create( "npc_grenade_frag", position, angles, (pOwner)?pOwner->BaseEntity():NULL );
	
	pGrenade->SetTimer( timer, timer - FRAG_GRENADE_WARN_TIME );
	pGrenade->SetVelocity( velocity, angVelocity );
	pGrenade->SetThrower( (cent)?cent->BaseEntity():NULL );
